...
from utils import get_current_user, encrypt_data, decrypt_data, hash_password, verify_password, create_token, generate_otp, generate_backup_code, send_otp_email, send_otp_sms, log_activity, send_push_notification, send_push_to_all_admins, update_estate_readiness
import uuid
import os
import asyncio
import base64
...
async def send_beneficiary_invitation(beneficiary_id: str, current_user: dict = Depends(get_current_user)):
    ...
    try:
        if RESEND_API_KEY:
            ...
            frontend_url = os.environ.get('FRONTEND_URL', 'https://platform-handoff.preview.emergentagent.com')
