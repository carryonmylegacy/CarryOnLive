...
async def designate_beneficiaries(
    ...
    estate = await db.estates.find_one(
        {"id": doc["estate_id"], "owner_id": current_user["id"]}, {"_id": 0, "id": 1, "name": 1}
    )
    if not estate:
        raise HTTPException(status_code=403, detail="Access denied")
    ...
    # this document. See _notify_newly_pre_shared below.
...
# document pre-transition listing in this file). Kept in one place so
# share-notification + access-control stay in lockstep.
