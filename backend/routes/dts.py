...
async def approve_dts_line_item(
    task_id: str,
    item_id: str,
    approved: bool,
...
async def update_dts_task(
    ...
    await log_activity(
        ...
        "dts_task_edited",
        f"DTS task edited and sent for re-quoting: {data.title or task['title']}",
    )
...
async def get_certificate_document(
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'inline; filename="{cert["file_name"]}"'},
    )
