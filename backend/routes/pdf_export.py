...
async def export_estate_pdf(
    ...
    pdf.cell(
        0,
        15,
        _safe("CarryOn Estate Plan Summary"),
        new_x="LMARGIN",
        new_y="NEXT",
        align="C",
    )
    ...
    pdf.cell(
        0,
        10,
        _safe("  ACTION ITEMS - What You Need To Do"),
        new_x="LMARGIN",
        new_y="NEXT",
        fill=True,
    )
    ...
    if pending_items:
        ...
        pdf.cell(
            0,
            8,
            f"Pending Checklist Items ({len(pending_items)}):",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.set_font("Helvetica", "", 10)
    ...
    if pending_dts:
        ...
        for t in pending_dts:
            ...
            pdf.cell(
                0,
                6,
                _safe(f"[ ] {t.get('title', '')} - Status: {t.get('status', '')}"),
                new_x="LMARGIN",
                new_y="NEXT",
            )
        pdf.ln(3)
    ...
    pdf.cell(
        0,
        10,
        _safe("  ESTATE STATUS - Current Snapshot"),
        new_x="LMARGIN",
        new_y="NEXT",
        fill=True,
    )
    ...
    pdf.cell(
        0,
        8,
        f"Secure Document Vault ({len(documents)} documents):",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.set_font("Helvetica", "", 10)
    ...
    for b in beneficiaries:
        ...
        pdf.cell(
            0,
            5,
            _safe(f"  {name} - {rel} (Status: {ben_status})"),
            new_x="LMARGIN",
            new_y="NEXT",
        )
    pdf.ln(3)
    ...
    pdf.cell(
        0,
        8,
        f"Immediate Action Checklist ({completed}/{total} completed):",
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.ln(3)
    ...
    if dts_tasks:
        ...
        pdf.cell(
            0,
            8,
            f"Designated Trustee Services ({len(dts_tasks)} tasks):",
            new_x="LMARGIN",
            new_y="NEXT",
        )
        pdf.set_font("Helvetica", "", 10)
        for t in dts_tasks:
            ...
            pdf.cell(
                0,
                5,
                _safe(f"  {t.get('title', '')} - {t.get('status', '')}"),
                new_x="LMARGIN",
                new_y="NEXT",
            )
        pdf.ln(3)
    ...
    pdf.cell(
        0,
        5,
        _safe("CarryOn - Every American Family. Ready. - carryon.us"),
        new_x="LMARGIN",
        new_y="NEXT",
        align="C",
    )
