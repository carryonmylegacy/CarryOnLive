...
async def export_estate_pdf(estate_id: str, current_user: dict = Depends(get_current_user)):
    ...
    user_state = user.get("address_state") or estate.get("state") or "Not specified"
    ...
    jurisdiction_text = (
        ...
        f"This analysis is informed by {user_state} estate planning statutes and probate rules."
    )
    pdf.multi_cell(0, 5, _safe(jurisdiction_text), border=1, fill=True, align="C")
    pdf.ln(3)
    ...
    pdf.multi_cell(
        ...
        3.5,
        _safe(
            "DISCLAIMER: This summary is generated by an AI assistant for informational purposes only and does not constitute legal advice. "
            "CarryOn Estate Guardian is not a licensed attorney. For legally binding decisions, always consult a bar-certified "
            f"attorney licensed in {user_state}. No attorney-client relationship is created by using this service."
        ),
        border=1,
        fill=True,
    )
