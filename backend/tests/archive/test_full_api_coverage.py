...
class TestEstates:
    ...
    def test_create_estate(self, api_client):
        ...
        api_client.headers.update({"Authorization": f"Bearer {TestState.access_token}"})
        response = api_client.post(
            f"{BASE_URL}/api/estates", json={"name": "Test Estate"}
        )
        assert response.status_code == 200
        data = response.json()
        assert "id" in data
    ...
    def test_update_estate(self, api_client):
        ...
        response = api_client.patch(
            f"{BASE_URL}/api/estates/{TestState.estate_id}",
            json={"name": "Updated Test Estate", "state": "California"},
        )
        assert response.status_code == 200
    ...
    def test_get_estate_readiness(self, api_client):
        ...
        response = api_client.get(
            f"{BASE_URL}/api/estate/{TestState.estate_id}/readiness"
        )
        assert response.status_code == 200
        data = response.json()
...
class TestBeneficiaries:
    ...
    def test_update_beneficiary(self, api_client):
        ...
        response = api_client.put(
            f"{BASE_URL}/api/beneficiaries/{TestState.beneficiary_id}",
            json={
                "estate_id": TestState.estate_id,
                "first_name": "John",
                "last_name": "Doe",
...
class TestChecklist:
    ...
    def test_update_checklist_item(self, api_client):
        ...
        response = api_client.put(
            f"{BASE_URL}/api/checklists/{TestState.checklist_id}",
            json={"title": "Contact Estate Attorney", "priority": "critical"},
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_toggle_checklist_item(self, api_client):
        ...
        response = api_client.patch(
            f"{BASE_URL}/api/checklists/{TestState.checklist_id}/toggle"
        )
        assert response.status_code == 200
        data = response.json()
...
class TestMessages:
    ...
    def test_update_message(self, api_client):
        ...
        response = api_client.put(
            f"{BASE_URL}/api/messages/{TestState.message_id}",
            json={
...
class TestGuardianAI:
    ...
    def test_chat_requires_auth(self, api_client):
        ...
        response = no_auth_client.post(
            ...
            json={"message": "Hello", "estate_id": TestState.estate_id},
        )
        assert response.status_code in [401, 403]
...
class TestSubscriptions:
    ...
    def test_get_subscription_plans(self, api_client):
        ...
        print(
            f"✓ Subscription plans: {len(data['plans'])} plans, beta_mode={data['beta_mode']}"
        )
...
class TestAdminEndpoints:
    ...
    def test_admin_stats(self, api_client):
        ...
        assert "documents" in data
        print(
            f"✓ Admin stats: users={data['users']['total']}, estates={data['estates']['total']}"
        )
