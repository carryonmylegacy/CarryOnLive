...
KNOWN = {
    ...
    ("plans_payload.tier_features", "seniors"): "B1 tier_features built from TIER_IDS (status.py:38) — no seniors column",
    ("apple_webhook.APPLE_TO_PLAN", "ben_new_adult"): "B2 routes/subscriptions/apple_webhook.py:25-72 lacks ben_new_adult",
    ...
    ("SubscriptionManagement.TIER_STYLES", "seniors"): "B4 components/settings/SubscriptionManagement.js:20-36 lacks seniors",
    ("SubscriptionManagement.TIER_STYLES", "ben_seniors"): "B4 components/settings/SubscriptionManagement.js:20-36 lacks ben_seniors",
    ("SubscriptionManagement.TIER_STYLES", "ben_new_adult"): "B4 components/settings/SubscriptionManagement.js:20-36 lacks ben_new_adult",
    ("SubscriptionManagement.TIER_STYLES", "ben_enterprise"): "B4 components/settings/SubscriptionManagement.js:20-36 lacks ben_enterprise",
}
for _b in BEN_TIERS:
    KNOWN[("checkout.beneficiary", _b)] = "B5 routes/subscriptions/checkout.py:76-79 resolves settings['plans'] only → 400 Invalid plan for every ben_*"
...
def test_catalog_plan_order_matches_default_plans():
    ...
    assert set(BEN_PLAN_ORDER) == set(BEN_PLAN), f"BEN_PLAN_ORDER vs BENEFICIARY_PLANS drift: {set(BEN_PLAN_ORDER) ^ set(BEN_PLAN)}"
...
def test_feature_gates_tier_ids_cover_tier(tier):
    ...
    assert tier in TIER_IDS, f"{tier}: feature_gates.TIER_IDS lacks it → is_feature_enabled_for_user() is False for every feature"
...
def test_apple_product_map_covers_paid_tier(tier):
    assert tier in _apple_plan_ids(), f"{tier}: no App Store product maps to it → purchase logs 'unknown product', nothing activates"
...
def test_admin_users_valid_tiers_cover_tier(tier):
    assert tier in _literal_list_after("routes/admin/users.py", "valid_tiers = ["), f"{tier}: admin cannot assign this tier"
...
def test_admin_bulk_ops_valid_tiers_cover_tier(tier):
    assert tier in _literal_list_after("routes/admin/bulk_ops.py", "valid_tiers = ["), f"{tier}: bulk tier assignment rejects it"
...
FRONTEND_MAPS = {
    ...
    "SubscriptionManagement.TIER_STYLES": ("components/settings/SubscriptionManagement.js", "TIER_STYLES", TIERS + BEN_TIERS),
    # Landing pricing lists consumer tiers only; enterprise is sold B2B and never rendered there.
    "LandingPricing.TIER_ICON": ("components/landing/LandingPricing.js", "TIER_ICON", [t for t in TIERS if t != "enterprise"]),
    "LandingPricing.TIER_ACCENT": ("components/landing/LandingPricing.js", "TIER_ACCENT", [t for t in TIERS if t != "enterprise"]),
}


@pytest.mark.parametrize(
    "stage,tier",
    [pytest.param(s, t.values[0], id=f"{s}-{t.values[0]}", marks=t.marks) for s, (_, _, ts) in FRONTEND_MAPS.items() for t in tier_params(s, ts)],
)
...
def test_iap_products_cover_paid_tier(tier):
    ...
    assert re.search(rf"^\s*{tier}_monthly\s*:", src, re.M), f"{tier}: iap.js has no App Store product → native paywall cannot sell it"
...
def test_status_resolves_beneficiary_to_own_tier(world, tier):
    ...
    assert row["beneficiary_locked_tier"] == f"ben_{tier}", f"{tier}: beneficiary_locked_tier={row['beneficiary_locked_tier']!r}"
    assert row["beneficiary_synth_plan_id"] == f"ben_{tier}", f"{tier}: synthesized subscription.plan_id={row['beneficiary_synth_plan_id']!r}"
...
def test_plans_payload_prices_match_catalog(world, tier):
    ...
    assert plan and _close(plan["price"], PLAN[tier]["price"]), f"{tier}: paywall card price {plan and plan.get('price')} != catalog {PLAN[tier]['price']}"
...
def test_plans_payload_has_feature_column_for_tier(world, tier):
    assert tier in world["plans_payload"]["tier_features"], f"{tier}: paywall card gets no feature list (tier_features has no {tier} key)"
...
def test_family_plan_benefactor_quote(world, tier):
    ...
    assert _close(pv["current_price"], price) and _close(pv["family_price"], round(price * (1 - BENEFACTOR_DISC / 100), 2)), (
...
def test_checkout_amount_matches_catalog_for_benefactor(world, tier):
    ...
    for cycle in ("monthly", "quarterly", "annual"):
        ...
        assert (got["amount"] is None and exp is None) or _close(got["amount"], exp), f"{tier}/{cycle}: Stripe amount {got['amount']} != {exp}"
