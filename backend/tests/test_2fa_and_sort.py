...
class Test2FAPreference:
    ...
    def test_get_2fa_preference(self, admin_session):
        ...
        response = requests.get(
            f"{BASE_URL}/api/auth/2fa-preference",
    ...
    def test_put_2fa_preference_blocked_when_global_off(self, admin_session):
        ...
        if global_disabled:
            ...
            response = requests.put(
                f"{BASE_URL}/api/auth/2fa-preference",
                headers=admin_session["headers"],
                json={"otp_enabled": True}
            )
            assert response.status_code == 400, f"Expected 400 when enabling 2FA while global is off: {response.status_code}"
    ...
    def test_put_2fa_preference_disable_allowed(self, admin_session):
        ...
        response = requests.put(
            f"{BASE_URL}/api/auth/2fa-preference",
            headers=admin_session["headers"],
            json={"otp_enabled": False}
        )
        assert response.status_code == 200, f"PUT 2fa-preference (disable) failed: {response.text}"
        data = response.json()
        assert data.get("otp_enabled") == False, f"Expected otp_enabled=false: {data}"
