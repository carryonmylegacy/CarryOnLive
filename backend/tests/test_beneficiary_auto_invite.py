...
class TestBeneficiaryAutoInvite:
        ...
        if login_resp.status_code == 200:
            ...
            if data.get("otp_required"):
                ...
                otp_resp = requests.post(
                    ...
                    json={"email": ADMIN_EMAIL, "otp": "000000", "trust_today": True}
                )
    ...
    def estate_id(self, admin_token):
        ...
        estates = resp.json()
        owned = [e for e in estates if e.get("user_role_in_estate") == "owner" or (not e.get("user_role_in_estate") and not e.get("is_beneficiary_estate"))]
    ...
    def test_create_beneficiary_with_email_returns_auto_invited_true(self, admin_token, estate_id):
        ...
        assert data["auto_invited"] is True, f"auto_invited should be true when email provided, got {data['auto_invited']}"
        assert data.get("invitation_token") is not None, "Should have invitation_token when email provided"
        assert data.get("invitation_status") == "pending", f"invitation_status should be pending, got {data.get('invitation_status')}"
...
class TestBenefactorReminderSettings:
    ...
    def test_update_profile_accepts_hide_benefactor_reminder(self, admin_token):
        ...
        original_value = me_resp.json().get("hide_benefactor_reminder", False)
        
        # Set to true
        update_resp = requests.put(
            f"{BASE_URL}/api/auth/profile",
            json={"hide_benefactor_reminder": True},
            headers=headers
        )
        ...
        assert verify_resp.json().get("hide_benefactor_reminder") is True, "hide_benefactor_reminder should be True after update"
        
        # Set back to false
        restore_resp = requests.put(
            f"{BASE_URL}/api/auth/profile",
            json={"hide_benefactor_reminder": False},
            headers=headers
        )
        ...
        assert final_resp.json().get("hide_benefactor_reminder") is False, "hide_benefactor_reminder should be False after restore"
