...
class TestBeneficiaryInvitationFlow:
    ...
    def test_01_login_barnetharris(self):
        ...
        login_response = self.session.post(
            ...
            json={"email": "barnetharris@mac.com", "password": "9170873"},
        )
        ...
        verify_response = self.session.post(
            ...
            json={"email": "barnetharris@mac.com", "otp": otp},
        )
        ...
        print(
            f"  User: {verify_data['user']['name']}, Role: {verify_data['user']['role']}"
        )
    ...
    def test_03_get_beneficiaries(self):
        ...
        assert self.__class__.token and self.__class__.estate_id, (
    ...
    def test_04_create_beneficiary_with_demographics(self):
        ...
        assert created["address_street"] == "123 Test Street, Apt 4B", (
            "Street not saved"
        )
        ...
        assert created["notes"] == "Test beneficiary for automated testing", (
            "Notes not saved"
        )
        assert created["invitation_status"] == "pending", (
    ...
    def test_05_send_invitation(self):
        ...
        assert self.__class__.token and self.__class__.created_beneficiary_id, (
            "Prerequisites not met"
        )
        ...
        assert response.status_code == 200, (
            f"Failed to send invitation: {response.text}"
        )
        ...
        test_ben = next(
            (
                ...
                if b["id"] == self.__class__.created_beneficiary_id
            ),
            None,
        )
        assert test_ben, "Test beneficiary not found"
        assert test_ben.get("invitation_token"), "No invitation token set"
        assert test_ben.get("invitation_status") == "sent", (
            f"Status should be 'sent', got: {test_ben.get('invitation_status')}"
        )
    ...
    def test_06_get_invitation_details(self):
        ...
        assert data["beneficiary"]["first_name"] == "TEST_John", (
        ...
        print(
            f"  Beneficiary: {data['beneficiary']['first_name']} {data['beneficiary']['last_name']}"
        )
    ...
    def test_07_accept_invitation(self):
        ...
        assert response.status_code == 200, (
            f"Failed to accept invitation: {response.text}"
        )
        data = response.json()
    ...
    def test_08_verify_beneficiary_status_updated(self):
        ...
        assert test_ben, "Test beneficiary not found"
        assert test_ben["invitation_status"] == "accepted", (
            f"Status should be 'accepted', got: {test_ben['invitation_status']}"
        )
