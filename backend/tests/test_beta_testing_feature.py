...
class TestBetaFeatures:
    ...
    def test_07_toggle_beta_nonexistent_user(self, api_client):
        ...
        resp = api_client.put(
            f"{BASE_URL}/api/admin/user/{fake_user_id}/beta",
            json={"is_beta": True},
            headers=self.get_admin_headers()
        )
    ...
    def test_11_auth_me_returns_beta_fields(self, api_client):
        ...
        print(f"auth/me response includes: is_beta_tester={data.get('is_beta_tester')}, beta_accepted={data.get('beta_accepted')}")
    ...
    def test_12_subscription_status_returns_beta_fields(self, api_client):
        ...
        print(f"subscription/status: is_beta_tester={data.get('is_beta_tester')}, beta_accepted={data.get('beta_accepted')}, free_access={data.get('free_access')}")
