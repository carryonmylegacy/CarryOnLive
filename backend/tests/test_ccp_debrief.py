...
class TestCCPDebriefEndpoints:
    ...
    def test_debrief_returns_401_without_auth(self):
        ...
        response = self.session.post(f"{BASE_URL}/api/ccp/debrief/fake-activation-id", json={
            "rating": 4,
            "went_well": "Test",
            "to_improve": "Test"
        })
    ...
    def test_debrief_returns_400_for_invalid_rating_zero(self):
        ...
        data = response.json()
        assert "rating" in data.get("detail", "").lower() or "1" in data.get("detail", ""), f"Expected rating error, got {data}"
    ...
    def test_debrief_returns_400_for_invalid_rating_six(self):
        ...
        assert response.status_code == 400, f"Expected 400, got {response.status_code}"
        data = response.json()
        assert "rating" in data.get("detail", "").lower() or "5" in data.get("detail", ""), f"Expected rating error, got {data}"
    ...
    def test_debrief_returns_404_for_nonexistent_activation(self):
        ...
        self.session.headers.update({"Authorization": f"Bearer {token}"})
        response = self.session.post(f"{BASE_URL}/api/ccp/debrief/nonexistent-activation-id", json={
            "rating": 4,
            "went_well": "Test went well",
            "to_improve": "Test to improve"
        })
    ...
    def test_debrief_stats_returns_valid_structure(self):
        ...
        if response.status_code == 403:
            print(f"✓ GET /api/ccp/debrief-stats returns 403 for non-member (expected)")
            return
        ...
        print(f"✓ GET /api/ccp/debrief-stats returns valid structure: entries={len(data['entries'])}, total_drills={data['total_drills']}, average_rating={data['average_rating']}")
...
class TestDebriefRequestModel:
    ...
    def test_debrief_accepts_valid_rating_1(self):
        ...
        self.session.headers.update({"Authorization": f"Bearer {token}"})
        response = self.session.post(f"{BASE_URL}/api/ccp/debrief/test-activation", json={
            "rating": 1,
            "went_well": "",
            "to_improve": ""
        })
    ...
    def test_debrief_accepts_valid_rating_5(self):
        ...
        response = self.session.post(f"{BASE_URL}/api/ccp/debrief/test-activation", json={
            "rating": 5,
            "went_well": "Everything was great",
            "to_improve": "Nothing to improve"
        })
        # Should NOT return 400 for valid rating (will return 404 for nonexistent activation)
        assert response.status_code != 400 or "rating" not in response.json().get("detail", "").lower(), \
    ...
    def test_debrief_accepts_empty_notes(self):
        ...
        assert response.status_code != 422, f"Empty notes should be valid, got {response.status_code}: {response.json()}"
