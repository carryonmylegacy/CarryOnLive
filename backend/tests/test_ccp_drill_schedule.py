...
class TestDrillScheduleEndpoint:
    ...
    def test_drill_schedule_endpoint_returns_401_without_auth(self):
        ...
        response = self.session.patch(
            f"{BASE_URL}/api/ccp/plans/fake-plan-id/drill-schedule",
            json={"enabled": True}
        )
    ...
    def test_drill_schedule_endpoint_returns_404_for_nonexistent_plan(self):
        ...
        response = self.session.patch(
            f"{BASE_URL}/api/ccp/plans/nonexistent-plan-id-12345/drill-schedule",
            json={"enabled": True}
        )
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
        data = response.json()
        assert "not found" in data.get("detail", "").lower() or "Plan not found" in data.get("detail", "")
        print(f"✓ PATCH drill-schedule returns 404 for nonexistent plan")
...
class TestPlanCRUDWithDrillSchedule:
    ...
    def test_create_plan_accepts_drill_schedule(self):
        ...
        if response.status_code == 201 or response.status_code == 200:
            ...
            assert data["drill_schedule"]["enabled"] == True
            print(f"✓ POST /api/ccp/plans accepts drill_schedule field")
    ...
    def test_update_plan_accepts_drill_schedule(self):
        ...
        if response.status_code == 200:
            ...
            print(f"✓ PUT /api/ccp/plans accepts drill_schedule field")
        else:
...
class TestDrillReminderFunctions:
    ...
    def test_build_drill_reminder_email_returns_valid_html(self):
        ...
        try:
            ...
            print(f"✓ build_drill_reminder_email returns valid HTML with CarryOn branding")
...
class TestServerLifespanScheduler:
    ...
    def test_drill_scheduler_imported_in_server(self):
        ...
        assert "drill_reminder_scheduler()" in server_content, \
            "drill_reminder_scheduler not called in server.py"
