...
class TestPerRoleDockPreferences:
    ...
    def test_save_benefactor_dock_preferences(self):
        ...
        response = self.session.put(f"{BASE_URL}/api/user-preferences/dock", json={
            "items": test_items,
            "role": "benefactor"
        })
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    ...
    def test_per_role_isolation(self):
        ...
        resp1 = self.session.put(f"{BASE_URL}/api/user-preferences/dock", json={
            "items": benefactor_items,
            "role": "benefactor"
        })
        assert resp1.status_code == 200
        
        # Save beneficiary preferences
        resp2 = self.session.put(f"{BASE_URL}/api/user-preferences/dock", json={
    ...
    def test_dock_requires_auth(self):
        ...
        # PUT without auth
        put_response = no_auth_session.put(f"{BASE_URL}/api/user-preferences/dock", json={
            "items": ["/dashboard"],
            "role": "benefactor"
        })
