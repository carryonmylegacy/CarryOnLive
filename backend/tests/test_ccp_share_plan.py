...
class TestCCPSharePlanAuth:
    ...
    def test_share_plan_returns_401_without_auth(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/ccp/plans/fake-plan-id/share", headers={"Content-Type": "application/json"}
        )
...
class TestCCPSharePlanNotFound:
    ...
    def setup(self):
        ...
        login_response = requests.post(
            f"{BASE_URL}/api/auth/login", json={"email": TEST_USERNAME, "password": TEST_PASSWORD}
        )
    ...
    def test_share_plan_returns_404_for_nonexistent_plan(self):
        ...
        response = requests.post(f"{BASE_URL}/api/ccp/plans/nonexistent-plan-id-12345/share", headers=self.headers)
    ...
    def test_revoke_share_returns_404_for_nonexistent_plan(self):
        ...
        response = requests.delete(f"{BASE_URL}/api/ccp/plans/nonexistent-plan-id-12345/share", headers=self.headers)
        # Could be 404 (not found) or 403 (not owner of estate)
        assert response.status_code in [404, 403], f"Expected 404/403, got {response.status_code}"
...
class TestPublicShareEndpoint:
    ...
    def test_public_endpoint_returns_404_for_invalid_token(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/public/ccp/invalid-token-12345", headers={"Content-Type": "application/json"}
        )
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
        data = response.json()
        assert "detail" in data, "Response should have detail field"
        print("✓ GET /api/public/ccp/{share_token} returns 404 for invalid token")
    ...
    def test_public_endpoint_no_auth_required(self):
        ...
        print("✓ GET /api/public/ccp/{share_token} is public (no auth required) - returns 404 not 401")
...
class TestShareEndpointExists:
    ...
    def test_share_endpoint_exists(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/ccp/plans/test-plan-id/share", headers={"Content-Type": "application/json"}
        )
    ...
    def test_public_endpoint_exists(self):
        ...
        response = requests.get(f"{BASE_URL}/api/public/ccp/test-token")
...
class TestSharePlanResponseStructure:
    ...
    def test_public_endpoint_404_response_structure(self):
        ...
        response = requests.get(f"{BASE_URL}/api/public/ccp/invalid-token-structure-test")
