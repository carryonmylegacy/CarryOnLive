...
class TestDownloadProxy:
    ...
    def test_prepare_download_requires_auth(self):
        ...
        res = requests.post(
            ...
            json={"action": "document", "params": {"document_id": "test123"}, "filename": "test.pdf"},
        )
    ...
    def test_prepare_download_all_valid_actions(self):
        ...
        for action in valid_actions:
            res = requests.post(
                f"{BASE_URL}/api/downloads/prepare",
                headers=self.headers,
                json={"action": action, "params": {}, "filename": f"test_{action}.pdf"},
            )
    ...
    def test_download_token_one_time_use(self):
        ...
        prepare_res = requests.post(
            f"{BASE_URL}/api/downloads/prepare",
            headers=self.headers,
            json={"action": "ega_checklist", "params": {}, "filename": "checklist.pdf"},
        )
    ...
    def test_ccp_plan_download_nonexistent_plan(self):
        ...
        prepare_res = requests.post(
            f"{BASE_URL}/api/downloads/prepare",
            headers=self.headers,
            json={"action": "ccp_plan", "params": {"plan_id": "nonexistent-plan-id-12345"}, "filename": "plan.pdf"},
        )
        assert prepare_res.status_code == 200
        token = prepare_res.json()["token"]
    ...
    def test_ccp_plan_full_flow(self):
        ...
        plan_data = {
            ...
            "rendezvous_points": [{"name": "Primary Location", "address": "123 Main St", "notes": "Meet here first"}],
        ...
        create_res = requests.post(f"{BASE_URL}/api/ccp/plans", headers=self.headers, json=plan_data)
        ...
        prepare_res = requests.post(
            f"{BASE_URL}/api/downloads/prepare",
            headers=self.headers,
            json={"action": "ccp_plan", "params": {"plan_id": self.plan_id}, "filename": "CCP_Test_Plan.pdf"},
        )
        assert prepare_res.status_code == 200
        token = prepare_res.json()["token"]
    ...
    def test_document_download_nonexistent(self):
        ...
        prepare_res = requests.post(
            f"{BASE_URL}/api/downloads/prepare",
            headers=self.headers,
            json={"action": "document", "params": {"document_id": "nonexistent-doc-id-12345"}, "filename": "doc.pdf"},
        )
        assert prepare_res.status_code == 200
        token = prepare_res.json()["token"]
