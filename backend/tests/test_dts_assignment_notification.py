...
class TestDTSTaskAssignment:
    ...
    def test_02_get_operators_list(self, founder_token):
        ...
        if operators:
            print(f"   First operator: {operators[0].get('name', 'N/A')}, role: {operators[0].get('operator_role', 'N/A')}")
        return operators
    ...
    def test_04_dts_status_update_invalid(self, founder_token):
        ...
        print(f"✅ Invalid status rejected correctly (400)")
    ...
    def test_06_dts_assign_invalid_operator(self, founder_token):
        ...
        print(f"✅ Invalid operator assignment rejected correctly (404)")
...
class TestNotificationTriggers:
    ...
    def test_01_founder_notifications_endpoint(self, founder_token):
        ...
        print(f"✅ GET /api/notifications - Founder has {len(notifications)} notifications, unread: {data.get('unread_count', 0)}")
    ...
    def test_03_founder_notification_types(self, founder_token):
        ...
        found_signup_notifications = [
            ...
            if "signup" in n.get("title", "").lower() or 
               "signup" in n.get("body", "").lower() or
               "registered" in n.get("body", "").lower() or
               "new user" in n.get("title", "").lower()
        ]
    ...
    def test_04_benefactor_notifications_endpoint(self, benefactor_token):
        ...
        invitation_notifications = [
            n for n in notifications
            if "invitation" in n.get("title", "").lower() or 
               "invitation" in n.get("body", "").lower() or
               "accepted" in n.get("title", "").lower() or
               "joined" in n.get("title", "").lower()
        ]
    ...
    def test_06_notification_mark_all_read(self, founder_token):
        ...
        assert resp.status_code == 200, f"Failed: {resp.text}"
        print(f"✅ POST /api/notifications/read-all - All marked as read")
...
class TestNewUserRegistrationNotification:
    ...
    def test_registration_endpoint_exists(self):
        ...
        if resp.status_code == 429:
            print(f"⚠️ POST /api/auth/register rate limited (429) - endpoint exists but rate limited")
...
class TestDTSQuoteCreation:
    ...
    def test_01_submit_quote_for_task(self, founder_token):
        ...
        tasks = tasks_resp.json()
        submitted_task = next(
            (t for t in tasks if t.get("status") == "submitted"),
            None
        )
        ...
        if task_resp.status_code == 200:
            ...
            print(f"   Verified: Task status is now 'quoted'")
...
class TestOpsOpsDashboard:
    ...
    def test_01_founder_admin_stats(self, founder_token):
        ...
        if resp.status_code == 200:
            stats = resp.json()
            print(f"✅ GET /api/admin/stats - Dashboard stats accessible")
        else:
