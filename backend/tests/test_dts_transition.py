...
class TestDTSBackend:
    ...
    def test_create_dts_task_benefactor(self, benefactor_session, estate_id):
        ...
        assert response.status_code == 200, f"Failed to create DTS task: {response.text}"
    ...
    def test_get_all_dts_tasks_admin(self, admin_session):
        ...
        assert response.status_code == 200, f"Failed to get all DTS tasks: {response.text}"
    ...
    def test_admin_submit_quote(self, admin_session, benefactor_session, estate_id):
        ...
        response = session.post(f"{BASE_URL}/api/dts/tasks/{task_id}/quote", json=quote_data)
    ...
    def test_admin_update_status(self, admin_session, benefactor_session, estate_id):
        ...
        session, _ = admin_session
        response = session.post(f"{BASE_URL}/api/dts/tasks/{task_id}/status?status=ready")
...
class TestTransitionVerification:
    ...
    def test_reject_certificate_requires_admin(self):
        ...
        assert response.status_code == 403, "Non-admin should not be able to reject certificates"
...
class TestVoiceEndpoint:
    ...
    def test_voice_transcribe_endpoint_exists(self):
        ...
        print(f"✓ Voice transcribe endpoint exists (got {response.status_code} - expected validation error)")
