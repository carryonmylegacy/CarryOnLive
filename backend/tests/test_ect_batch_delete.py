...
class TestECTBatchDelete:
    ...
    def test_batch_delete_returns_400_for_empty_list(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/estate-chat/channels/batch-delete", json={"channel_ids": []}, headers=self.headers
        )
    ...
    def test_batch_delete_returns_400_for_over_50_channels(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/estate-chat/channels/batch-delete", json={"channel_ids": channel_ids}, headers=self.headers
        )
        assert response.status_code == 400, f"Expected 400, got {response.status_code}"
        data = response.json()
    ...
    def test_batch_delete_returns_failed_for_nonexistent_channels(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/estate-chat/channels/batch-delete", json={"channel_ids": fake_ids}, headers=self.headers
        )
    ...
    def test_get_channels_still_works(self):
        ...
        response = requests.get(f"{BASE_URL}/api/estate-chat/channels", headers=self.headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
...
class TestECTChannelCRUD:
    ...
    def setup(self):
        ...
        contacts_res = requests.get(f"{BASE_URL}/api/estate-chat/contacts", headers=self.headers)
        ...
        for ch_id in self.created_channels:
            try:
                requests.delete(
                    f"{BASE_URL}/api/estate-chat/channels/{ch_id}", headers={"Authorization": f"Bearer {self.token}"}
                )
            except Exception:
                pass
    ...
    def test_create_and_single_delete_channel(self):
        ...
        create_res = requests.post(
            ...
            json={"estate_id": self.estate_id, "channel_type": "direct", "member_ids": [self.other_member_id]},
        ...
        delete_res = requests.delete(
            f"{BASE_URL}/api/estate-chat/channels/{channel_id}", headers={"Authorization": f"Bearer {self.token}"}
        )

        assert delete_res.status_code == 200, f"Single delete failed: {delete_res.text}"
        data = delete_res.json()
        assert data.get("success"), "Delete should return success: true"
    ...
    def test_create_and_batch_delete_channels(self):
        ...
        channels_res = requests.get(f"{BASE_URL}/api/estate-chat/channels", headers=self.headers)
