...
class TestEditFunctionality:
    ...
    def test_create_test_beneficiary(self, auth_headers, estate_id):
        ...
        response = requests.post(f"{BASE_URL}/api/beneficiaries", json=payload, headers=auth_headers)
    ...
    def test_update_beneficiary(self, auth_headers, estate_id):
        ...
        create_response = requests.post(f"{BASE_URL}/api/beneficiaries", json=create_payload, headers=auth_headers)
        assert create_response.status_code == 200, f"Failed to create beneficiary: {create_response.text}"
        ...
        assert update_response.status_code == 200, f"Failed to update beneficiary: {update_response.text}"
        
        updated = update_response.json()
        
        # Verify all fields were updated
        assert updated["first_name"] == "TEST_Updated", f"First name not updated: {updated['first_name']}"
        assert updated["middle_name"] == "NewMiddle", f"Middle name not updated: {updated['middle_name']}"
        assert updated["last_name"] == "Modified", f"Last name not updated: {updated['last_name']}"
        assert updated["suffix"] == "Jr.", f"Suffix not updated: {updated['suffix']}"
        assert updated["relation"] == "Sibling", f"Relation not updated: {updated['relation']}"
        assert updated["phone"] == "+1-555-9999", f"Phone not updated: {updated['phone']}"
        assert updated["address_street"] == "789 Updated Ave", f"Address not updated: {updated['address_street']}"
        assert updated["address_city"] == "Updated City", f"City not updated: {updated['address_city']}"
        assert updated["address_state"] == "TX", f"State not updated: {updated['address_state']}"
        assert updated["notes"] == "Updated notes after edit", f"Notes not updated: {updated['notes']}"
        assert updated["avatar_color"] == "#10b981", f"Avatar color not updated: {updated['avatar_color']}"
        
        print(f"✓ Successfully updated beneficiary {beneficiary_id}")
        print(f"  Name: {updated['name']}")
        print(f"  Relation: {updated['relation']}")
        print(f"  Address: {updated['address_street']}, {updated['address_city']}, {updated['address_state']}")
        
        # Cleanup - delete test beneficiary
        delete_response = requests.delete(f"{BASE_URL}/api/beneficiaries/{beneficiary_id}", headers=auth_headers)
        assert delete_response.status_code == 200, f"Failed to delete test beneficiary: {delete_response.text}"
    ...
    def test_update_document(self, auth_headers, estate_id):
        ...
        update_response = requests.put(
            ...
            headers={"Authorization": auth_headers["Authorization"]}  # No Content-Type for form data
        )
        ...
        assert updated["category"] == update_data["category"], f"Category not updated: {updated['category']}"
        assert updated["notes"] == update_data["notes"], f"Notes not updated: {updated.get('notes')}"
        ...
        for ben in beneficiaries:
            if ben.get("first_name", "").startswith("TEST_"):
                ...
                if delete_response.status_code == 200:
                    deleted_count += 1
                    print(f"Deleted test beneficiary: {ben['first_name']} {ben['last_name']}")
