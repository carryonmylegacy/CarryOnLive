...
class TestEstateRenameCheck:
    ...
    def benefactor_token(self):
        ...
        response = requests.post(f"{BASE_URL}/api/auth/login", json={
            "email": TEST_BENEFACTOR_EMAIL,
            "password": TEST_BENEFACTOR_PASSWORD
        })
        if response.status_code != 200:
    ...
    def test_rename_check_returns_needs_rename_true_for_default_name(self, benefactor_token):
        ...
        response = requests.get(
            f"{BASE_URL}/api/estates/rename-check",
            headers={"Authorization": f"Bearer {benefactor_token}"}
        )
        assert response.status_code == 200
        data = response.json()
        ...
        assert data.get("needs_rename") == True, f"Expected needs_rename=True, got {data}"
        assert "estate_id" in data, "Missing estate_id in response"
        assert "current_name" in data, "Missing current_name in response"
        assert "Family Estate" in data.get("current_name", ""), f"Expected 'Family Estate' pattern, got {data.get('current_name')}"
    ...
    def test_rename_check_returns_needs_rename_false_for_admin(self, admin_token):
        ...
        assert data.get("needs_rename") == False, f"Expected needs_rename=False for admin, got {data}"
    ...
    def test_customize_name_saves_new_name(self, benefactor_token, benefactor_estate_id):
        ...
        assert check_data.get("needs_rename") == False, f"Expected needs_rename=False after customization, got {check_data}"
    ...
    def test_customize_name_empty_still_sets_customized(self, benefactor_token, benefactor_estate_id):
        ...
        assert check_response.status_code == 200
        check_data = check_response.json()
        assert check_data.get("needs_rename") == False, f"Expected needs_rename=False after dismiss, got {check_data}"
...
class TestPatchEstateSetsCusomized:
    ...
    def test_patch_estate_sets_name_customized(self, benefactor_session):
        ...
        assert estate_data.get("name_customized") == True, f"Expected name_customized=True, got {estate_data.get('name_customized')}"
...
class TestAdminCannotRenameOtherEstates:
    ...
    def tokens(self):
        ...
        benefactor_response = requests.post(f"{BASE_URL}/api/auth/login", json={
            "email": TEST_BENEFACTOR_EMAIL,
            "password": TEST_BENEFACTOR_PASSWORD
        })
    ...
    def benefactor_estate_id(self, tokens):
        ...
        response = requests.get(
            f"{BASE_URL}/api/estates",
            headers={"Authorization": f"Bearer {tokens['benefactor']}"}
        )
        if response.status_code == 200:
            estates = response.json()
            if estates:
...
class TestCleanup:
    ...
    def test_cleanup_restore_original_estate_name(self):
        ...
        result = db.estates.update_one(
            {"id": estate_id},
            {
                "$set": {"name": "TestUser Family Estate"},
                "$unset": {"name_customized": ""}
            }
        )
        print(f"✓ Cleanup: Restored estate name to 'TestUser Family Estate' and unset name_customized")
