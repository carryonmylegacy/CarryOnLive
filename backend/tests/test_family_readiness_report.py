...
class TestFamilyReadinessReport:
    ...
    def test_family_readiness_report_requires_auth(self):
        ...
        res = requests.post(
            f"{BASE_URL}/api/downloads/prepare",
            json={"action": "family_readiness_report", "params": {"estate_id": TEST_ESTATE_ID}, "filename": "test.pdf"},
    ...
    def test_ccp_plan_action_in_valid_actions(self):
        ...
        res = self.session.post(
            f"{BASE_URL}/api/downloads/prepare",
            json={"action": "ccp_plan", "params": {"plan_id": "nonexistent-plan-id"}, "filename": "test.pdf"},
        )
    ...
    def test_download_token_single_use(self):
        ...
        assert download2.status_code == 401, (
            f"Second download should fail (token consumed), got {download2.status_code}"
        )
