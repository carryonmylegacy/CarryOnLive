...
class TestFeatureGatesAPI:
    ...
    def test_get_feature_gates_returns_correct_structure(self):
        ...
        for feature_key in EXPECTED_FEATURES:
            ...
            for tier_id in EXPECTED_TIERS:
                ...
                assert isinstance(gates[feature_key][tier_id], bool), "Gate value should be boolean"
        ...
        print("✓ GET /api/admin/feature-gates: 9 features, 8 tiers, gates structure valid")
    ...
    def test_put_feature_gates_publishes_changes(self):
        ...
        # Publish the change
        put_response = self.session.put(f"{BASE_URL}/api/admin/feature-gates", json={"gates": gates})

        assert put_response.status_code == 200, f"Expected 200, got {put_response.status_code}: {put_response.text}"

        put_data = put_response.json()
        assert put_data.get("success"), "Expected success: true"
        ...
        assert not verify_data["gates"]["ega"]["base"], "EGA for base tier should be OFF"

        # Restore original state
        gates["ega"]["base"] = original_ega_base
        restore_response = self.session.put(f"{BASE_URL}/api/admin/feature-gates", json={"gates": gates})
        assert restore_response.status_code == 200

        print("✓ PUT /api/admin/feature-gates: Toggle EGA for base tier works, changes persist")
    ...
    def test_put_feature_gates_validates_structure(self):
        ...
        print("✓ PUT /api/admin/feature-gates: Validates structure - missing feature returns 400")
    ...
    def test_put_feature_gates_validates_tiers(self):
        ...
        print("✓ PUT /api/admin/feature-gates: Validates structure - missing tier returns 400")
    ...
    def test_get_enabled_features_for_admin(self):
        ...
        assert data["all_enabled"], "Admin should have all_enabled=True"
    ...
    def test_put_feature_gates_requires_admin_auth(self):
        ...
        response = no_auth_session.put(f"{BASE_URL}/api/admin/feature-gates", json={"gates": {}})
    ...
    def test_global_toggle_all_tiers_on(self):
        ...
        for tier in EXPECTED_TIERS:
            assert verify_data["gates"]["ega"][tier], f"EGA for {tier} should be ON"

        # Restore original state
        self.session.put(f"{BASE_URL}/api/admin/feature-gates", json={"gates": original_gates})

        print("✓ Global toggle: All tiers ON for EGA works correctly")
    ...
    def test_global_toggle_all_tiers_off(self):
        ...
        for tier in EXPECTED_TIERS:
            assert not verify_data["gates"]["ffn"][tier], f"FFN for {tier} should be OFF"

        # Restore original state
        self.session.put(f"{BASE_URL}/api/admin/feature-gates", json={"gates": original_gates})

        print("✓ Global toggle: All tiers OFF for FFN works correctly")
