...
class TestFounderAccessRequests:
    ...
    def test_01_submit_request_success(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder/requests",
            json={"name": "Test User", "email": TEST_EMAIL, "message": "I want to learn about the founder"},
        )
    ...
    def test_02_submit_duplicate_request_returns_already_pending(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder/requests",
            json={"name": "Test User Again", "email": TEST_EMAIL, "message": "Trying again"},
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
    ...
    def test_03_submit_request_missing_name(self):
        ...
        response = requests.post(f"{BASE_URL}/api/founder/requests", json={"name": "", "email": "test@example.com"})
    ...
    def test_04_submit_request_missing_email(self):
        ...
        response = requests.post(f"{BASE_URL}/api/founder/requests", json={"name": "Test User", "email": ""})
        assert response.status_code == 400, f"Expected 400, got {response.status_code}"
    ...
    def test_07_approve_request_requires_auth(self):
        ...
        response = requests.post(f"{BASE_URL}/api/founder/requests/fake-id/approve", json={"password": "test123"})
        assert response.status_code in [401, 403], f"Expected 401/403, got {response.status_code}"
    ...
    def test_11_founder_login_success(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder-about/login", json={"email": TEST_EMAIL, "password": TEST_PASSWORD}
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data.get("valid"), f"Expected valid=true, got {data}"
    ...
    def test_12_founder_login_wrong_password(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder-about/login", json={"email": TEST_EMAIL, "password": "WrongPassword"}
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        data = response.json()
        assert not data.get("valid"), f"Expected valid=false, got {data}"
    ...
    def test_13_founder_login_no_access(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder-about/login", json={"email": "nonexistent@test.com", "password": "anypassword"}
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        data = response.json()
        assert not data.get("valid"), f"Expected valid=false, got {data}"
    ...
    def test_14_founder_login_reusable(self):
        ...
        for i in range(3):
            ...
            data = response.json()
            assert data.get("valid"), f"Login {i + 1} failed: {data}"
    ...
    def test_16_revoke_access_success(self, auth_headers):
        ...
        response = requests.post(f"{BASE_URL}/api/founder/requests/{TEST_REQUEST_ID}/revoke", headers=auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
    ...
    def test_17_founder_login_after_revoke_fails(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}"
        data = response.json()
        assert not data.get("valid"), f"Expected valid=false after revoke, got {data}"
        assert data.get("reason") == "no_access", f"Expected reason='no_access', got {data}"
    ...
    def test_19_deny_request_success(self, auth_headers):
        ...
        submit_response = requests.post(
            f"{BASE_URL}/api/founder/requests",
            json={"name": "Deny Test User", "email": deny_email, "message": "This will be denied"},
        )
        assert submit_response.status_code == 200
        request_id = submit_response.json().get("request_id")

        # Deny it
        response = requests.post(f"{BASE_URL}/api/founder/requests/{request_id}/deny", headers=auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
    ...
    def test_20_deny_nonexistent_request(self, auth_headers):
        ...
        response = requests.post(f"{BASE_URL}/api/founder/requests/nonexistent-id/deny", headers=auth_headers)
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
        print("✓ Nonexistent request correctly returns 404")
...
class TestInviteLinksStillWork:
    ...
    def test_create_invite_link(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder/invites", json={"note": "TEST_access_request_coexist"}, headers=auth_headers
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
    ...
    def test_verify_invite_token(self, auth_headers):
        ...
        create_response = requests.post(
            f"{BASE_URL}/api/founder/invites", json={"note": "TEST_verify_coexist"}, headers=auth_headers
        )
