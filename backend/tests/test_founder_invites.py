...
class TestFounderInvitesBackend:
    ...
    def test_create_invite_success(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/founder/invites",
            json={"note": "TEST_invite_for_testing"},
        ...
        assert data.get("used") == False, "New invite should not be used"
        assert data.get("revoked") == False, "New invite should not be revoked"
    ...
    def test_verify_valid_invite(self, auth_headers):
        ...
        assert data.get("valid") == True, "Valid invite should return valid=True"
        print(f"PASS: Verified valid invite token")
    ...
    def test_use_invite_single_use(self, auth_headers):
        ...
        create_res = requests.post(
            f"{BASE_URL}/api/founder/invites",
            json={"note": "TEST_single_use_test"},
            headers=auth_headers
        )
        assert create_res.status_code == 200
        token = create_res.json()["token"]
    ...
    def test_revoke_invite_success(self, auth_headers):
        ...
        revoke_res = requests.delete(
            f"{BASE_URL}/api/founder/invites/{token}",
            headers=auth_headers
        )
    ...
    def test_revoke_requires_auth(self, auth_headers):
        ...
        create_res = requests.post(
            f"{BASE_URL}/api/founder/invites",
            json={"note": "TEST_revoke_auth_test"},
            headers=auth_headers
        )
        token = create_res.json()["token"]
    ...
    def test_revoke_nonexistent_token(self, auth_headers):
        ...
        response = requests.delete(
            f"{BASE_URL}/api/founder/invites/{fake_token}",
            headers=auth_headers
        )
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
