...
BASE_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://settings-refactor-v2.preview.emergentagent.com').rstrip('/')
...
class TestAuthenticationAndSecurity:
    ...
    def test_login_with_valid_credentials(self):
        ...
        response = self.session.post(f"{BASE_URL}/api/auth/dev-login", json={
            "email": ADMIN_EMAIL,
    ...
    def test_dev_login_restricted_to_admin(self):
        ...
        admin_response = self.session.post(f"{BASE_URL}/api/auth/dev-login", json={
            "email": ADMIN_EMAIL,
            "password": ADMIN_PASSWORD
        })
