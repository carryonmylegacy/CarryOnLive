...
class TestFunnelStep:
    ...
    def test_funnel_step_records_interests(self, api_client):
        ...
        # Record step 1
        response = api_client.post(f"{BASE_URL}/api/funnel/step", json={
...
class TestFunnelComplete:
    ...
    def test_funnel_complete_marks_session(self, api_client):
        ...
        # Complete the funnel
        response = api_client.post(f"{BASE_URL}/api/funnel/complete", json={
            "session_id": session_id,
            "referral_email": None
        })
    ...
    def test_funnel_complete_with_referral(self, api_client):
        ...
        response = api_client.post(f"{BASE_URL}/api/funnel/complete", json={
            "session_id": session_id,
            "referral_email": "test_referral@example.com"
        })
...
class TestFunnelConvert:
    ...
    def test_funnel_convert_links_user(self, api_client):
        ...
        api_client.post(f"{BASE_URL}/api/funnel/complete", json={
            "session_id": session_id
        })
        
        # Convert with a test user_id
        response = api_client.post(f"{BASE_URL}/api/funnel/convert", json={
            "session_id": session_id,
...
class TestFunnelAnalytics:
    ...
    def test_analytics_returns_data(self, api_client, admin_headers):
        ...
        expected_fields = [
            "total_sessions", "completed", "converted",
            "completion_rate", "conversion_rate", "drop_offs",
            "by_source", "by_campaign", "by_device", "by_state",
            "by_interest", "referrals_sent", "recent_sessions"
        ]
        ...
        print(f"✓ Analytics returned: {data['total_sessions']} sessions, {data['completed']} completed, {data['converted']} converted")
...
class TestFullFunnelFlow:
    ...
    def test_complete_funnel_flow(self, api_client):
        ...
        step1_resp = api_client.post(f"{BASE_URL}/api/funnel/step", json={
            "session_id": session_id,
            "step": 1,
            "name": "interests",
        ...
        step2_resp = api_client.post(f"{BASE_URL}/api/funnel/step", json={
            "session_id": session_id,
            "step": 2,
            "name": "family",
            "selections": {
        ...
        step3_resp = api_client.post(f"{BASE_URL}/api/funnel/step", json={
            "session_id": session_id,
            "step": 3,
            "name": "plan",
            "selections": {
        ...
        # Step 4: CTA
        step4_resp = api_client.post(f"{BASE_URL}/api/funnel/step", json={
            "session_id": session_id,
            "step": 4,
            "name": "cta",
            "selections": {"action": "start_trial"}
        })
        assert step4_resp.status_code == 200
        print("✓ Step 4 (CTA) completed")
        
        # Step 5: Referral + Complete
        complete_resp = api_client.post(f"{BASE_URL}/api/funnel/complete", json={
            "session_id": session_id,
            "referral_email": "e2e_test_referral@example.com"
        })
        assert complete_resp.status_code == 200
        print("✓ Step 5 (referral) + funnel completed")
        
        # Convert (simulate signup)
        convert_resp = api_client.post(f"{BASE_URL}/api/funnel/convert", json={
            "session_id": session_id,
