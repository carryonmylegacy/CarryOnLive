...
class TestGhostEstateAutoCleanup:
    ...
    def test_03_create_ghost_estate_scenario(self):
        ...
        if verify_resp.status_code != 200:
            ...
            print("Dev-login failed, trying to bypass OTP manually")
            # Get user ID and verify directly
            pytest.skip("Cannot bypass OTP for test user")
        ...
        create_resp1 = user_session.post(
            f"{BASE_URL}/api/accounts/create-estate",
            json={
        ...
        create_resp2 = user_session.post(
            f"{BASE_URL}/api/accounts/create-estate",
            json={"beneficiary_enrollments": []},
        )
    ...
    def test_04_verify_is_also_benefactor_flag_after_create(self):
        ...
        login_resp = self.session.post(
            f"{BASE_URL}/api/auth/dev-login",
            json={"email": test_email2, "password": self.test_password},
        )
        ...
        assert user_data.get("is_also_benefactor"), (
    ...
    def test_05_blocking_user_with_real_estate(self):
        ...
        login_resp = self.session.post(
            f"{BASE_URL}/api/auth/dev-login",
            json={"email": test_email3, "password": self.test_password},
        )
        ...
        user_token = login_resp.json().get("access_token")
        login_resp.json().get("user", {}).get("id")
...
class TestEstateCreationResponse:
    ...
    def test_create_estate_response_structure(self):
        ...
        assert response["success"], f"Expected success=True, got {response['success']}"
        assert len(response["estate_id"]) > 0, "estate_id should not be empty"

        print("PASS: Estate creation response has correct structure")
...
class TestAuthMeAfterEstateCreation:
    ...
    def test_auth_me_returns_is_also_benefactor(self):
        ...
        print(
            f"INFO: Initial state - role: {user_before.get('role')}, is_also_benefactor: {user_before.get('is_also_benefactor')}"
        )
        ...
        assert user_after.get("is_also_benefactor"), (
