...
def auth_token():
    ...
    verify_resp = requests.post(
        f"{BASE_URL}/api/auth/verify-otp", json={"email": TEST_EMAIL, "otp": otp}
    )
...
class TestEstateGuardianChat:
    ...
    def test_basic_chat_message(self, auth_headers, estate_id):
        ...
        response = requests.post(
            ...
            json={"message": "What documents am I missing?", "estate_id": estate_id},
    ...
    def test_chat_response_includes_action_result_field(self, auth_headers, estate_id):
        ...
        response = requests.post(
            f"{BASE_URL}/api/chat/guardian",
            headers=auth_headers,
            json={"message": "Hello", "estate_id": estate_id},
            timeout=120,
        )
    ...
    def test_analyze_vault_action(self, auth_headers, estate_id):
        ...
        response = requests.post(
            f"{BASE_URL}/api/chat/guardian",
            headers=auth_headers,
            json={"message": "", "estate_id": estate_id, "action": "analyze_vault"},
            timeout=120,
        )
        ...
        print(
            f"PASS: Vault analysis returned {len(data['response'])} chars with document references"
        )
    ...
    def test_generate_checklist_action(self, auth_headers, estate_id):
        ...
        if data.get("action_result"):
            ...
            print(
                "PASS: Checklist action completed (items_added may be 0 if all exist)"
            )
    ...
    def test_analyze_readiness_action(self, auth_headers, estate_id):
        ...
        response = requests.post(
            f"{BASE_URL}/api/chat/guardian",
            headers=auth_headers,
            json={"message": "", "estate_id": estate_id, "action": "analyze_readiness"},
            timeout=120,
        )
        ...
        for key in ["documents", "messages", "checklist"]:
            ...
            assert isinstance(readiness[key]["score"], int), (
                f"{key} score should be int"
            )

        print(
            f"PASS: Readiness analysis returned with overall score {readiness['overall_score']}%"
        )
    ...
    def test_chat_with_estate_id_fetches_context(self, auth_headers, estate_id):
        ...
        assert has_context, (
            "Response should include estate context (Mitchell family, Penny, etc.)"
        )
...
class TestEstateStateField:
    ...
    def test_estate_has_state_field(self, auth_headers, estate_id):
        ...
        response = requests.get(
            f"{BASE_URL}/api/estates/{estate_id}", headers=auth_headers
        )
        ...
        print(
            f"PASS: Estate retrieved, state field value: {data.get('state', 'not set')}"
        )
    ...
    def test_patch_estate_with_state(self, auth_headers, estate_id):
        ...
        assert data.get("state") == "California", (
            f"State not updated. Got: {data.get('state')}"
        )
...
class TestChatRequestModel:
    ...
    def test_chat_without_estate_id_uses_first_estate(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/chat/guardian",
            headers=auth_headers,
            json={"message": "What's my estate readiness?"},
            timeout=120,
        )
