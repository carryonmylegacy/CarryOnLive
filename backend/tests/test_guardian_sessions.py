...
class TestGuardianSessions:
    ...
    def test_create_new_chat_session(self, headers):
        ...
        print(
            f"✓ POST /api/chat/guardian - Created session: {data['session_id'][:30]}..."
        )
    ...
    def test_get_chat_history_valid_session(self, headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/chat/history/{session_id}", headers=headers
        )
        ...
        if len(data) > 0:
            ...
            print(
                f"✓ GET /api/chat/history/{session_id[:20]}... - Found {len(data)} messages"
            )
        else:
    ...
    def test_get_chat_history_nonexistent_session(self, headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/chat/history/{fake_session}", headers=headers
        )
    ...
    def test_delete_session(self, headers):
        ...
        delete_response = requests.delete(
            f"{BASE_URL}/api/chat/sessions/{session_id}", headers=headers
        )
        ...
        print(
            f"✓ DELETE /api/chat/sessions/{session_id[:20]}... - Deleted {data['deleted']} messages"
        )
        ...
        assert len(verify_response.json()) == 0, (
            "Session history should be empty after delete"
        )
    ...
    def test_delete_nonexistent_session(self, headers):
        ...
        response = requests.delete(
            f"{BASE_URL}/api/chat/sessions/{fake_session}", headers=headers
        )
