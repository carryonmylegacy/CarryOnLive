...
class TestFamilyTreeFeatures:
    ...
    def test_beneficiaries_endpoint_returns_dob_for_sorting(self, benefactor_auth):
        ...
        for ben in beneficiaries:
            ...
            print(f"  - {ben.get('first_name', ben.get('name', 'Unknown'))}: DOB={ben.get('date_of_birth', 'N/A')}, is_primary={ben.get('is_primary', False)}")
...
class TestAdminGraphView:
    ...
    def test_admin_users_linked_beneficiaries_have_dob(self, admin_auth):
        ...
        benefactors_with_bens = [u for u in users if u.get('role') == 'benefactor' and len(u.get('linked_beneficiaries', [])) > 0]
    ...
    def test_estates_correctly_annotates_beneficiary_estates(self, benefactor_auth):
        ...
        for estate in estates:
            ...
            if role == 'beneficiary':
                ...
                assert is_ben_estate == True, f"Estate {estate.get('name')} has user_role_in_estate=beneficiary but is_beneficiary_estate is not True"
                print(f"✓ Beneficiary estate: {estate.get('name')} (is_beneficiary_estate=True)")
            elif role == 'owner':
                # Should not have is_beneficiary_estate=True
                assert is_ben_estate == False, f"Owned estate {estate.get('name')} should not have is_beneficiary_estate=True"
...
class TestPreviousBugFixes:
    ...
    def test_admin_delete_requires_password(self, admin_auth):
        ...
        assert response.status_code in [401, 422, 404], f"Expected 401/422/404 without password, got {response.status_code}"
