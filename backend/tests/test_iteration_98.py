...
class TestAuthAndSetup:
    ...
    def test_multi_role_user_login(self):
        ...
        assert "access_token" in data
        user = data.get("user", {})
        print(f"✅ Multi-role user login successful - role: {user.get('role')}, is_also_benefactor: {user.get('is_also_benefactor')}")
        return data["access_token"]
...
class TestSubscriptionPlansAPI:
    ...
    def test_subscription_plans_no_wizard_echo(self):
        ...
        forbidden_strings = ["will/trust wizard", "eternal echo", "will wizard", "trust wizard"]
        ...
        for plan in plans:
            ...
            for feature in features:
                ...
                for forbidden in forbidden_strings:
                    assert forbidden not in feature_text.lower(), f"Feature '{feature_text}' in plan '{plan_name}' contains forbidden text: {forbidden}"
    ...
    def test_estates_endpoint_has_benefactor_name(self, multi_role_token):
        ...
        for estate in estates:
            ...
            if role == "beneficiary" or estate.get("is_beneficiary_estate"):
                ...
                print(f"   - Beneficiary estate: {estate.get('name')}, benefactor_name: {estate.get('benefactor_name', 'N/A')}")
