...
class TestDebugUserStateEndpoint:
    ...
    def test_debug_user_state_has_multi_role_fields(self):
        ...
        expected_fields = [
            "db_is_also_benefactor",
            "db_is_also_beneficiary",
            "owns_estates",
            "computed_is_also_benefactor",
        ]
        for field in expected_fields:
            assert field in data, f"Missing field: {field}"

        print(
            f"Multi-role state: db_is_also_benefactor={data.get('db_is_also_benefactor')}, "
...
class TestOnboardingProgress:
    ...
    def test_onboarding_progress_endpoint(self, auth_token):
        ...
        assert "progress_pct" in data, "Should have progress_pct"

        print(
            f"Onboarding progress: {data.get('progress_pct')}% complete, {data.get('completed_count')}/{data.get('total_steps')} steps"
        )
