...
class TestAuthAndOperatorRole:
    ...
    def founder_token(self):
        ...
        resp = requests.post(f"{BASE_URL}/api/auth/dev-login", json={
            "email": FOUNDER_EMAIL,
            "password": FOUNDER_PASSWORD
        })
...
class TestP1ContactSettings:
    ...
    def test_p1_contact_settings_update_founder_only(self, auth_headers):
        ...
        resp = requests.put(f"{BASE_URL}/api/founder/p1-contact-settings", headers=auth_headers, json=update_data)
        assert resp.status_code == 200
        data = resp.json()
...
class TestOperatorEditDelete:
    ...
    def test_edit_operator(self, auth_headers):
        ...
        resp = requests.put(f"{BASE_URL}/api/founder/operators/{operator_id}", headers=auth_headers, json=edit_data)
        assert resp.status_code == 200
