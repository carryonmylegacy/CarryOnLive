...
class TestOnboardingWizard:
    ...
    def test_onboarding_progress_returns_steps(self, auth_headers):
        ...
        expected_keys = ["create_estate", "add_beneficiary", "upload_document", "create_message", "review_readiness"]
        ...
        print(f"✓ Onboarding progress returned {data['completed_count']}/{data['total_steps']} steps complete ({data['progress_pct']}%)")
...
class TestEmergencyAccess:
    ...
    def test_request_requires_auth(self):
        ...
        response = requests.post(
            ...
            json={"estate_id": "fake", "reason": "test", "relationship_to_benefactor": "test"}
        )
        assert response.status_code in [401, 403], f"Expected 401/403, got {response.status_code}"
    ...
    def test_request_forbidden_for_non_beneficiary(self, auth_headers, estate_id):
        ...
        assert response.status_code == 403, f"Expected 403 for non-beneficiary, got {response.status_code}: {response.text}"
