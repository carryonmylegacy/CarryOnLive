...
class TestOnboardingWizard:
    ...
    def test_onboarding_progress_returns_steps(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/onboarding/progress", headers=auth_headers
        )
    ...
    def test_onboarding_founder_has_some_progress(self, auth_headers):
        ...
        estate_step = next(
            (s for s in data["steps"] if s["key"] == "create_estate"), None
        )
        ...
        assert data["completed_count"] >= 1, (
            f"Expected at least 1 completed step, got {data['completed_count']}"
        )
    ...
    def test_complete_step_review_readiness(self, auth_headers):
        ...
        assert data.get("step") == "review_readiness", (
            "Response should echo the step key"
        )
    ...
    def test_dismiss_onboarding(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/onboarding/dismiss", headers=auth_headers, json={}
        )
        assert response.status_code == 200, (
            f"Expected 200, got {response.status_code}: {response.text}"
        )
        ...
        assert progress_response.json().get("dismissed"), (
            "Dismissed should be True after dismiss"
        )
...
class TestQuickStartTemplates:
    ...
    def test_get_scenario_templates(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/templates/scenarios", headers=auth_headers
        )
        assert response.status_code == 200, (
            f"Expected 200, got {response.status_code}: {response.text}"
        )
    ...
    def test_template_item_counts(self, auth_headers):
        ...
        for tid, expected_count in expected_counts.items():
            ...
            assert actual_count == expected_count, (
                f"{tid} should have {expected_count} items, got {actual_count}"
            )

        print(
            "✓ Template item counts: hospice=14, military=10, new_parent=9, recently_married=9"
        )
    ...
    def test_apply_hospice_template(self, auth_headers, estate_id):
        ...
        assert data.get("template") == "Hospice Care", (
            "Response should identify template name"
        )
    ...
    def test_apply_military_template(self, auth_headers, estate_id):
        ...
        print(
            f"✓ Applied Military Deployment template: {data.get('items_added', 0)} items added"
        )
...
class TestEmergencyAccess:
    ...
    def test_my_requests_returns_list(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/emergency-access/my-requests", headers=auth_headers
        )
        assert response.status_code == 200, (
            f"Expected 200, got {response.status_code}: {response.text}"
        )
    ...
    def test_active_returns_list(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/emergency-access/active", headers=auth_headers
        )
        assert response.status_code == 200, (
            f"Expected 200, got {response.status_code}: {response.text}"
        )
    ...
    def test_admin_endpoint_requires_admin_role(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/admin/emergency-access", headers=auth_headers
        )
