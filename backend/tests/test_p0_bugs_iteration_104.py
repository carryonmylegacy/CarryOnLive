...
class TestBug1AccountLockout:
    ...
    def test_users(self, api_client):
        ...
        reg_resp2 = api_client.post(f"{BASE_URL}/api/auth/register", json={
    ...
    def test_create_beneficiary_with_existing_email_has_pending_status(
        ...
        print(f"PASS: Beneficiary created with status='pending', user_id pre-linked")
    ...
    def test_existing_user_not_added_to_estate_beneficiaries_array(
        ...
        assert test_users["existing_user"]["user_id"] not in beneficiaries_array, \
            f"Bug 1 FAIL: Existing user was incorrectly added to estate's beneficiaries array"
        
        print(f"PASS: Existing user NOT in estate's beneficiaries array")
    ...
    def test_existing_user_can_still_login_with_original_password(
        ...
        print(f"PASS: Existing user can login with original password")
    ...
    def test_benefactor_can_send_invitation_for_pending_beneficiary(
        ...
        response = api_client.get(
            f"{BASE_URL}/api/beneficiaries/{test_users['estate_id']}",
            headers=headers
        )
        assert response.status_code == 200
        ...
        invite_resp = api_client.post(
            f"{BASE_URL}/api/beneficiaries/{target['id']}/invite",
            headers=headers
        )
        ...
        print(f"PASS: Benefactor can send invitation for pending beneficiary with pre-linked user_id")
...
class TestBug1EmailCaseSensitivity:
    ...
    def test_email_case_insensitive_lookup(self, api_client, mixed_case_user):
        ...
        print(f"PASS: Email case-insensitive lookup works correctly")
...
class TestBug1InvitationAccept:
    ...
    def invitation_test_data(self, api_client, mongo_client):
        ...
        ben_resp = api_client.post(f"{BASE_URL}/api/beneficiaries", headers=headers, json={
            "estate_id": estate_id,
    ...
    def test_accept_invitation_for_prelinked_beneficiary(
        ...
        response = api_client.post(f"{BASE_URL}/api/invitations/accept", json={
        ...
        if response.status_code == 200:
            ...
            print(f"PASS: Invitation accept works for pre-linked beneficiary")
        elif response.status_code == 400:
            # Already accepted - that's OK
            print(f"PASS: Invitation already accepted (expected)")
        else:
...
class TestBug2PhotoInOrbit:
    ...
    def photo_test_data(self, api_client, mongo_client):
        ...
        reg_a = api_client.post(f"{BASE_URL}/api/auth/register", json={
            "email": a_email,
            "password": a_password,
            "first_name": "PhotoA",
            "last_name": "Test",
        ...
        login_a = api_client.post(f"{BASE_URL}/api/auth/login", json={
            "email": a_email,
            "password": a_password
        })
        ...
        reg_b = api_client.post(f"{BASE_URL}/api/auth/register", json={
            "email": b_email,
            "password": b_password,
            "first_name": "PhotoB",
            "last_name": "Test",
        ...
        login_b = api_client.post(f"{BASE_URL}/api/auth/login", json={
            "email": b_email,
            "password": b_password
        })
        ...
        ben_resp = api_client.post(f"{BASE_URL}/api/beneficiaries", headers=a_headers, json={
    ...
    def test_upload_photo_to_beneficiary_record(self, api_client, photo_test_data):
        ...
        headers = {"Authorization": f"Bearer {photo_test_data['a']['token']}"}
        ...
        data = response.json()
        assert data.get("success") == True or "photo_url" in data
        
        print(f"PASS: Photo uploaded to beneficiary record")
    ...
    def test_family_connections_photo_fallback(self, api_client, mongo_client, photo_test_data):
        ...
        if bens_resp.status_code == 200:
            bens = bens_resp.json()
            a_ben = next((b for b in bens if b.get("email") == photo_test_data["a"]["email"]), None)
            if a_ben and a_ben.get("invitation_token"):
                # Accept invitation as A
                accept_resp = api_client.post(f"{BASE_URL}/api/invitations/accept", json={
        ...
        if response.status_code == 200:
            ...
            if b_connection:
                ...
                if photo_url:
                    print(f"PASS: family-connections has photo_url fallback from beneficiary record")
                else:
                    # This might mean A is not yet accepted - check status
                    print(f"! B found but no photo_url - checking if fallback worked")
            else:
                print(f"! B not found in A's family-connections (A may need to accept invitation)")
        else:
    ...
    def test_get_estates_owner_photo_fallback(self, api_client, mongo_client, photo_test_data):
        ...
        b_estate = next(
            (e for e in estates if e.get("is_beneficiary_estate") and e.get("owner_id") == photo_test_data["b"]["user_id"]),
            None
        )
        ...
        if b_estate:
            ...
            print(f"! B's estate not found in A's list (A may need to accept invitation)")
    ...
    def test_get_beneficiaries_photo_fallback_from_linked_user(
        ...
        mongo_client.beneficiaries.update_one(
            {"id": photo_test_data["b_beneficiary_id"]},
            {"$unset": {"photo_url": ""}}
        )
        ...
        if b_ben:
            ...
            if photo_url:
                print(f"PASS: Beneficiary has photo_url fallback from linked user")
            else:
                print(f"! B's beneficiary record has no photo_url")
        else:
            print(f"! B not found in beneficiaries list")
