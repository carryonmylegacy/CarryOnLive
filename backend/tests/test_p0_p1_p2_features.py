...
class TestVapidPublicKey:
    ...
    def test_vapid_public_key_returns_valid_key(self):
        ...
        assert len(data["public_key"]) > 50, "public_key should be a valid base64 VAPID key (50+ chars)"
...
class TestAdminAuth:
    ...
    def get_admin_token():
        ...
        login_response = requests.post(f"{BASE_URL}/api/auth/dev-login", json={
            "email": ADMIN_EMAIL,
...
class TestAdminActivityLog:
    ...
    def test_activity_log_returns_array(self):
        ...
        if len(data) > 0:
            ...
            print(f"PASS: First activity type: {activity['type']}, desc: {activity['description'][:50]}...")
...
class TestAdminStats:
    ...
    def test_admin_stats(self):
        ...
        assert "pending_certificates" in data, "Stats should have 'pending_certificates'"
        ...
        print(f"PASS: Stats - Total users: {users['total']}, Estates: {data['estates']['total']}, Documents: {data['documents']}, Pending certs: {data['pending_certificates']}")
