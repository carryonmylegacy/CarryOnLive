...
class TestBug1AccountLockout:
    ...
    def test_create_beneficiary_with_existing_email_has_pending_status(
        ...
        print(f"✓ Bug 1 FIX VERIFIED: status='pending', user_id pre-linked")
    ...
    def test_benefactor_can_invite_pending_beneficiary_with_prelinked_user(
        ...
        response = api_client.get(
            f"{BASE_URL}/api/beneficiaries/{benefactor_a_estate['id']}",
            headers=headers
        )
        assert response.status_code == 200
        ...
        if target.get("invitation_status") == "pending":
            ...
            print(f"✓ Invite endpoint works for pending beneficiary with pre-linked user_id")
...
class TestBug2PhotoInOrbit:
    ...
    def test_upload_photo_to_beneficiary_record(
        ...
        if photo_resp.status_code == 200:
            ...
            print(f"✓ Photo uploaded to beneficiary record")
        else:
    ...
    def test_family_connections_photo_fallback(self, api_client, benefactor_a, user_b):
        ...
        login_resp = api_client.post(f"{BASE_URL}/api/auth/login", json={
            "email": benefactor_a["email"],
            "password": benefactor_a["password"]
        })
...
class TestFullFlowPhotoOrbit:
    ...
    def test_full_photo_orbit_flow(self, api_client):
        ...
        login_x = api_client.post(f"{BASE_URL}/api/auth/login", json={
            "email": x_email, "password": x_password
        })
        assert login_x.status_code == 200
        x_token = login_x.json()["access_token"]
        x_user_id = login_x.json()["user"]["id"]
        print(f"✓ Step 1: Benefactor X registered")
        ...
        reg_y = api_client.post(f"{BASE_URL}/api/auth/register", json={
            "email": y_email,
            "password": y_password,
            "first_name": "UserY",
            "last_name": "Test",
        ...
        login_y = api_client.post(f"{BASE_URL}/api/auth/login", json={
            "email": y_email, "password": y_password
        })
        assert login_y.status_code == 200
        y_token = login_y.json()["access_token"]
        y_user_id = login_y.json()["user"]["id"]
        print(f"✓ Step 2: User Y registered")
        
        # Step 3: X adds Y as beneficiary
        add_ben_resp = api_client.post(f"{BASE_URL}/api/beneficiaries", headers=x_headers, json={
        ...
        print(f"✓ Step 3: X added Y as beneficiary (status=pending, user_id pre-linked)")
        ...
        if photo_resp.status_code == 200:
            print(f"✓ Step 4: X uploaded photo for Y's beneficiary record")
        else:
        ...
        if create_estate_resp.status_code == 200:
            ...
            y_estate_id = next((e["id"] for e in y_estates if e.get("user_role_in_estate") == "owner"), None)
        ...
        add_x_resp = api_client.post(f"{BASE_URL}/api/beneficiaries", headers=y_headers, json={
        ...
        if add_x_resp.status_code == 200:
            ...
            print(f"✓ Step 6: Y added X as beneficiary (status=pending)")
        else:
        ...
        if y_bens.status_code == 200:
            ...
            if x_in_y and x_in_y.get("invitation_token"):
                ...
                accept_resp = api_client.post(f"{BASE_URL}/api/invitations/accept", json={
                    "token": x_in_y["invitation_token"],
                    "password": x_password
                })
                if accept_resp.status_code in [200, 400]:
                    print(f"✓ Step 7: X accepted invitation to Y's estate")
        
        # Step 8: X views family-connections and should see Y's photo from fallback
        # Refresh X's token
        x_login = api_client.post(f"{BASE_URL}/api/auth/login", json={
            "email": x_email, "password": x_password
        })
        ...
        if fc_resp.status_code == 200:
            ...
            if y_connection:
                ...
                if photo_url:
                    print(f"✓ Bug 2 FIX VERIFIED: Y's photo visible via beneficiary record fallback")
                else:
