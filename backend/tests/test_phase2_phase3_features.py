...
class TestSessionPolicy:
    ...
    def test_update_session_policy(self, auth_headers):
        ...
        update_data = {"role_type": "worker", "timeout_minutes": 30, "enabled": True}
        response = requests.put(f"{BASE_URL}/api/admin/session-policy", json=update_data, headers=auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        ...
        # Reset to disabled
        reset_data = {"role_type": "worker", "timeout_minutes": 15, "enabled": False}
    ...
    def test_session_policy_invalid_role_type(self, auth_headers):
        ...
        update_data = {"role_type": "invalid_role", "timeout_minutes": 30, "enabled": True}
        response = requests.put(f"{BASE_URL}/api/admin/session-policy", json=update_data, headers=auth_headers)
    ...
    def test_session_policy_invalid_timeout(self, auth_headers):
        ...
        # Timeout too low
        update_data = {"role_type": "worker", "timeout_minutes": 0, "enabled": True}
        response = requests.put(f"{BASE_URL}/api/admin/session-policy", json=update_data, headers=auth_headers)
        assert response.status_code == 400
...
class TestTeamChat:
    ...
    def test_send_message_and_retrieve(self, auth_headers):
        ...
        send_data = {"channel_id": "general", "content": test_content}
        response = requests.post(f"{BASE_URL}/api/team/messages", json=send_data, headers=auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    ...
    def test_get_messages_chronological_order(self, auth_headers):
        ...
        response = requests.get(f"{BASE_URL}/api/team/messages/general", headers=auth_headers)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
    ...
    def test_send_empty_message_rejected(self, auth_headers):
        ...
        send_data = {"channel_id": "general", "content": "   "}
        response = requests.post(f"{BASE_URL}/api/team/messages", json=send_data, headers=auth_headers)
        assert response.status_code == 400
...
class TestShiftScheduling:
    ...
    def test_create_shift_validation(self, auth_headers):
        ...
        # Invalid shift type
        shift_data = {"operator_id": "test-id", "shift_type": "invalid_type", "date": "2026-01-15"}
        response = requests.post(f"{BASE_URL}/api/ops/shifts", json=shift_data, headers=auth_headers)
    ...
    def test_update_nonexistent_shift(self, auth_headers):
        ...
        response = requests.put(
            f"{BASE_URL}/api/ops/shifts/nonexistent-shift-id", json={"status": "confirmed"}, headers=auth_headers
        )
        assert response.status_code == 404

    def test_delete_nonexistent_shift(self, auth_headers):
        """DELETE /api/ops/shifts/{id} returns 404 for non-existent shift"""
        response = requests.delete(f"{BASE_URL}/api/ops/shifts/nonexistent-shift-id", headers=auth_headers)
        assert response.status_code == 404
...
class TestTrainingTracker:
    ...
    def test_mark_training_complete(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/ops/training/complete", json={"article_id": module_id}, headers=auth_headers
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
...
class TestIntegration:
    ...
    def test_full_shift_lifecycle(self, auth_headers):
        ...
        create_response = requests.post(f"{BASE_URL}/api/ops/shifts", json=shift_data, headers=auth_headers)
        ...
        update_response = requests.put(
            f"{BASE_URL}/api/ops/shifts/{shift_id}", json={"status": "confirmed"}, headers=auth_headers
        )
        assert update_response.status_code == 200, f"Update failed: {update_response.text}"

        # Delete (cancel) shift
        delete_response = requests.delete(f"{BASE_URL}/api/ops/shifts/{shift_id}", headers=auth_headers)
