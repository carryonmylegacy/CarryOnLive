...
BASE_URL = os.environ.get("REACT_APP_BACKEND_URL", "https://estate-transition.preview.emergentagent.com").rstrip("/")
...
class TestSetPrimaryBeneficiary:
    ...
    def test_set_primary_not_found(self, benefactor_token):
        ...
        headers = {"Authorization": f"Bearer {benefactor_token}"}
        response = requests.put(
            f"{BASE_URL}/api/beneficiaries/non-existent-id/set-primary",
            headers=headers
        )
...
class TestBeneficiaryModel:
    ...
    def test_beneficiaries_list_includes_is_primary(self, benefactor_token, test_estate_id):
        ...
        headers = {"Authorization": f"Bearer {benefactor_token}"}
        response = requests.get(
            f"{BASE_URL}/api/beneficiaries/{test_estate_id}",
            headers=headers
        )
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
