...
class TestPropertyAssets:
    ...
    def setup(self):
        ...
        # Login
        login_resp = requests.post(f"{BASE_URL}/api/auth/login", json={"email": TEST_EMAIL, "password": TEST_PASSWORD})
    ...
    def test_update_property_asset(self):
        ...
        resp = requests.put(
            f"{BASE_URL}/api/financial/property/{created['id']}", json=update_payload, headers=self.headers
        )
    ...
    def test_update_property_designation(self):
        ...
        bens_resp.json() if bens_resp.status_code == 200 else []

        # Update designation
        designation_payload = {"designated_beneficiaries": ["all"], "visibility_timing": {}}
        resp = requests.put(
            f"{BASE_URL}/api/financial/property/{created['id']}/designation",
            json=designation_payload,
            headers=self.headers,
        )
        assert resp.status_code == 200, f"Update designation failed: {resp.text}"
        print("✓ Property designation updated successfully")
    ...
    def test_financial_summary_includes_property(self):
        ...
        assert data["total_assets"] == expected_total, (
            f"total_assets mismatch: {data['total_assets']} != {expected_total}"
        )

        print(
            f"✓ Summary includes property: count={data['property_count']}, value=${data['property_value']}, total_assets=${data['total_assets']}"
        )
    ...
    def test_financial_coverage_score(self):
        ...
        print(
            f"  Breakdown: coverage={breakdown['coverage']}, detail={breakdown['detail']}, designations={breakdown['designations']}"
        )
...
class TestPropertyEdgeCases:
    ...
    def test_create_property_without_value(self):
        ...
        payload = {"estate_id": self.estate_id, "name": "TEST_No Value Property", "category": "other"}
        resp = requests.post(f"{BASE_URL}/api/financial/property", json=payload, headers=self.headers)
        assert resp.status_code == 200
        ...
        self.created_property_ids.append(data["id"])
        print("✓ Property created without value")
    ...
    def test_update_nonexistent_property(self):
        ...
        resp = requests.put(
            f"{BASE_URL}/api/financial/property/nonexistent-id-12345", json={"name": "Test"}, headers=self.headers
        )
    ...
    def test_delete_nonexistent_property(self):
        ...
        assert resp.status_code == 404
        print("✓ Delete non-existent property returns 404")
