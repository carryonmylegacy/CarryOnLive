...
class TestReadinessScoreAlgorithm:
    ...
    def test_get_readiness_returns_detailed_breakdown(self, auth_headers, estate_id):
        ...
        print(f"Readiness breakdown: overall={data['overall_score']}, docs={data['documents']['score']}%, msgs={data['messages']['score']}%, checklist={data['checklist']['score']}%")
    
    # Test 2: POST readiness endpoint recalculates score
    def test_post_readiness_recalculates_score(self, auth_headers, estate_id):
        """POST /api/estate/{estate_id}/readiness recalculates and returns readiness"""
        resp = requests.post(f"{BASE_URL}/api/estate/{estate_id}/readiness", headers=auth_headers)
    ...
    def test_document_score_based_on_5_required_docs(self, auth_headers, estate_id):
        ...
        assert data["documents"]["required"] == 5, f"Expected 5 required docs, got {data['documents']['required']}"
        ...
        if data["documents"]["missing"]:
            expected_docs = ["Last Will and Testament", "Revocable Living Trust", 
            ...
            for missing in data["documents"]["missing"]:
                ...
                found_match = any(exp.lower() in missing.lower() or missing.lower() in exp.lower() 
                                for exp in expected_docs)
                assert found_match, f"Unexpected missing document: {missing}"
        
        print(f"Document score: {data['documents']['score']}% ({data['documents']['found']}/5 found)")
    ...
    def test_create_beneficiary_with_dob_and_gender(self, auth_headers, estate_id):
        ...
        resp = requests.post(f"{BASE_URL}/api/beneficiaries", headers=auth_headers, json={
            "estate_id": estate_id,
        ...
        assert data.get("date_of_birth") == "2010-05-15", f"DOB not stored: {data.get('date_of_birth')}"
        ...
        delete_resp = requests.delete(f"{BASE_URL}/api/beneficiaries/{data['id']}", headers=auth_headers)
        assert delete_resp.status_code == 200
        
        print(f"Beneficiary created with DOB={data['date_of_birth']}, gender={data['gender']}")
    ...
    def test_readiness_updates_when_beneficiary_added(self, auth_headers, estate_id):
        ...
        initial_resp = requests.get(f"{BASE_URL}/api/estate/{estate_id}/readiness", headers=auth_headers)
        initial_data = initial_resp.json()
        initial_msg_score = initial_data["messages"]["score"]
        
        # Add a beneficiary
        unique_email = f"test_ben2_{uuid.uuid4().hex[:8]}@test.com"
        create_resp = requests.post(f"{BASE_URL}/api/beneficiaries", headers=auth_headers, json={
            "estate_id": estate_id,
            "name": "TEST_Jane Doe",
            "email": unique_email,
            "relation": "Daughter",
        ...
        updated_resp = requests.get(f"{BASE_URL}/api/estate/{estate_id}/readiness", headers=auth_headers)
        ...
        print(f"Message score before: {initial_msg_score}%, after: {updated_data['messages']['score']}%")
    ...
    def test_new_estate_gets_default_checklist_items(self, auth_headers):
        ...
        # Create a new estate
        estate_resp = requests.post(f"{BASE_URL}/api/estates", headers=auth_headers, json={
            "name": "TEST_New Estate for Checklist Test"
        })
        assert estate_resp.status_code == 200, f"Create estate failed: {estate_resp.text}"
        new_estate = estate_resp.json()
        new_estate_id = new_estate["id"]
        
        try:
            # Get checklists for new estate
            checklist_resp = requests.get(f"{BASE_URL}/api/checklists/{new_estate_id}", headers=auth_headers)
            assert checklist_resp.status_code == 200
            checklists = checklist_resp.json()
            
            # Should have 30 default items
            assert len(checklists) == 30, f"Expected 30 default checklist items, got {len(checklists)}"
            
            # Verify some expected categories
            categories = set(item["category"] for item in checklists)
            expected_categories = {"immediate", "first_week", "two_weeks", "first_month"}
            assert categories == expected_categories, f"Unexpected categories: {categories}"
            
            print(f"New estate got {len(checklists)} default checklist items with categories: {categories}")
        finally:
            # Cleanup - delete the test estate
            requests.delete(f"{BASE_URL}/api/estates/{new_estate_id}", headers=auth_headers)
    ...
    def test_checklist_score_requires_25_items(self, auth_headers, estate_id):
        ...
        assert checklist["required"] >= 25, f"Checklist required should be >= 25, got {checklist['required']}"
        
        # If less than 25 items, missing should mention adding more
        if checklist["found"] < 25:
            missing_text = " ".join(checklist.get("missing", []))
            print(f"Checklist score: {checklist['score']}%, found: {checklist['found']}, missing info: {missing_text}")
    
    # Test 8: Messages score calculation based on beneficiary demographics
    def test_messages_score_based_on_beneficiary_demographics(self, auth_headers, estate_id):
        ...
        print(f"Messages score: {messages['score']}%, found: {messages['found']}, required: {messages['required']}")
    ...
    def test_toggle_checklist_updates_readiness(self, auth_headers, estate_id):
        ...
        # Get checklists
        checklist_resp = requests.get(f"{BASE_URL}/api/checklists/{estate_id}", headers=auth_headers)
        checklists = checklist_resp.json()
        
        # Find an incomplete item
        incomplete_item = next((item for item in checklists if not item["is_completed"]), None)
        ...
        print(f"Checklist score before toggle: {initial_score}%, after: {updated_score}%")
        
        # Toggle back to restore state
        requests.patch(f"{BASE_URL}/api/checklists/{incomplete_item['id']}/toggle", headers=auth_headers)
...
class TestReadinessAPIResponses:
    ...
    def auth_token(self):
        ...
        login_resp = requests.post(f"{BASE_URL}/api/auth/login", json={  # noqa: F841
            "email": "pete@mitchell.com",
            "password": "password123"
        })
    ...
    def test_readiness_persists_to_estate(self, auth_headers):
        ...
        if estates:
            ...
            if "readiness_breakdown" in estate:
                ...
                print(f"Estate has persisted readiness_breakdown: {estate['readiness_breakdown']}")
    ...
    def test_readiness_404_for_invalid_estate(self, auth_headers):
        ...
        resp = requests.get(f"{BASE_URL}/api/estate/{fake_id}/readiness", headers=auth_headers)
