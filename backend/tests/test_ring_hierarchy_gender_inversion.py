...
class TestResolveInverseLogic:
    ...
    def auth_token(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/auth/login", json={"email": LOGIN_EMAIL, "password": LOGIN_PASSWORD}, timeout=30
        )
    ...
    def test_family_connections_endpoint_exists(self, auth_token):
        ...
        response = requests.get(f"{BASE_URL}/api/beneficiary/family-connections", headers=headers, timeout=30)
    ...
    def test_resolve_inverse_father_unknown_gender(self):
        ...
        assert resolve_inverse("Father", "prefer_not_to_say") == "Son/Daughter", (
            "Father + prefer_not_to_say should be Son/Daughter"
        )
...
class TestBackendAPIIntegration:
    ...
    def test_estates_endpoint(self, auth_token):
        ...
        headers = {"Authorization": f"Bearer {auth_token}"}
        response = requests.get(f"{BASE_URL}/api/estates", headers=headers, timeout=30)
        assert response.status_code == 200
