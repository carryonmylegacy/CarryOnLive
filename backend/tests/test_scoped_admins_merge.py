...
class TestScopedAdminsMerge:
    ...
    def test_create_scoped_admin_new_email(self):
        ...
        assert data.get("merged") is None or not data.get("merged"), "merged should be False or absent for new admin"
    ...
    def test_create_scoped_admin_merge_existing_email(self):
        ...
        assert data2.get("merged"), "merged flag should be True"
    ...
    def test_update_scoped_admin(self):
        ...
        # Update the admin
        update_payload = {"admin_scope": ["compliance", "marketing"], "first_name": "Updated"}
        ...
        assert data.get("updated"), "updated should be True"
    ...
    def test_delete_scoped_admin(self):
        ...
        assert data.get("deleted"), "deleted should be True"
    ...
    def test_delete_nonexistent_admin_returns_404(self):
        ...
        print("Correctly returned 404 for non-existent admin")
    ...
    def test_update_nonexistent_admin_returns_404(self):
        ...
        response = self.session.put(f"{BASE_URL}/api/admin/scoped-admins/{fake_id}", json={"admin_scope": ["finance"]})
    ...
    def test_all_valid_scopes_accepted(self):
        ...
        for scope in test_scopes:
            ...
            assert response.status_code == 200, (
                f"Scope '{scope}' should be valid, got {response.status_code}: {response.text}"
            )
        ...
        for op in operators:
            ...
            if operator_role == "manager":
                assert "ops_manager" in admin_scope or len(admin_scope) > 0, (
                    "Manager operator should have ops_manager scope or explicit scopes"
                )
            elif operator_role == "worker":
                assert "ops_team" in admin_scope or len(admin_scope) > 0, (
                    "Worker operator should have ops_team scope or explicit scopes"
                )
...
class TestScopedAdminsEdgeCases:
    ...
    def test_update_admin_with_empty_payload(self):
        ...
        create_response = self.session.post(
            f"{BASE_URL}/api/admin/scoped-admins",
            json={
                "email": unique_email,
                "password": "TestPass123!",
                "first_name": "Empty",
        ...
        assert update_response.status_code == 200
        data = update_response.json()
        assert not data.get("updated"), "updated should be False for empty payload"
    ...
    def test_email_case_insensitivity(self):
        ...
        data = response2.json()
        assert data.get("merged"), "Should merge with case-insensitive email match"
