...
class TestDocumentsPreTransition:
    ...
    def test_pre_transition_endpoint_exists(self, auth_token):
        ...
        response = requests.get(
            f"{BASE_URL}/api/documents/{estate_id}/pre-transition",
            headers=headers
        )
...
class TestDocumentDesignation:
    ...
    def test_designate_beneficiaries_accepts_visibility_timing(self, auth_token):
        ...
        payload = {
            "beneficiary_ids": ["all"],
            "visibility_timing": {
                "test_ben_id": {"pre": True, "post": True}
            }
        }
...
class TestECTAudioUpload:
    ...
    def test_upload_accepts_audio_webm(self, auth_token):
        ...
        files = {
            "file": ("voice-message.webm", io.BytesIO(audio_content), "audio/webm")
        }
        
        response = requests.post(
            f"{BASE_URL}/api/estate-chat/channels/{channel_id}/upload",
            headers=headers,
            files=files
        )
    ...
    def test_upload_accepts_audio_mp4(self, auth_token):
        ...
        files = {
            "file": ("voice-message.m4a", io.BytesIO(audio_content), "audio/mp4")
        }
