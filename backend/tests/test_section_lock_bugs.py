...
class TestSectionLockBackend:
    ...
    def estate_id(self, auth_token):
        ...
        response = requests.get(f"{BASE_URL}/api/estates", headers={
    ...
    def document_id(self, auth_token, estate_id):
        ...
        response = requests.get(f"{BASE_URL}/api/documents/{estate_id}", headers={
            "Authorization": f"Bearer {auth_token}"
        })
    ...
    def test_security_settings_endpoint(self, auth_token):
        ...
        response = requests.get(f"{BASE_URL}/api/security/settings", headers={
            "Authorization": f"Bearer {auth_token}"
        })
    ...
    def test_sdv_lock_blocks_download_when_active(self, auth_token, estate_id, document_id):
        ...
        settings_response = requests.get(f"{BASE_URL}/api/security/settings", headers={
            "Authorization": f"Bearer {auth_token}"
        })
...
class TestAPIEndpoints:
    ...
    def test_beneficiaries_endpoint(self, auth_token):
        ...
        # First get estate
        estates_response = requests.get(f"{BASE_URL}/api/estates", headers={
            "Authorization": f"Bearer {auth_token}"
        })
    ...
    def test_messages_endpoint(self, auth_token):
        ...
        response = requests.get(f"{BASE_URL}/api/messages/{estate_id}", headers={
            "Authorization": f"Bearer {auth_token}"
        })
        assert response.status_code == 200
    ...
    def test_checklists_endpoint(self, auth_token):
        ...
        response = requests.get(f"{BASE_URL}/api/checklists/{estate_id}", headers={
            "Authorization": f"Bearer {auth_token}"
        })
        assert response.status_code == 200
