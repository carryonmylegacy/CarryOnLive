...
def auth_token():
    ...
    verify_response = requests.post(
        f"{BASE_URL}/api/auth/verify-otp", json={"email": ADMIN_EMAIL, "otp": dev_otp}
    )
...
class TestSecurityQuestions:
    ...
    def test_get_security_questions(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/security/questions", headers=auth_headers
        )
...
class TestSecuritySettingsRetrieval:
    ...
    def test_get_security_settings_returns_all_sections(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/security/settings", headers=auth_headers
        )
        ...
        for section_id in SECTION_IDS:
            ...
            assert "password_enabled" in section, (
                f"Missing password_enabled for {section_id}"
            )
            assert "voice_enabled" in section, f"Missing voice_enabled for {section_id}"
            assert "security_question_enabled" in section, (
                f"Missing security_question_enabled for {section_id}"
            )
...
class TestPasswordSecurityLayer:
    ...
    def test_create_password_security_for_sdv(self, auth_headers):
        ...
        requests.delete(f"{BASE_URL}/api/security/settings/sdv", headers=auth_headers)
        ...
        assert response.status_code == 200, (
            f"Failed to create password security: {response.text}"
        )
...
class TestSecurityQuestionLayer:
    ...
    def test_create_security_question_for_mm(self, auth_headers):
        ...
        requests.delete(f"{BASE_URL}/api/security/settings/mm", headers=auth_headers)
        ...
        assert response.status_code == 200, (
            f"Failed to create security question: {response.text}"
        )
...
class TestRemoveSecurity:
    ...
    def test_delete_security_settings(self, auth_headers):
        ...
        response = requests.delete(
            f"{BASE_URL}/api/security/settings/sdv", headers=auth_headers
        )
...
class TestInvalidSectionHandling:
    ...
    def test_invalid_section_on_delete(self, auth_headers):
        ...
        response = requests.delete(
            f"{BASE_URL}/api/security/settings/invalid_section", headers=auth_headers
        )
        assert response.status_code == 400
...
class TestNoSecurityConfigured:
    ...
    def test_verify_no_security_returns_verified(self, auth_headers):
        ...
        requests.delete(f"{BASE_URL}/api/security/settings/ega", headers=auth_headers)
...
class TestCleanup:
    ...
    def test_cleanup_test_security_settings(self, auth_headers):
        ...
        for section_id in SECTION_IDS:
            requests.delete(
                f"{BASE_URL}/api/security/settings/{section_id}", headers=auth_headers
            )
