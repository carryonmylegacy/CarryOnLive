...
class TestSectionUnlockSession:
    ...
    def test_verify_section_creates_unlock_session(self, authenticated_client):
        ...
        assert settings_res.status_code == 200, (
            f"Failed to get settings: {settings_res.text}"
        )
        ...
        response = authenticated_client.post(
            f"{BASE_URL}/api/security/verify/{section_id}", data=form_data
        )
        assert response.status_code == 200, (
            f"Expected 200, got {response.status_code}: {response.text}"
        )
    ...
    def test_check_unlock_status_unlocked(self, authenticated_client):
        ...
        # Check unlock status
        status_res = authenticated_client.get(
            f"{BASE_URL}/api/security/unlock-status/{section_id}"
        )
        assert status_res.status_code == 200, (
            f"Expected 200, got {status_res.status_code}: {status_res.text}"
        )
        data = status_res.json()
        assert "unlocked" in data, f"Expected 'unlocked' key in response: {data}"
        assert data["unlocked"] is True, (
            f"Expected unlocked=True after verify, got {data}"
        )
    ...
    def test_estate_readiness(self, authenticated_client):
        ...
        if len(estates) > 0:
            ...
            assert "overall_score" in data or "documents" in data, (
                f"Expected readiness data: {data}"
            )
