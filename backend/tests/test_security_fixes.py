...
class TestSectionUnlockSession:
    ...
    def test_verify_section_creates_unlock_session(self, authenticated_client):
        ...
        form_data = {}
        response = authenticated_client.post(f"{BASE_URL}/api/security/verify/{section_id}", data=form_data)
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
    ...
    def test_check_unlock_status_unlocked(self, authenticated_client):
        ...
        form_data = {}
        verify_res = authenticated_client.post(f"{BASE_URL}/api/security/verify/{section_id}", data=form_data)
