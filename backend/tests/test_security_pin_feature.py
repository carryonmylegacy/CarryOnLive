...
class TestSecuritySettingsUpdate:
    ...
    def test_update_settings_persists_pin(self, auth_headers):
        ...
        data = response.json()
        assert data["mm"]["has_pin"] == True, "PIN should be set"
        assert data["mm"]["pin_enabled"] == True, "PIN should be enabled"
...
class TestRemoveSecurity:
    ...
    def test_remove_section_security(self, auth_headers):
        ...
        assert data["iac"]["is_active"] == False, "Security should be removed"
...
class TestMultiLayerSecurity:
    ...
    def test_enable_all_three_layers(self, auth_headers):
        ...
        assert data["sdv"]["pin_enabled"] == True
        assert data["sdv"]["has_pin"] == True
        assert data["sdv"]["password_enabled"] == True
        assert data["sdv"]["has_password"] == True
        assert data["sdv"]["security_question_enabled"] == True
        assert data["sdv"]["has_security_question"] == True
        assert data["sdv"]["is_active"] == True
    ...
    def test_verify_all_three_layers(self, auth_headers):
        ...
        data = response.json()
        assert data.get("verified") == True
        assert data.get("results", {}).get("pin") == True
        assert data.get("results", {}).get("password") == True
