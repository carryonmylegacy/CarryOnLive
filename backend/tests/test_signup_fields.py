...
class TestSignupRegistration:
    ...
    def test_register_with_all_suffix_options(self):
        ...
        for suffix in suffix_options:
            ...
            response = requests.post(f"{BASE_URL}/api/auth/register", json=payload)
            assert response.status_code == 200, f"Failed for suffix '{suffix}': {response.status_code}"
    ...
    def test_register_with_all_gender_options(self):
        ...
        for gender in gender_options:
            ...
            response = requests.post(f"{BASE_URL}/api/auth/register", json=payload)
            assert response.status_code == 200, f"Failed for gender '{gender}': {response.status_code}"
    ...
    def test_register_missing_last_name(self):
        ...
        # Should fail with 422 (validation error)
        assert response.status_code == 422, f"Expected 422 for missing last_name, got {response.status_code}"
