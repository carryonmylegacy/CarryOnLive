...
class TestSingleSessionEnforcement:
    ...
    def _get_admin_token(self):
        ...
        try:
            response = self.session.post(
                ...
                json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD, "force_login": True},
            )
    ...
    def test_01_admin_login_exempt_from_single_session(self):
        ...
        assert not data2.get("active_session_exists"), "Admin should be exempt from single-session blocking"
    ...
    def test_02_force_login_overrides_existing_session(self):
        ...
        response = self.session.post(
            f"{BASE_URL}/api/auth/login", json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD, "force_login": True}
        )
        ...
        assert not data.get("active_session_exists"), "force_login should override existing session"
    ...
    def test_03_logout_clears_active_session(self):
        ...
        # Logout
        logout_response = self.session.post(f"{BASE_URL}/api/auth/logout", headers={"Authorization": f"Bearer {token}"})
...
class TestPollingRefPattern:
    ...
    def test_01_dashboard_uses_ref_pattern(self):
        ...
        try:
            ...
            assert "getAuthHeadersRef" in content, "DashboardPage should use getAuthHeadersRef pattern"
    ...
    def test_02_checklist_uses_ref_pattern(self):
        ...
        try:
            ...
            # Check for getAuthHeadersRef pattern
            assert "getAuthHeadersRef" in content, "ChecklistPage should use getAuthHeadersRef pattern"
...
class TestGuardianIACSummary:
    ...
    def test_01_guardian_shows_items_added_and_duplicates(self):
        ...
        try:
            ...
            assert "items_added" in content, "GuardianPage should reference items_added"

            # Check for duplicates_skipped display
            assert "duplicates_skipped" in content, "GuardianPage should reference duplicates_skipped"

            # Check for IAC items added text
            assert "IAC items added" in content, "GuardianPage should display 'IAC items added' text"
