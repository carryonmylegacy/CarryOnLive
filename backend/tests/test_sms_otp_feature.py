...
class TestSMSOTPFeature:
    ...
    def test_sms_otp_status_endpoint_exists(self):
        ...
        response = self.session.get(
            f"{BASE_URL}/api/auth/sms-otp-status",
        ...
        print(f"✓ SMS OTP status endpoint works: enabled={data['sms_otp_enabled']}, masked_phone={data['masked_phone']}")
    ...
    def test_sms_otp_verify_requires_pending_verification(self):
        ...
        response = self.session.post(
            f"{BASE_URL}/api/auth/sms-otp-verify",
            headers={"Authorization": f"Bearer {token}"},
            json={"otp": "123456"}
        )
    ...
    def test_sms_otp_disable_endpoint(self):
        ...
        assert data["sms_otp_enabled"] == False, "sms_otp_enabled should be False after disable"
    ...
    def test_resend_otp_accepts_method_parameter(self):
        ...
        response = self.session.post(
            ...
            json={"email": "nonexistent@test.com", "method": "email"}
        )
        ...
        response = self.session.post(
            f"{BASE_URL}/api/auth/resend-otp",
            json={"email": "nonexistent@test.com", "method": "sms"}
        )
    ...
    def test_login_response_structure_for_sms_user(self):
        ...
        if data.get("otp_required"):
            ...
            print(f"✓ Login OTP response has SMS fields: otp_method={data.get('otp_method')}, has_sms={data.get('has_sms')}")
        else:
