...
class TestSubscriptionPricing:
    ...
    def test_get_subscription_settings_returns_family_discount_fields(self):
        ...
        assert isinstance(data["family_benefactor_discount_percent"], (int, float)), "family_benefactor_discount_percent should be numeric"
        assert isinstance(data["family_beneficiary_discount_percent"], (int, float)), "family_beneficiary_discount_percent should be numeric"
    ...
    def _update_plan_price(self, plan_id, price):
        ...
        return requests.put(
            f"{BASE_URL}/api/admin/plans/{plan_id}/price",
            data={"price": price},
            headers=headers
        )
    ...
    def _update_beneficiary_plan_price(self, plan_id, price):
        ...
        headers = {k: v for k, v in self.session.headers.items() if k.lower() != 'content-type'}
        return requests.put(
            f"{BASE_URL}/api/admin/beneficiary-plans/{plan_id}/price",
            data={"price": price},
            headers=headers
        )
    ...
    def test_update_military_plan_price(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data.get("success") == True, f"Military plan price update should succeed, got {data}"
    ...
    def test_update_new_adult_plan_price(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data.get("success") == True, f"New Adult plan price update should succeed, got {data}"
    ...
    def test_update_hospice_plan_price(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data.get("success") == True, f"Hospice plan price update should succeed, got {data}"
    ...
    def test_update_veteran_plan_price(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data.get("success") == True, f"Veteran plan price update should succeed, got {data}"
    ...
    def test_update_enterprise_plan_price(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data.get("success") == True, f"Enterprise plan price update should succeed, got {data}"
    ...
    def test_update_family_benefactor_discount(self):
        ...
        response = self.session.put(
            ...
            json={"family_benefactor_discount_percent": 15}
        )
    ...
    def test_update_family_beneficiary_discount(self):
        ...
        response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={"family_beneficiary_discount_percent": 20}
        )
    ...
    def test_update_both_family_discounts(self):
        ...
        response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={
                "family_benefactor_discount_percent": 10,
                "family_beneficiary_discount_percent": 25
            }
        )
    ...
    def test_family_discount_persists(self):
        ...
        update_response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={
                "family_benefactor_discount_percent": 12,
                "family_beneficiary_discount_percent": 18
            }
        )
        ...
        assert data.get("family_benefactor_discount_percent") == 12, f"Expected 12, got {data.get('family_benefactor_discount_percent')}"
        assert data.get("family_beneficiary_discount_percent") == 18, f"Expected 18, got {data.get('family_beneficiary_discount_percent')}"
    
    def test_family_discount_rejects_negative_value(self):
        """PUT /admin/family-discount-settings should reject values < 0"""
        response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={"family_benefactor_discount_percent": -5}
        )
    ...
    def test_family_discount_rejects_over_100(self):
        ...
        response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={"family_beneficiary_discount_percent": 150}
        )
    ...
    def test_family_discount_accepts_zero(self):
        ...
        response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={"family_benefactor_discount_percent": 0}
        )
    ...
    def test_family_discount_accepts_100(self):
        ...
        response = self.session.put(
            f"{BASE_URL}/api/admin/family-discount-settings",
            json={"family_beneficiary_discount_percent": 100}
        )
