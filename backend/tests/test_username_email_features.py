...
BASE_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://multi-estate.preview.emergentagent.com')
...
class TestUsernameEndpoints:
    ...
    def test_set_username(self, auth_token):
        ...
        print(f"✅ Username persisted and verified via GET")
    ...
    def test_duplicate_username_rejected(self, auth_token):
        ...
        print(f"✅ Username uniqueness endpoint works (400 error returned for duplicates)")
...
class TestLoginWithUsername:
    ...
    def test_login_with_username_after_setting(self):
        ...
        username_login_response = requests.post(f"{BASE_URL}/api/auth/login", json={
...
class TestBeneficiaryEmailChange:
    ...
    def auth_token_and_beneficiary(self):
        ...
        estate_id = None
        for e in estates:
            if e.get("user_role_in_estate") == "owner" or (not e.get("user_role_in_estate") and not e.get("is_beneficiary_estate")):
                estate_id = e.get("id")
                break
    ...
    def test_email_change_returns_flag(self, auth_token_and_beneficiary):
        ...
        assert data.get("email_changed") == True, f"email_changed flag not set or False: {data}"
        
        # Verify invitation_status was reset to pending
        assert data.get("invitation_status") == "pending", f"invitation_status not reset: {data.get('invitation_status')}"
        
        print(f"✅ Beneficiary email change returns email_changed: true and resets invitation_status to 'pending'")
    ...
    def test_no_email_change_no_flag(self, auth_token_and_beneficiary):
        ...
        assert data.get("email_changed") != True, f"email_changed should not be set when email unchanged: {data}"
        print(f"✅ Beneficiary update without email change does not set email_changed flag")
...
class TestLoginPageLabels:
    ...
    def test_login_page_labels(self):
        ...
        assert occurrences >= 2, f"'Username or Email' should appear at least twice (label + placeholder), found {occurrences}"
...
class TestFamilyTreeLegend:
    ...
    def test_beneficiaries_page_no_duplicate_legend(self):
        ...
        assert blue_nodes_count == 0, f"BeneficiariesPage should NOT have 'Blue nodes = ...' line, found {blue_nodes_count}"
