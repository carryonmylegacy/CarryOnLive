...
BASE_URL = os.environ.get("REACT_APP_BACKEND_URL", "https://ui-responsive-fix-10.preview.emergentagent.com")
...
class TestAdminVerifications:
    ...
    def test_admin_verifications_list(self, api_client, admin_headers):
        ...
        if data:
            ...
            print(
                f"✓ Sample verification: tier={v.get('tier_requested')}, status={v.get('status')}, user={v.get('user_email')}"
            )
        return data
    ...
    def test_admin_verification_notify(self, api_client, admin_headers):
        ...
        if not approved_verification:
            ...
            if approved_any:
                ...
                print(
                    f"✓ Notify endpoint responded: {response.status_code} - {response.text[:100] if response.text else 'No body'}"
                )
                return
            else:
