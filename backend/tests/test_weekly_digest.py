...
class TestDigestPreferences:
    ...
    def test_get_preferences_requires_auth(self):
        ...
        assert resp.status_code == 403 or resp.status_code == 401, f"Expected 401/403 but got {resp.status_code}"
        print(f"GET /api/digest/preferences without auth: {resp.status_code} (expected)")
    ...
    def test_get_preferences_default_true(self, admin_token):
        ...
        assert isinstance(data["weekly_digest"], bool), "weekly_digest should be boolean"
    ...
    def test_put_preferences_requires_auth(self):
        ...
        assert resp.status_code == 403 or resp.status_code == 401, f"Expected 401/403 but got {resp.status_code}"
        print(f"PUT /api/digest/preferences without auth: {resp.status_code} (expected)")
...
class TestDigestSendWeekly:
    ...
    def non_admin_token(self):
        ...
        admin_resp = requests.post(f"{BASE_URL}/api/auth/dev-login", json=ADMIN_CREDENTIALS)
    ...
    def test_send_weekly_requires_auth(self):
        ...
        print(f"POST /api/digest/send-weekly without auth: {resp.status_code} (expected)")
...
class TestDigestPreview:
    ...
    def test_preview_for_authorized_user(self, admin_token):
        ...
        if resp.status_code == 200:
            ...
            assert resp.status_code != 403, f"Admin should be authorized for preview: {resp.text}"
