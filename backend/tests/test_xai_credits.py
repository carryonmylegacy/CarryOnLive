...
class TestXAICreditsAPI:
    ...
    def setup(self):
        ...
        # Login as admin
        login_res = requests.post(f"{BASE_URL}/api/auth/login", json={"email": ADMIN_EMAIL, "password": ADMIN_PASSWORD})
    ...
    def test_set_balance_requires_balance_usd(self):
        ...
        res = requests.post(f"{BASE_URL}/api/admin/xai-credits/set-balance", headers=self.headers, json={})
    ...
    def test_set_balance_with_valid_amount(self):
        ...
        # Set balance to $500
        res = requests.post(
            f"{BASE_URL}/api/admin/xai-credits/set-balance", headers=self.headers, json={"balance_usd": 500.0}
        )
    ...
    def test_set_balance_reflects_in_get(self):
        ...
        set_res = requests.post(
            f"{BASE_URL}/api/admin/xai-credits/set-balance", headers=self.headers, json={"balance_usd": 750.0}
        )
        ...
        assert data["initial_balance_usd"] == 750.0, (
            f"Expected initial_balance_usd=750.0, got {data['initial_balance_usd']}"
        )
    ...
    def test_warning_level_healthy_above_100(self):
        ...
        # Set balance to $200
        requests.post(
            f"{BASE_URL}/api/admin/xai-credits/set-balance", headers=self.headers, json={"balance_usd": 200.0}
        )
    ...
    def test_warning_level_warning_between_25_and_100(self):
        ...
        # Set balance to $50
        requests.post(f"{BASE_URL}/api/admin/xai-credits/set-balance", headers=self.headers, json={"balance_usd": 50.0})
    ...
    def test_warning_level_critical_below_25(self):
        ...
        # Set balance to $10
        requests.post(f"{BASE_URL}/api/admin/xai-credits/set-balance", headers=self.headers, json={"balance_usd": 10.0})
...
class TestXAICreditsUnauthorized:
    ...
    def test_set_balance_requires_auth(self):
        ...
        res = requests.post(f"{BASE_URL}/api/admin/xai-credits/set-balance", json={"balance_usd": 100.0})
        assert res.status_code in [401, 403], f"Expected 401/403, got {res.status_code}"
