...
export default function ECTSecurityIntro({ introStep, setIntroStep, onDismiss, onBack }) {
  return (
    <div className="fixed inset-0 z-[60] flex flex-col items-center overflow-y-auto" style={{ background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(12px)', paddingLeft: '14px', paddingRight: '14px', paddingTop: 'calc(64px + env(safe-area-inset-top, 0px))', paddingBottom: 'calc(84px + env(safe-area-inset-bottom, 0px))' }}>
      <div className="w-full max-w-md rounded-2xl px-3.5 py-2.5 my-auto flex-shrink-0" data-testid="ect-security-intro" style={{ background: 'rgba(15,22,41,0.95)', border: '1px solid rgba(212,175,55,0.3)', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}>
        ...
        {introStep === 1 && (
          <div data-testid="ect-intro-step-1">
            <div className="text-center mb-2">
              ...
              <p className="text-xs mt-0.5" style={{ color: 'var(--t4)' }}>Estate Comms isn't like other messaging apps. Here's why.</p>
            </div>
            <div className="space-y-1 mb-2">
              {[
                { icon: Lock, title: 'Closed Network', desc: 'Only people explicitly connected to your estate can message you.' },
                { icon: Shield, title: 'No Phone Number Needed', desc: 'Your number is never exposed. No contact scanning or profile discovery.' },
                { icon: Users, title: 'Owner-Controlled Access', desc: 'The estate benefactor controls who is in and who is out.' },
                { icon: X, title: 'Zero Data Mining', desc: 'No ads, no tracking, no metadata sold. Your conversations are yours.' },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-2 px-2 py-1.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <item.icon className="w-3.5 h-3.5 mt-0.5 flex-shrink-0" style={{ color: '#d4af37' }} />
                  <div>
            ...
            <p className="text-[11px] text-center mb-2" style={{ color: 'var(--t5)' }}>
        ...
        {introStep === 2 && (
          <div data-testid="ect-intro-step-2">
            <div className="text-center mb-2">
              ...
              <p className="text-xs mt-0.5" style={{ color: 'var(--t4)' }}>It's simple — here's everything you need to know.</p>
            </div>
            <div className="space-y-1 mb-2">
              {[
                { num: '1', title: 'Start a Conversation', desc: 'Tap the gold + button in the top right corner to start a new chat.' },
                { num: '2', title: 'Pick Who to Chat With', desc: 'Choose one family member for a private chat, or select several for a group.' },
                { num: '3', title: 'Type and Send', desc: 'Type your message in the box at the bottom and tap the send arrow.' },
                { num: '4', title: 'Send Photos & Voice', desc: 'Tap the paperclip to attach files. Tap the mic to record voice.' },
              ].map((item, i) => (
                <div key={i} className="flex items-start gap-2 px-2 py-1.5 rounded-lg" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div className="flex gap-2.5">
              ...
              <button
                onClick={onDismiss}
                className="flex-1 py-2 rounded-xl text-sm font-bold transition-all active:scale-[0.97]"
