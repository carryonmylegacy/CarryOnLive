...
export const useFounder = () => {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/public/site-content`).then(r => {
      const d = r.data || {};
      setFounder({ name: d.founder_name || DEFAULTS.name, title: d.founder_title || DEFAULTS.title, photo_url: d.founder_photo_url || '', linkedin_url: d.founder_linkedin_url || '', video_id: d.homepage_video_id || '' });
    }).catch(() => {});
  }, []);
  return founder;
};
