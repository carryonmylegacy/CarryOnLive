    ...
    <form onSubmit={submit} className="space-y-4" data-testid="testimonial-form">
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input id="t-name" required minLength={2} maxLength={60} value={form.name} onChange={set('name')} placeholder="e.g. Maria R." className={cls} style={input} data-testid="testimonial-name" />
        </div>
        <div>
          <label className="text-[#8b97ab] text-xs font-semibold block mb-1.5" htmlFor="t-location">City, State</label>
          <input id="t-location" maxLength={60} value={form.location} onChange={set('location')} placeholder="e.g. Arlington, VA" className={cls} style={input} data-testid="testimonial-location" />
        </div>
        <div>
          <label className="text-[#8b97ab] text-xs font-semibold block mb-1.5" htmlFor="t-role">Which best describes you?</label>
          <select id="t-role" value={form.role} onChange={set('role')} className={cls} style={input} data-testid="testimonial-role">
        ...
        <div>
          ...
          <input id="t-since" maxLength={20} value={form.member_since} onChange={set('member_since')} placeholder="e.g. 2025" className={cls} style={input} data-testid="testimonial-since" />
        </div>
      </div>
      <div>
        ...
        <textarea id="t-quote" required minLength={40} maxLength={600} rows={5} value={form.quote} onChange={set('quote')} placeholder="What made you set this up? What changed for your family?" className="w-full px-4 py-3 rounded-lg text-base text-white placeholder:text-[#4a5568] focus:outline-none focus:border-[#d4af37]" style={input} data-testid="testimonial-quote" />
      </div>
      <div>
        ...
        <input id="t-email" type="email" required value={form.email} onChange={set('email')} placeholder="you@example.com" className={cls} style={input} data-testid="testimonial-email" />
      </div>
