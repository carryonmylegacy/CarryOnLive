...
const ChangelogPage = () => {
  ...
  return (
    <div className={`min-h-screen transition-opacity duration-400 ${exiting ? 'opacity-0' : 'opacity-100'}`} style={{ background: '#0E1829' }} data-testid="changelog-page">
      <SEO title={t('changelog.seo.title')} description={t('changelog.seo.description')} path="/changelog" />
      ...
      <section className="relative overflow-hidden" style={{ paddingTop: 'calc(8rem + env(safe-area-inset-top, 0px))' }}>
        ...
        <RevealSection className="max-w-[760px] mx-auto px-6 text-center relative z-10 pb-14">
          <p className="text-[#d4af37] text-xs font-bold uppercase tracking-[0.2em] mb-4">{t('changelog.hero.eyebrow')}</p>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.08] mb-5" style={{ fontFamily: 'Outfit, sans-serif' }} data-testid="changelog-h1">{t('changelog.hero.title')}</h1>
          <p className="text-[#a0aec0] text-base lg:text-lg leading-relaxed">{renderCopy(t('changelog.hero.intro'))}</p>
        </RevealSection>
      </section>
          ...
          {entries.map((e, i) => (
            <RevealSection key={e.title} delay={i * 0.06}>
              <article className="rounded-2xl p-6 sm:p-8" style={{ background: 'linear-gradient(160deg, #1a2d4d 0%, #16284a 50%, #142240 100%)', border: `1px solid ${i === 0 ? 'rgba(212,175,55,0.45)' : 'rgba(212,175,55,0.2)'}` }} data-testid={`changelog-entry-${i}`}>
                <p className="text-[#d4af37] text-xs font-bold uppercase tracking-wider mb-2 flex items-center gap-2">{i === 0 && <Sparkles className="w-3.5 h-3.5" />}{e.date ? formatDate(e.date) : t('changelog.foundation')}</p>
      ...
      <section className="relative z-10 py-20 text-center" style={{ background: '#0D1B2A' }}>
        <RevealSection className="max-w-[600px] mx-auto px-6">
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4" style={{ fontFamily: 'Outfit, sans-serif' }}>{t('changelog.cta.title')}</h2>
          <p className="text-[#8b97ab] text-base mb-8">{renderCopy(t('changelog.cta.text'))}</p>
          <button onClick={() => navigateWithFade('/start')} className="inline-flex items-center gap-2 px-10 py-4 rounded-lg font-bold text-base transition-transform active:scale-95" style={{ background: '#d4af37', color: '#0B1221' }} data-testid="changelog-start-now">{t('changelog.cta.button')} <ChevronRight className="w-4 h-4" /></button>
        </RevealSection>
      </section>

      <MarketingFooter hide="changelog" testIdSuffix="-changelog" />
    </div>
  );
};
