...
const CustomersPage = () => {
  ...
  return (
    <div className={`min-h-screen transition-opacity duration-400 ${exiting ? 'opacity-0' : 'opacity-100'}`} style={{ background: '#0E1829' }} data-testid="customers-page">
      <SEO title={t('customers.seo.title')} description={t('customers.seo.description')} path="/customers" />
      ...
      <section className="relative overflow-hidden" style={{ paddingTop: 'calc(8rem + env(safe-area-inset-top, 0px))' }}>
        ...
        <RevealSection className="max-w-[800px] mx-auto px-6 text-center relative z-10 pb-16">
          <p className="text-[#d4af37] text-xs font-bold uppercase tracking-[0.2em] mb-4">{t('customers.hero.eyebrow')}</p>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.08] mb-5" style={{ fontFamily: 'Outfit, sans-serif', textWrap: 'balance' }} data-testid="customers-h1">
            {t('customers.hero.h1a')} <span className="text-[#d4af37]">{t('customers.hero.h1b')}</span>
          </h1>
          <p className="text-[#a0aec0] text-base lg:text-lg leading-relaxed max-w-[640px] mx-auto">
            {renderCopy(t('customers.hero.intro'), 'text-[#10b981] font-medium')}
          </p>
        </RevealSection>
      </section>
          ...
          {loaded && total === 0 && (
            <RevealSection>
              <div className="rounded-2xl p-8 sm:p-10 text-center" style={{ background: 'linear-gradient(160deg, #1a2d4d 0%, #16284a 50%, #142240 100%)', border: '1px solid rgba(212,175,55,0.25)' }} data-testid="customers-empty">
                ...
                <h2 className="text-white text-2xl font-bold mb-3" style={{ fontFamily: 'Outfit, sans-serif' }}>{t('customers.empty.title')}</h2>
                <p className="text-[#a0aec0] text-base leading-relaxed max-w-[560px] mx-auto">
                  {renderCopy(t('customers.empty.text'))}
                </p>
              </div>
            </RevealSection>
          )}
      ...
      <section className="relative z-10 py-20" style={{ background: 'linear-gradient(180deg, #111F34, #0D1B2A)' }} data-testid="customers-founder">
        <div className="max-w-[1000px] mx-auto px-6">
          <RevealSection>
            <p className="text-[#d4af37] text-xs font-bold uppercase tracking-[0.2em] text-center mb-3">{t('customers.founder.eyebrow')}</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-10" style={{ fontFamily: 'Outfit, sans-serif' }}>{t('customers.founder.title')}</h2>
          </RevealSection>
      ...
      <section id="share" className="relative z-10 py-20" style={{ background: '#0D1B2A' }} data-testid="customers-share">
        <div className="max-w-[760px] mx-auto px-6">
          <RevealSection>
            <p className="text-[#d4af37] text-xs font-bold uppercase tracking-[0.2em] text-center mb-3">{t('customers.share.eyebrow')}</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-white text-center mb-3" style={{ fontFamily: 'Outfit, sans-serif' }}>{t('customers.share.title')}</h2>
            <p className="text-[#a0aec0] text-base text-center leading-relaxed mb-10 max-w-[560px] mx-auto">{renderCopy(t('customers.share.text'))}</p>
          </RevealSection>
      ...
      <section className="relative z-10 py-20 text-center" style={{ background: '#0E1829' }}>
        <RevealSection className="max-w-[600px] mx-auto px-6">
          <h2 className="text-3xl sm:text-4xl font-bold text-white mb-4" style={{ fontFamily: 'Outfit, sans-serif' }}>{t('customers.cta.title')}</h2>
          <p className="text-[#8b97ab] text-base mb-8">{renderCopy(t('customers.cta.text'))}</p>
          <button onClick={() => navigateWithFade('/start')} className="inline-flex items-center gap-2 px-10 py-4 rounded-lg font-bold text-base transition-transform active:scale-95" style={{ background: '#d4af37', color: '#0B1221' }} data-testid="customers-start-now">{t('customers.cta.button')} <ChevronRight className="w-4 h-4" /></button>
          <TrustBadges className="mt-8" testIdSuffix="-customers" />
        </RevealSection>
      </section>

      <MarketingFooter hide="customers" testIdSuffix="-customers" />
    </div>
  );
};
