  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto p-4 lg:p-8 pt-2 lg:pt-6 pb-24 lg:pb-8" data-testid="benefactor-dashboard"
      ...
      {!onboardingHiddenForToday && (() => {
        ...
        // Build-Your-Financial-Picture is now a step INSIDE the
        // Getting Started checklist (see OnboardingWizard.js → step
        // `build_financial_picture`), so we no longer count it
        // separately here. The wizard's `onboardingWizardHasContent`
        // signal already reflects whether that step is incomplete.
      ...
      {(user?.role === 'benefactor' || user?.is_also_benefactor) &&
       ...
       (onboardingProgress?.steps || []).some(s => !s.completed) && (
        ...
        {/* "Build Your Financial Picture" is now step 8 inside the
            Getting Started checklist (see OnboardingWizard.js →
            STEP_CONFIG.build_financial_picture + backend
            ONBOARDING_STEPS entry of the same key). The dedicated
            tile that used to live here was removed on Feb 26 2026. */}
