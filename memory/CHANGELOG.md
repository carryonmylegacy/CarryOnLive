...
**Verified visually** on `https://admin-verified-2.preview.emergentagent.com/financial` in light mode: "+ Add Bill", "Quick Add", "All" filter pill, "Map your entities & trusts", sidebar portal switchers, and the Public Device Mode pill all render identically. Build tag bumped to `V2026.05.21.6`.
...
iteration_123 testing-agent run identified that the JS-side fixes alone weren't enough — the S3 presigned URLs were 307-redirecting from the legacy global hostname to the regional one, and the redirect response was dropping CORS headers. The bucket CORS policy itself was correctly configured all along (`https://admin-verified-2.preview.emergentagent.com`, `https://carryon.us`, `https://www.carryon.us` — verified live via `s3.get_bucket_cors`).
