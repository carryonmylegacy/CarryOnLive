...
## Sep 21 2026 — "About" follows the Founder Story switch (NOT PUSHED)
- Founder request: when Admin → Marketing → Site Content → Founder Story is **Public**, the About link (hamburger + desktop nav on every public page) opens `/founder-about` directly; when **Invite-only**, it opens the generic `/about`. Same rule the homepage "Read his story" card already used (`FounderCard.js`).
- `MobileNav.js`: `aboutHref(flags)` + `resolveLinks(links, flags)`; applied in `MobileNav`, `MarketingNav`, `HomePage`, `LoginPage` (desktop + inline mobile dropdown) and the `/about` desktop nav (About becomes a link to the story when public; the separate "Founder" link shows only when invite-only). Gold "you are here" follows the resolved href, so About is gold on `/founder-about` when public.
- Verified on preview in both flag states (toggled via `PUT /api/admin/platform-settings`, restored to OFF).
