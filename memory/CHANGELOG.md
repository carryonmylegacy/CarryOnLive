...
- **RESOLVED Sep 21 (founder: option B — CarryOn Technologies LLC operates the platform, CarryOn Enterprises Inc is the parent/copyright holder):** Terms §1 contracting party → "CarryOn Technologies LLC, a CarryOn Enterprises Inc company"; §6 IP → "owned by CarryOn Technologies LLC or its parent company, CarryOn Enterprises Inc"; §12 contact → Technologies LLC; Privacy §1 → same operator identity; PublicFooter entity block → operator disclosure line (copyright stays "© CarryOn Enterprises Inc — All rights reserved.", period-free via `copyrightLine()`); JSON-LD on home/pricing/start/about → `legalName` "CarryOn Technologies LLC" + `parentOrganization` "CarryOn Enterprises Inc"; llms.txt updated; Terms + Privacy "Last updated" → September 21, 2026. `COMPANY` in `config/company.js` documents the structure.
- **(was) legal/operating entity conflict:** About footer says "CarryOn is operated by CarryOn Technologies LLC, a CarryOn Enterprises Inc company" (added Sep 20 per the audit's parent-disclosure instruction) while Terms §1 says CarryOn Enterprises Inc is "the company that operates the CarryOn™ platform". One of them is wrong. Once the founder states which entity operates the platform, align About disclosure (`COMPANY.disclosure`), Terms §1/§6/§12, Privacy Policy, footer identity and JSON-LD `legalName` — do NOT touch the Terms contracting entity before that instruction.

- **Founder correction:** the legal name is **CarryOn Enterprises Inc — no period**. Every occurrence (footers, JSON-LD legalName, Terms, partner "Powered by" lines, e-mails, llms.txt, SMS consent) now reads "Inc"; copyright line is "© 2026 CarryOn Enterprises Inc — All rights reserved."

- **Identity**: footer copyright → "CarryOn Enterprises Inc. All rights reserved." (`footer.copyright`, `footer.short_copyright`); phone standardised to (703) 889-0017 everywhere (`config/company.js` is the single source; `LandingContent`/`HomePage`/`LoginPage` defaults and the About footer read it); About footer adds "CarryOn is operated by CarryOn Technologies LLC, a CarryOn Enterprises Inc. company."; JSON-LD on `/`, `/pricing`, `/start`, `/about` → name "CarryOn", legalName "CarryOn Enterprises Inc.", url www.carryon.us; `llms.txt` + quiz-result email footer updated. Startup migration `public_claims_cleanup_v1` (`db_indexes.py`) fixes the Site Content `footer_phone` on prod and deletes any Site Copy override that still carries a retired phrase (history kept in `site_copy_history`). Founder Story narrative ("CarryOn Technologies was born") and Terms contracting entity left as-is.
- **Security wording**: Security H1 → "Your family's most private information deserves *security you can inspect.*"; "No backdoors. No exceptions." and every "support team can't open them from their screens" (home tool card, home security card, About cards, Privacy §5 plain-language line, llms.txt) → "not casually accessible to CarryOn staff — privileged access is restricted to defined administrative functions, controlled and audited"; `home.security.sub` no longer quotes "bank-grade".
- **Claims**: 76% → "76% of U.S. adults reported not having a will in a 2025 survey" (+ "even having a will doesn't necessarily mean…") on About and Founder Story; About "Who we are" → "three out of four adults without a will"; 570 h → "Settling an estate takes the average executor about 570 hours (spread over roughly 16 months)" on home + pricing (source doesn't split organized/unorganized records); hospice "the vast majority have no plan in place" removed (no source); attorney rate → "$200–$400, up to $500 for complex matters" with new `/sources#attorney` (LeanLaw 2025); `ATTORNEY_HOUR_LOW` 250 → 200. `/sources` now 5 entries.
- **Pricing/start**: pricing reduced-pricing line → `pricing.reduced.note` ("Reduced pricing is available for seniors, military/first responders, veterans, and young adults. Certified hospice families receive full access free."); `/start` Special Pricing now includes **Seniors** (was missing vs. /pricing); discreet Wind-Down Promise nudge (`pricing.winddown.q/cta`) under the plans on `/pricing` and `/start`; `/start` H1 → "Get your family's affairs in order". `/readiness-score` gained `readiness.hero.line` ("Readiness isn't about how much you own…").
- **SEO**: `/partner` removed from the sitemap (page is noindex); archived `/landing-consumer` set noindex; sitemap lastmods bumped; all 27 public routes verified: unique title/description, self-canonical on www, one H1, OG/Twitter tags, noindex only on auth/acquisition/archived pages. Public changelog entry 2026-09-20 added.
- Tests: iteration_213 — 27 routes × 3 viewports, zero retired phrases, all anchors/JSON-LD/sitemap/robots pass; check.sh ALL CLEAR. Console hydration warnings on tables are from the dev-only visual-editor plugin (not in prod build).
- **Founder next**: push; then (a) counsel glance at Privacy §5 plain-language sentence; (b) confirm the 5 `/sources` entries; (c) Google Search Console: resubmit sitemap, request indexing for `/sources`.


## Sep 20, 2026 — Marketing audit executed (4 → 2 → 3 → 5 → 6 → 1 → 7), iteration_212 — VERIFIED, NOT PUSHED

- **(4) Landing-page attribution** — Admin → Marketing → Funnel now opens with a **By Landing Page** card (7/30/90 d): visitors → CTA clicks → signups → activated → paid per entry page, with the three conversion rates. Backend `GET /admin/funnel-analytics/landing-pages` (`funnel_analytics.py`): visitors/CTA = distinct actors of `landing_view`/`landing_cta_click` by `meta.page`; signups = users tagged `landing_page`; **activated = 1+ vault document or 1+ milestone message**; paid = real (non-beta) `user_subscriptions` active/past_due. Every signup is now tagged: acquisition pages set it explicitly, everyone else gets the first path of their visit (`rememberEntryPath()` in `App.js`, `entryLandingPage()` in `SignupPage`) → "home", "pricing", … The homepage now records `landing_view {page:'home'}`.
- **(2) `/ready`** — Family Readiness page: "If something happened to you tomorrow, would your family know what to do?" → the 60-second quiz inline (`ReadinessQuiz` gained an `onStart` override so the ad's UTM is never overwritten) → gaps-to-features block → product preview → trust → Start Free. Tag `landing_page: ready`.
- **(3) `/moments`** — Milestone Messages page: "Your words for the moments you might miss." → founder video (when `homepage_video_id` is set) → product preview opening on the Messages tab (`ProductPreview defaultTab`) → three steps → "A message from Dad" card → trust → CTA. Tag `moments`. Both pages share `components/benefactor/useAcquisition.js`, the benefactor nav/trust/footer, and are fully editable in Site Copy ("Family Readiness landing page", "Milestone Messages landing page"). noindex, prerendered for link previews, not in the sitemap.
- **(5) `/sources` — Sources & Methodology** (indexed, in sitemap + both footers): one entry per figure — claim as used, *what it measures*, publisher, link, last-checked. Defaults: **570 hours** → EstateExec General Statistics; **76%** → Caring.com 2025 Wills Survey (24% have a will) — note the entry says plainly this measures *missing wills*; **300,000 in hospice** → NHPCO Facts & Figures (derived point-in-time estimate); **live homepage numbers** → counted live every 10 min, *includes the demo account*. Small gold `[n]` superscripts (`SourceRef`) now sit next to the figure on the homepage problem stat + hospice paragraph, `/pricing` value anchor, `/about` 76% paragraph and the Founder Story. All text is Site Copy ("Sources & Methodology").
- **(6) Security sub-heading** default (`home.security.sub`) no longer says "same standards that protect banks and government systems"; it lists the specific controls (separate key per family, support can't open files, two-step sign-in, human verification, access record, export/delete). If prod has a Site Copy override for this key it must be updated in the editor too.
- **(1) Homepage progressive collapse** — `MoreDetails.js`: outcomes, 12 tools (#features), platform, five steps (#steps), security (#security), FAQ (#faq) and hospice sit behind one "See everything CarryOn includes / Show me everything" expander; the Trust section moved up so the default read is hero → preview → video → problem → quiz → trust → expander → final CTA. Nav/footer anchor links auto-open the group and scroll; content stays in the DOM for search. Only the homepage collapses (`collapseDetails` prop) — `/login` and `/speak-with-us` unchanged.
- **(7) Founder earlier** — compact `FounderCard` under the homepage video (which *is* the founder video: `homepage_video_id` feeds both the homepage block and Customer Stories) and the founder video block on `/moments`.
- **Follow-up (founder review, same night):** homepage showed the founder twice — the compact card I had added under the video is gone (the full card in the Trust section, now early in the page, stays); same duplicate removed on `/moments` (its trust block already carries the card). Three oversights fixed while looking: (a) `recordFunnelEvent` now ignores automation — Vercel's build-time prerender (HeadlessChrome) and Playwright would otherwise count as a visitor on every build/test; (b) signups from the native iOS/Android app are tagged `landing_page: app`, not "home"; (c) the homepage hero **Start Now** now records `landing_cta_click {page:'home'}` so the Homepage row's CTA column isn't a permanent 0.
- Tests: iteration_212 backend 8/8 (attribution endpoint, event ingest, E2E register-with-tag + purge), frontend all flows at 390/1440 incl. hash auto-open, superscripts, admin card. check.sh ALL CLEAR (one transient smoke blip re-run clean).
- **Founder next**: push → (a) Site Copy → check `home.security.sub` override on prod; (b) confirm the four source entries or edit them; (c) point ads at `/benefactor`, `/ready`, `/moments` with `utm_source`/`utm_campaign`; (d) read Admin → Marketing → Funnel → By Landing Page after ~1 week.
...
- **Why the founder still saw the tiles (Sep 19, 23:43):** carryon.us was serving the 40 KB tiles file from the 23:10 deploy — the tree capture landed in the repo at 23:11, *after* that push, so it was never deployed. Fix + guard: every screenshot `<img>` (`ProductPreview`, `StepsShowcase`, `HeroShot`) now carries `?v=<SHOTS_VERSION>` from `components/landing/shotsVersion.js`, which the capture script rewrites on every run — a re-shot can no longer be masked by a browser/CDN cache. Verified on preview: phone frame loads `m-contacts.webp?v=20260919-2311` = Pete's Estate Tree.
...
- Re-captured all 8 (desktop 2160×1350 + iPhone 780×1328) from `petemitchell` on carryon.us with `scripts/capture_product_screenshots.py`; script now also hides the QuickStart modal and the dashboard "Onboarding — N action items" group, and tolerates `networkidle` timeouts on `/beneficiaries` (websocket). **Dashboard shots start at the Total Family Continuity meter** (founder: "just from the meter on down") — welcome line + beneficiary vault banner scrolled out on desktop and phone; `SHOT_ONLY=` env re-shoots a single page. Dashboard alt text updated (72 % Total Family Continuity). README in `public/screenshots/` records the capture date + regenerate steps. Prod demo password confirmed `Demo1234!!!` (`test_credentials.md`). check.sh ALL CLEAR.
...
- **Frontend port finished**: Admin → Marketing gains `Readiness Quiz` + `Testimonials` tabs (`adminSections.js`, `AdminPage.js`); Site Content tab gains **Founder Profile** (name/title/bio/LinkedIn) + **Live Platform Numbers** (auto/on/off) cards on top of Job L's headshot uploader; Paywall / Settings / Get Started say "exploration period" (never "free trial"), paywall verification copy = 24 hours, paywall-skip only while trial active/subscribed; `LandingContent` regained Job L's `skipToRealFamilies` + `ctaOverride` props (Speak With Us page) and a `footerInfo` default; `LoginPage` passes `footerInfo` (was crashing `/login`).
- **Plain-English security copy** (founder-approved wording) replaces "AES-256 / zero-knowledge / bank-grade / air-gapped" on: hero badges (`heroCopy.js`), pillar 02/03, SECURITY_ITEMS, TRUST_ITEMS, About values, Signup/Login/Partner/AcceptInvitation badges, Settings + Beneficiary Settings footers, Paywall footer, Get Started tiles, Start/Pricing trust footers, HomePage meta/JSON-LD, Dashboard/Checklist EGA progress note, BEC tooltip, Privacy §5 (plain sentence prepended; xAI disclosure kept). `/security` page intentionally keeps the technical, honest wording.
- **Business claims**: "continuity escrow" → "our written Wind-Down Promise" + link (FAQ 1 & 2, `faq-link-N`); About: "24/7 platform support" → "Human support from the founder, in-app", "nationwide network… Three teams" paragraph rewritten, CTA → `/start`; "iOS and Android" → "Works on any phone — add it to your home screen and get alerts"; "Family plan savings" no longer asserts a discount; StartPage "Unlimited storage" → "Secure document vault".
- **/pricing Compare Plans** now reads `tier_features` (portal gates) via `FEATURE_ROWS` + `isEnabled`; hardcoded "Up to 3/5", "Basic/Expanded/Unlimited vault", "Priority human support" rows removed; helper text aligned (Standard adds EGA; Premium unlocks everything).
- **Preview DB mirrors LIVE** via new `backend/scripts/mirror_live_config.py` (`--check` = drift report, exit 1 on drift): beta_mode false, family discounts 30/50, live prices + launch/final prices, beneficiary prices, all feature gates. `info@carryon.us` role reset to `benefactor` (was stale admin).
- **Backend fixes found by the gate**: `services/email.py` regained `is_valid_email` (quiz/testimonials); `ccp_depth.py` imports `SENDER_EMAIL` from `config` (ruff --fix strips re-exports from `utils`); `readiness_quiz_results` added to erasure manifest (USER_UNLINK by email, handled in `services/erasure.py`); dead unmounted `routes/resend_webhooks.py` removed and its orphan preview collections dropped; `scripts/check_tests_fast.py` skips gitignored/absent test files instead of aborting; `scripts/pitch_smoke.sh` checkout uses a valid origin (beta_mode is now false).
- **Dependency hygiene**: cryptography 46.0.7→50.0.1, pyOpenSSL→26.4.0, click→8.3.3, react-router-dom→7.18.4, yarn `resolutions` for js-yaml/browserslist/ws/shell-quote/websocket-driver/form-data/node-forge (frontend vulns 167→56, high 76→1, critical 4→0). Backend residual 32 = litellm 1.80.0 (pinned by emergentintegrations wheel) + ecdsa (no fix). Baseline updated.
- **Decision surfaced (not changed)**: the homepage body is Job F's Sep-2026 narrative (problem → quiz → eight tools → trust → FAQ). Job L's Jun-2026 "continuity timeline / four pillars / Voices" sections are no longer on `/` or `/home` (Voices remains at `/voices`). Founder to confirm or ask for a blend.
- **Ship order (founder)**: Save to GitHub → main (force, from THIS chat) → CI SOC2 gate fires Render Deploy Hook → verify `/api/health` build string + `/api/quiz/results` 200 → Republish frontend → enable branch protection (no force-push). Never use the old Job L chat again.



## Sep 2, 2026 — Tier-1 dependency bump + Erasure Phase 1 core (iteration_191) — VERIFIED, awaiting founder push approval

Founder decisions D1–D8 received (14-day frozen cooling-off; beneficiary self-erasure = unlink + Art. 21 route; delete sender-side delivered messages; estates read-only at T0; delete support transcripts; user-level security/access logs 3 y (attorney review at Phase 18), audit_trail 7 y; notify partner manager with minimum detail; erasure proceeds with active Apple IAP). Production diagnostic: deletion_requests total=0; OTPs 39 rows plaintext with string created_at; transcript encryption verified 24/24 + 1/1, zero plaintext.

- **Tier 1 bump** (`requirements.txt` only, four lines): pypdf 6.11.0→6.16.1, pillow 12.2.0→12.3.0, python-multipart 0.0.27→0.0.31, starlette 1.0.1→1.3.1 (fastapi 0.136.1 allows `starlette>=0.46`). pip-audit **82 → 36**; every advisory in the four bumped packages gone. Remaining 36: litellm 12 (transitive via emergentintegrations), aiohttp 11, cryptography 4, pyasn1 4, httplib2 2, click/ecdsa/msgpack 1 each. Gate: `tests/test_tier1_dependency_gate.py` 8/8 (multipart PDF upload round-trip, garbage-PDF never 500s, estate-binder pypdf assembly, share-card Pillow render, starlette 1.3 path validation), fast suite 90/90, check.sh ALL CLEAR, testing agent iteration_191 100%/100%. `.dep_security_baseline.json` re-baselined: backend 79→36; **frontend recorded at its true current 139 (high 66)** — the previously committed lockfile under-audited; 47 unique advisories, 32 via react-scripts toolchain, runtime-reachable: react-router-dom (12, high) and pdfjs-dist (high, parses user PDFs client-side) → frontend Tier 1 for the dependency plan.
- **Erasure Phase 1 core**: `services/erasure_manifest.py` (ESTATE_DELETE 59 · USER_DELETE 58 · via-parent 3 · UNLINK 5 · ANONYMISE 14 · RETAIN 17 · NO_PII 45; PERSONAL_KEYS drift keys) and `services/erasure.py` (`erase_estate`, `erase_user` → estates → via-parent children → user-keyed + blobs + chunk buffers → unlink (D2 `beneficiaries.status=account_deleted`, `$pull` memberships) → anonymise → revoke tokens → storage → users → PII-free `erasure_receipts` + compliance audit_trail entry). Wired behind admin user-delete (returns receipt_id + counts), benefactor `DELETE /estates/{id}`, ghost-estate replace, admin estate-health single/bulk delete and orphan cleanup. Phantom `activity_logs` cascade fixed by construction (manifest lists `activity_log`). **Drift guard** `tests/regression/test_erasure_manifest_drift.py` (tracked → runs in CI; live-DB check skips when Mongo unreachable) added to the fast suite: any collection written with a personal key or present in the live DB without a manifest entry fails the push. Scratch-DB end-to-end `tests/test_erasure_service.py` 6/6 (delete/unlink/anonymise/retain/receipt/idempotent).
- **Trial reminders**: `_undeliverable()` marks RFC-2606 reserved-domain accounts as sent instead of failing at Resend on every scheduler run (root cause of the recurring housekeeping WARN).
- **TTL audit (preview)**: working — audit_trail.stored_at (130 legacy rows missing the field never expire), funnel_events, llm_cost_ledger.created_at_ttl, rate_limits, scheduler_locks, section_unlock_sessions, token_blacklist.expires_at, binder_shares/download_tokens/message_download_tokens/download_events (Date writers). **Never fire**: otps.created_at (string), referral_visits.created_at (string), token_blacklist.blacklisted_at (string; dead duplicate index — expires_at does the work), otp_trust.expires_at (written as isoformat string). **Index never created**: idempotency_keys (middleware create_index not run at startup), subscription_intents (lazy path). **No TTL at all**: failed_logins, webauthn_challenges, client_errors (857 rows, IP+UA), funnel_sessions (IP+demographics), chunked_uploads, token_revocations. → Step 3 hygiene pass.
- **Stripe SDK assessment**: `emergentintegrations` is used only in `routes/subscriptions/checkout.py` for `StripeCheckout.create_checkout_session` / `get_checkout_status` — thin wrappers over `stripe.checkout.Session.create/retrieve`. The official `stripe` SDK (14.3.0) is already a direct dependency and already used directly in `founders_circle.py` and inside checkout.py itself. Replacement ≈ 40 lines mapping the same fields (status, payment_status, amount_total, currency, metadata) + removing `emergentintegrations==0.1.2` drops litellm, google-genai, google-generativeai, openai as transitive deps (−12 pip-audit advisories at least). Touches LIVE REVENUE PATHS → founder-authorised task with Stripe test-mode checkout + webhook verification.
- Not yet: Step 3 hygiene (OTP hashing + Date TTLs + client_errors/funnel_sessions/chunked_uploads/token_revocations TTLs + idempotency index at startup), Phase 2 (self-service flow, 14-day pending state, scheduler, emails, remove accepted-beneficiary blocker, wire deletion_requests), Phase 3 (admin page, erase-by-email, copy, generated retention policy).


## Sep 1, 2026 — Transcript deletion finality (iteration_190) — VERIFIED, ready to push

Founder ran the read-only production diagnostic: 56 unresolvable plaintext EGA rows = 42 rows / 11 deleted users (visible to nobody) + 10 rows petemitchell (demo, all resolvable via cost ledger) + 4 rows founder@carryon.us owning 0 estates (one an explicit delete-validation test). No third-party user. Decisions built exactly as given:

- **`scripts/migrate_encrypt_transcripts.py`** — `--backfill` now resolves sole-owner **then guardian `llm_cost_ledger` ±180 s** (exactly one entry, estate still owned; document-name matching deliberately NOT added; session-sibling / only-estate-at-time NOT added — a session can outlive an estate switch and the time method is blind to deleted estates, so neither is as deterministic as the ledger). New **separate `--delete-orphans` flag** (implies --backfill, still dry-run without `--apply`): user_id missing / user no longer exists / user owns no estates (EGA only) / estate no longer exists. Every row is LISTED (reason | email or DELETED id | session | timestamp | role | char count — never content) BEFORE any delete; deletion only inside `if apply:`. Dry-run `_salt` is now a read-only estate lookup (previously get_estate_salt could persist a salt for a legacy estate even in dry-run). Apply mode prints the remaining plaintext count.
- **`services/transcript_purge.py`** (new) — `purge_estate_transcripts`, `purge_user_transcripts` ($or user_id / estate_id $in) over `chat_history` + `beneficiary_concierge_messages`. Wired into admin user-delete (user-keyed section, runs even with 0 estates; old estate-only chat_history delete removed), benefactor `DELETE /estates/{id}`, ghost-estate delete, admin estate_health single/bulk delete, and cleanup-orphans (by orphan estate ids + orphan owner ids).
- **Deletion-finality audit (reported, NOT fixed — founder decision):** 30+ estate-scoped content collections are not cascaded on estate delete and ~60 user-keyed collections are not cascaded on admin user delete (see the founder report of Sep 1 / PRD backlog). Highest-exposure: `digital_wallet` (DAV secrets), `financial_accounts`/`bills`/`debts`/`property_assets` (CFP), `cfp_entities`+relationships/external_people (CES), `emergency_plans`/`ccp_risk_profile` (CCP), `ffn_contacts`, `estate_channels`/`estate_messages` (ECT), `milestone_deliveries`, `ega_tasks`, `binder_shares` (public share tokens), `cached_pdfs`/`latest_pdfs`, `beneficiary_estates`, `family_connections`, `edit_history`, `readiness_history`, `quickstart_progress`, `support_conversations`, `notifications`, `webauthn_challenges`, `otp_codes`/`otps`, `payment_transactions`, `referral_codes`, `tier_verifications`, `user_preferences`/`notification_preferences`/`digest_preferences`. Audit/consent logs (`security_audit_log`, `phi_access_log`, `consent_audit_log`, `sensitive_access_log`, `activity_log`, `token_blacklist/revocations`) are retention-by-design and should stay. No self-service account deletion endpoint exists.
- **Fast-suite hygiene:** `tests/test_idor_guards.py` attacker fixture now deletes its throwaway account on teardown (via the admin cascade); 171 leaked `idor-test-*@example.com` preview accounts purged through the same endpoint — this was the source of the daily trial-reminder Resend errors behind the housekeeping "recent backend logs" WARN (now PASS).
- **Tests:** `tests/test_transcript_orphan_purge.py` 6/6 (scratch-DB end-to-end incl. --apply; cascades via API). Testing agent iteration_190: backend 100% (6/6 new + 47/47 existing + fast 81/81; preview dry-run read-only verified by before/after counts), frontend 100% (one transient dev-server ChunkLoadError during rapid nav, not a code path). `check.sh` ALL CLEAR. Housekeeping: DS pip-audit 79→82 is CVE-feed drift on an untouched requirements.txt (pypdf/pillow/litellm advisories) — per Jun 2026 precedent, separate dependency-bump task, not bundled here.
- **Render sequence for the founder:** `python scripts/migrate_encrypt_transcripts.py --backfill --delete-orphans` (dry) → review the 46 listed rows → same command `--apply` → expect "Plaintext transcript rows remaining: 0". Ledger TTL is 180 days (`created_at_ttl`), so the 10 petemitchell rows are only resolvable while their ledger entries survive — run promptly.
- **Still open for founder:** retire the `/security` "AI chat transcripts … are currently stored unencrypted" sentence once the Render run reports 0 (proposed wording in the Sep 1 report). Test files under `backend/tests/test_*.py` are gitignored by design (credentials) — they stay preview-local.
...
## Aug 31, 2026 — B2 export protections built + trial-expired email line (NOT pushed; B1/B3 held)
- NEW backend/routes/export_stepup.py: POST /compliance/export/step-up (password → passkey REQUIRED if enrolled, else email OTP, SMS only if sole factor) + POST /compliance/data-export (verifies+consumes proof, streams export with no-store/attachment headers + "sensitivity" field). Old GET endpoint removed; compliance.py export body refactored into build_user_export() (content unchanged — B1/B3 fields NOT added yet).
- Protections: per-user 5/24h Mongo rate limits on both endpoints; OTP sha256-hashed, purpose-bound, 10-min TTL, single-use, 5 attempts; passkey challenge purpose-scoped (export_stepup), single-use, 10-min freshness; NO-DOWNGRADE enforced (valid OTP → 401 while passkey enrolled — proven E2E with planted cred). Hash-chained audit events (issued/denied/rate_limited/data_export w/ step-up method, per-category counts, IP, UA) + user activity_log row + "your data was exported" notification email. Sentry before_send scrub drops body/headers/cookies for both endpoints (config.py).
- Frontend PrivacyCard: 3-stage modal (plaintext warning + ack checkbox → password → OTP), passkey branch auto-invokes getPasskeyAssertion (new helper in services/passkey.js). testids: export-stepup-modal, export-ack-checkbox, export-warning-continue, export-password-input/submit, export-otp-input/submit.
- Trial-expired email: added approved line "Your complete data export — download every document, message, and note anytime from Settings → Privacy" to the "What's still accessible" box (trial_reminders.py).
- BUG FOUND while testing: webauthn lib requires PublicKeyCredentialDescriptor structs; plain dicts crash options_to_json with a 500. Fixed in export_stepup.py. SAME LATENT BUG EXISTS IN routes/webauthn.py login-options (~line 168) — reachable in prod when a passkey user types their email first (LoginPage.js:529 passes email). FLAGGED to founder, not fixed (login code out of B2 scope).
- Tests updated: test_iter168_regression.py (step-up flow w/ DB-injected OTP, uses e2e account so notification emails never hit info@), test_regression_comprehensive.py + test_ega_encryption_regression.py (401 without proof, 405 on GET). All E2E verified on preview: 401/405/429 paths, wrong-OTP attempts, replay refused, headers, audit rows. check.sh ALL CLEAR. NOT PUSHED — awaiting B1/B3 approval to ship together.

## Aug 31, 2026 — Production verification of B2 deploy (founder pushed)
- (1) Passkey login-options FIXED on prod: info@, founder@, barnetharris@ all have enrolled passkeys and return 200 + allowCredentials:1 (this exact call was the guaranteed 500). Full device ceremony = founder tap.
- (2) GET /api/compliance/data-export → 405; unauth POST → 401.
- (3) Step-up on prod as petemitchell: wrong password 401; correct password → method:passkey with real WebAuthn options (Pete HAS a prod passkey); OTP redemption attempt → 401 "Passkey verification required" — NO-DOWNGRADE PROVEN ON PRODUCTION. Final real-export download needs founder's device (passkey tap in Settings → Privacy) or Render OTP injection for megumiharris (no passkey). Rate budget used today: Pete step-up 2/5, export 1/5.
- (4) Frontend deployed in lockstep: chunk 506.78686354 POSTs both endpoints; ZERO stale GET callers across all 131 deployed JS files; www.carryon.us and app.carryon.us serve identical build main.b4096ca7.js.
- NEXT: B1/B3 field-inclusion build as its own unit + full testing-agent regression before push. Register-options struct twin bug still awaiting approval.

## Jun 2026 — Backlog Sections N–Q (public site CTA / performance / homepage IA / final verification)
Founder-approved completion of the A–Q public-site backlog (A–M shipped in the prior fork session).
- **N — CTA consistency**: primary signup CTA standardized to "Start your family's plan" in LoginPage nav (was "Open Account"), AboutPage final CTA + HomePage hero (were "Get Started"), VoicesPage header (was "Start Free") + footer (was "Start your CarryOn", superseded positioning). Left intentionally: dashboard onboarding "Let's Get Started" (not a signup CTA), /get-started page title, LandingPage (archived /landing-consumer) trial CTAs, "Create Account" secondary links beside sign-in forms, /speak-with-us "Book a Demo".
- **O — Performance & assets**:
  - `components/YouTubeFacade.js` (new): click-to-load facade (thumbnail + gold play button, maxres→hq fallback, iframe with autoplay injected on click, testId → `${testId}-player`). Replaced all 6 public YouTube iframes (LoginPage ×2, HomePage ×2, SpeakWithUsPage ×2). Admin SiteContentTab previews and the LeadConnector booking iframe untouched by design.
  - `components/FlagBackdrop.js` (new): shared `<picture>` — webp srcset (flag-bg-800/1280.webp, 12/22 KB) + jpg fallback, loading=lazy. Replaced all 14 `<img src="/flag-bg.jpg">` usages across 10 pages. flag-bg.jpg recompressed in place 1.16 MB → 63 KB (was saved at max quality). index.html boot-splash img wrapped in same `<picture>`; preload gained imagesrcset/imagesizes.
  - `public/og-image.jpg` (new, 1200×630, 67 KB): PIL-composed branded share card (logo + "The Family Continuity Platform" in Cormorant Garamond + tagline). og:image + twitter:image now point to it (old carryon-icon.jpg was 780×881 vs declared 1200×630).
  - `.well-known/security.txt`: removed dead references (pgp-key.txt Encryption line, /security/hall-of-fame Acknowledgments), contact confirmed security@carryon.us (founder choice), URLs normalized to www, Expires 2027-06-01.
- **P — Homepage IA** (founder approved structure before build): Five Steps section moved ABOVE Four Pillars (new z-[29]; conditional split so it also renders on skipToRealFamilies pages). Each pillar card's function list collapsed behind Radix accordion "See what's inside (N systems)" (testids pillar-0{1..4}-systems-toggle{suffix}), collapsed by default, keyboard accessible, pillar-colored trigger. BEC stays in Total Family Continuity end tile. No copy/name/order changes to pillars themselves.
- **Q — Final verification**: production build green after each section (N/O/P). testing agent iteration_184: 12/12 PASS, 100% frontend. check.sh ALL CLEAR (0 WARN/0 FAIL, 81 tests). Greps (full-count wc -l, src only, build ignored): 0 @carryon.com, 0 "Open Account", 0 eager public YouTube embeds, 0 direct flag-bg img tags. og-image ×2 in built index.html; security.txt in build; app.carryon.us X-Robots-Tag noindex intact. Only console noise: Firebase Installations 403 on preview host (allowlist, env-only).
- Informational (not fixed, for founder awareness): homepage nav CTA is a <button> (works, but <a href> would allow open-in-new-tab); Firebase preview 403 could be silenced by gating init on non-prod hosts.

## Jun 2026 — B1/B3 export field-inclusion + Wind-Down three-state rewrite (SHIPPED IN REPO, not pushed)
**B1/B3 (backend/routes/compliance.py build_user_export):**
- Per-estate salt map via services.encryption.get_estate_salt; reuses routes/messages._decrypt_message + routes/digital_wallet._decrypt_wallet_entry (no cycles).
- NOW INCLUDED (plaintext, behind the deployed B2 step-up gate): MM bodies+titles ("messages" key — RENAMED from "messages_metadata"), DAV secret values (password/additional_access; encrypted_* keys stripped), financial_picture{bills,bill_payments,debts,accounts,property_assets,custom_categories}, entities_structures{entities,external_people,relationships}, contingency_protocols{emergency_plans,activations,member_checkins,ccp_plans,household,go_bag,rendezvous,out_of_area,risk_profile,ccp_activations,drill_runs}, friends_family_notification (ffn_contacts), estate_plan_timeline (edit_history). Wills = SDV document files, still excluded by design (individually downloadable) — note field says so.
- export_stepup.py audit counts now nest per-category dicts. Note field rewritten (plaintext warning).
- Tests: test_iter168_regression TestGDPRExportShape → 4 tests w/ _fetch_export_cached (exports are 5/24h rate-limited — db.rate_limits keys ^export.*<user_id>; fetch ONCE per run). test_gdpr_compliance test_data_export → GET 405 + bare POST 401. Testing agent added tests/test_iter185_export_regression.py (11 tests).
- VERIFIED: main-agent live probe 21/21 (plaintext round-trip for seeded DAV+MM, all keys, zero ciphertext/hash leaks, nested audit counts in db.audit_trail); testing agent iteration_185 backend 17/17 + frontend 16/16, check.sh ALL CLEAR. e2e login now needs force_login:true (single-session guard).
**Wind-down (frontend/src/pages/WindDownPromisePage.js, FOUNDER-APPROVED copy + option (a)):**
- Three states: State 1 today (verified export surfaces only: step-up JSON export incl. DAV secret values, per-doc original-format downloads, MM media, Estate Binder PDFs — IAC/CCP/CFP-handoff/EGA plan+transcript/Emergency Card/Family Readiness Report, privacy@ mailto), State 2 wind-down window (90 days notice, features stay, export paths stay open, FC concierge, acquirer/successor clauses), State 3 after last day (everything readable forever offline).
- FALSE CLAIMS REMOVED: "single ZIP of everything" (founder decided ZIP not built), "IAC as CSV", decryption-CLI-for-encrypted-ZIP card → replaced with approved option (a): "No proprietary formats — ever… if we ever offer encrypted archive downloads, we commit to publishing an open-source decryption tool on GitHub at the same time."
...
## Jun 2026 — Founder Headshot self-service upload (Option 2, founder pick)
- Backend: POST/DELETE /api/admin/site-content/founder-headshot in routes/admin/platform.py (require_admin + founder-scope router; validates JPG/PNG/WebP ≤8MB, EXIF-transpose, center-crop square, resize 512px, JPEG q88 → Mongo db.site_assets _id "founder_headshot"). Public GET /api/public/founder-headshot in routes/public_content.py (404 until uploaded; Cache-Control max-age=300 + ETag).
- Frontend: SiteContentTab.js "Founder Headshot" card (preview/upload/replace/remove, testids founder-headshot-{preview,empty,input,upload-btn,remove-btn,view-about}); AboutPage.js Founder section renders the photo (data-testid founder-headshot, circular, gold ring) with onError fallback to the "coming soon" placeholder — this also removed the LAST <<<>>> comment in the repo.
- VERIFIED: curl (404→upload 200 512×512→serve 200; no-auth 401; bad-type 400), About page screenshot, testing agent iteration_186 7/7 (full remove→placeholder→upload→photo cycle + sibling cards regression). A TEST IMAGE is uploaded on PREVIEW db; PRODUCTION has none → prod /about shows placeholder until founder uploads via portal after deploy.
- Debugging note: /admin/site-content stalled on the AdminPage bootstrap skeleton in 4 main-agent screenshot runs with a "Back online — syncing" SW banner; backend endpoints all <0.1s; testing agent could NOT reproduce (tab mounted normally) → automation-browser service-worker offline-queue flakiness, not an app bug. If founder ever reports admin stuck on skeleton: suspect SW offline queue, hard reload.
- founder@carryon.us / CarryOntheWisdom! CONFIRMED VALID on PREVIEW pod (stale on production only).

## Jun 2026 — PROD BUG FIX: partner portal "Failed to create client portal" (E11000)
- Exhibit: Carpenter Collective partner portal (prod) — New Client Portal → generic red toast. ROOT CAUSE: db.users unique+SPARSE index on username_lower treats "" as a real value (sparse only skips MISSING fields); provision_client_portal wrote username/username_lower as "" for every white-glove client, so once ONE pending-claim client existed anywhere (prod: Christopher Strobel, awaiting claim since 8/24), EVERY subsequent client creation platform-wide crashed with DuplicateKeyError → 500 → generic toast. Reproduced on preview (same precondition existed) with jazmine-manager: POST /manager/clients → 500, E11000 in backend.err.log.
- FIX (2 files): routes/pro_clients.py provision_client_portal now OMITS username fields entirely (client picks one at claim — claim flow $sets it, unaffected); db_indexes.py startup repair `update_many({"username_lower": ""}, {"$unset": {username_lower, username}})` runs before index creation, idempotent — cleans prod's existing docs automatically on next Render deploy.
- VERIFIED: two consecutive provisions 200+200 (previously #2 always failed), duplicate email → clean 409 "A CarryOn account already exists for that email" (frontend toast surfaces detail), stored docs have no username fields, 0 users left with "". Permanent regression test tests/test_iter187_partner_provision.py (sync pymongo for DB asserts — motor+asyncio.run double-loop crashes; cleans up its own rows + decrements times_used). check.sh ALL CLEAR. Preview partner times_used restored to 12 after test cleanup.
- SHIPS ON NEXT PUSH: fix + auto-repair deploy together; partner can then create Sojourner Aheebee's portal normally.

## Jun 2026 — xAI billing/resilience investigation (READ-ONLY, no changes)
- Key probe GET api.x.ai/v1/api-key: name "EstateGuardianAI", team 605d0ed3-5f8e-47c9-995d-51f599e073f4, user 6e8ef391-..., wildcard model/endpoint ACLs, team_blocked=false (balance not exhausted; flag is a pollable exhaustion signal). Prepaid balance requires a MANAGEMENT key (management-api.x.ai /v1/billing/teams/{id}/prepaid/balance → 401 with inference key) — only visible in the inaccessible account's console.
- Prod Mongo unreachable from pod; PREVIEW xai_usage: 192 calls (grok-4 23/$0.91, grok-3-mini 169/$0.12), last-30d 52 calls/$0.045. Prod numbers available via admin ops dashboards + GET /admin/llm-cost-summary. Preview shares the SAME key → also bills that team.
- NO 402/insufficient-credits handling anywhere; exhaustion degrades gracefully: EGA → 500 "AI service temporarily unavailable" toast (model ladder ×2 each first); BEC → synthesized document-aware answer, no visible error. NO alerting for xAI errors or spend (dashboards only; ai_burn_guard per-user caps toggled OFF; EGA per-user 500K tokens/day 429).
- Migration = swap XAI_API_KEY env on Render (team id nowhere in code, ledgers key-agnostic). Suggested (NOT built): poll team_blocked for early warning.

## Jun 2026 — xAI silent model redirects: cost tracking fixed, explicit models, historical reprice
- CONFIRMED founder's finding + worse: probes show grok-4, grok-3 AND grok-3-mini ALL served by grok-4.3 (response.model), reasoning tokens burned even on "Say OK" (76–108) and xAI's usage.completion_tokens EXCLUDES reasoning tokens (billed at output rate) → we also under-recorded billable output everywhere.
- Real pricing decoded from GET /v1/language-models (units $0.0001/1M): grok-4.3 & all grok-4.20 variants $1.25 in/$2.50 out ($0.20 cached), grok-4.5/4.6 $2/$6, grok-build-0.1 $1/$2.
- Errors found: guardian.py hardcoded $3/$15 (grok-4) + $0.20/$0.50 (else); llm_cost_ledger PRICING was retired-model prices; both keyed on REQUESTED model.
- FIXES: llm_cost_ledger PRICING rewritten (retired names now priced at 4.3 redirect rates, unknown→4.3); _extract_usage returns (prompt, completion, reasoning); record_llm_call/record_xai_response/track_llm_call carry served_model (response.model) + reasoning_tokens, cost on served model with completion+reasoning — fixes all 7 ledger call sites at once; guardian.py xai_usage block uses estimate_cost + stores served_model/reasoning_tokens.
- Explicit models: config defaults XAI_MODEL=grok-4.3, XAI_MODEL_LIGHT=grok-4.20-0309-non-reasoning (non-reasoning light path, founder-requested flag). Hardcoded "grok-3"/"grok-3-mini" rungs replaced with grok-4.20-0309-reasoning / XAI_MODEL_LIGHT in guardian.py ladders, connected_protocol.py, ccp_depth.py, beneficiary_concierge.py (ladder + diagnose probe list).
- scripts/recompute_xai_costs.py (dry-run default, --apply): re-prices retired-name records at 4.3 rates, marks served_model_assumed/repriced_jun2026, skips post-fix records. PREVIEW applied: xai_usage $1.0283→$0.9797, llm_cost_ledger $0.9340→$0.6900 (LOWER BOUND — historical reasoning tokens unrecorded). FOUNDER: run on Render for prod (dry then --apply).
- VERIFIED: live EGA chat on new models 200 in 4.6s, substantive persona-correct answer; new ledger row served_model==requested (no redirect on explicit names), reasoning_tokens=0 on non-reasoning, cost math exact. BEC diagnose (admin) probes all 3 ladder models OK (0.52s/2.06s/1.6s). tests/test_llm_cost_ledger.py updated for new pricing/3-tuple (7/7 pass). check.sh ALL CLEAR.
- DEPLOY NOTES: check Render env for XAI_MODEL/XAI_MODEL_LIGHT overrides (retired names there would override new defaults); run recompute script on Render. BEC-fallback wording + xAI alerting from earlier proposal still awaiting founder approval (Parts 2/3).

## Jun 2026 — ZDR disclosure + preview key migrated to founder-controlled team
...
## Sep 1, 2026 — AI Alerting toggle + Integrations automation layer (founder Option A)
- AI Alerting on/off toggle (Radix Switch, `ai-alerting-toggle`) in the AI Alerting card: OFF pauses daily job + all alert emails (run-now still shows results, sends nothing); persists via xai_alerting_enabled in platform_settings. Verified E2E.
- Integrations automation (services/integration_health.py): ENV_BINDINGS env-aware tiles (auto missing_config status + 'key missing' chips), verify_all() 9 live checks (mongo ping, xAI key, Stripe balance, Resend empty-POST auth probe [sending-only keys 401 GETs — 422 = valid], Twilio account, S3 head_bucket, Render self, Vercel URL, Sentry DSN presence) stored in db.integration_health, nightly integration_verify_scheduler, drift detection (untracked credential env vars + missing tile keys → warnings bar), cost_stale >90d flag cleared by Edit→Save (cost_reviewed_at).
- New tiles: Sentry (missing_config until SENTRY_DSN set) + GitHub. xAI tile details refreshed (grok-4.3 models, ZDR, $1.25/$2.50 pricing). Railway: no tile existed; removed dead RAILWAY_ENVIRONMENT fallback in config.py; suggestions text Railway→Render.
- Removed unused EMERGENT_LLM_KEY from backend/.env + security_scan rotation list.
- UI: 'Verify all' button + '8 live / 1 failing' summary chips + per-tile LIVE ✓ / CHECK FAILED / key missing / cost review due chips.
- Housekeeping fixes: BuildTag || fallback pattern, 2× text-[10px] → 11px (headshot placeholders), YouTubeFacade hardcoded #0b1221 → var(--bg2), projection scanner now honors allow-missing-id comments (+2 sites annotated).
- PREVIEW DB STATE RESTORED after accidental sidebar-OTP toggle during automation: platform otp_disabled=True (login OTP globally off on preview — original state), all 882 preview users otp_enabled=False. NEVER click sidebar OTP toggles in automation.
- check.sh ALL CLEAR; testing agent iteration_188: backend 100% (6/6 new + 6/6 BEC regression), frontend 100%. Regression file: tests/test_iter188_integrations.py.

## Sep 8, 2026 — `fix/tier-maps` (Push #2): B1–B11 + C1–C10 tier-parity defects closed, zero xfails
- Branch `fix/tier-maps` off `5100cd9a`, 2 commits: `4e34d9e1` (defects encoded as strict xfails + frozen benefactor baseline) → `4e3c6fd0` (fixes). NOT PUSHED — founder pushes.
- **Billing (B5 = C1+C2+C3+C5, C4):** `plans.plan_lookup(settings)` (plans + beneficiary_plans) and `plans.cycle_total(plan, cycle, discount)` now drive `/subscriptions/checkout` (live + beta), `/change-plan`, `/change-billing`. Pre-fix every `ben_*` plan_id was 400/404 on the web; change-plan mixed `ben_price` with the benefactor's quarterly/annual price and dropped `custom_discount` on quarterly/annual. Rule now: full-period amount from the plan's own cycle prices, THEN the per-user discount (same as checkout always did). Benefactors without a discount are byte-identical to `tests/regression/fixtures/benefactor_billing_baseline.json` (generated pre-fix, asserted per tier).
- **B6** status.py DOB eligibility adds `seniors` at 65+. **B7** new_adult age-out → `standard` (was `ben_standard`). **B9** family preview queries estates by `owner_id` (was `user_id` → FPO-only tree); beneficiary rows priced from `ben_<tier>`; unknown sub plan → 400. **B11** trial emails quote `starting_monthly_price(settings)` (lowest paid no-verification tier, 7.99 today).
- **B1/B3/B4** seniors in `TIER_IDS` (new Feature Gates column, default ON like every tier), admin `valid_tiers`, FeatureGatesCard, UsersTab, SubscriptionManagement TIER_STYLES (+ ben_seniors/ben_new_adult/ben_enterprise). **B2** `ben_new_adult_{monthly,quarterly,annual}` in APPLE_TO_PLAN + iap.js (App Store Connect products still to be created by founder). **B10** paywall family tile copy rendered from `family_benefactor_discount_percent` / `family_beneficiary_discount_percent`.
- **C7** beneficiary price line reads `beneficiary_plans[ben_<tier>]` per cycle (`beneficiaryMonthlyPrice` helper) — flat-rate tiers show 1.99 on every cycle. **C8** admin beneficiary price edit keeps flat-rate plans flat; **C8b (found live)** stored `beneficiary_plans` on preview carried 1.99/1.79/1.59 for ben_military/veteran/new_adult — `get_subscription_settings()` now merges missing keys and heals cycle prices (flat → one price; toggle → ×0.9/×0.8), persisted on first load. Preview healed; production heals on its first `/subscriptions/plans` call after deploy. **C9** `get_enabled_features_for_tier`: unknown tier → nothing (matches `is_feature_enabled_for_user`), `ben_<tier>` inherits `<tier>`. **C10** Apple activation stores the catalog plan name (was `plan_id.title()` → "Ben Premium").
- Tests: `test_tier_price_parity.py` 328 passed / 0 xfail; check.sh ALL CLEAR (fast suite 428). Housekeeping advisory: pip-audit ratchet 36→38 (advisory-DB drift on aiohttp/cryptography/litellm/pyasn1/msgpack/httplib2 — no dependency changed on this branch; belongs to the Tier-2 dependency-bump task).
- Observations (not changed): `check_dob_subscription_events` scans `role: "beneficiary"` users only, so a benefactor on new_adult never ages out; MRR in `get_subscription_stats` counts benefactor plans only; `SubscriptionManagement` keeps its own portal-based `isBeneficiary`.

## Sep 8, 2026 — Push #2 verified live · `feat/founder-pricing-rules` (Push #3 candidate, NOT PUSHED)
- **Push #2 (`fix/tier-maps`) confirmed on production** via public `GET /api/subscriptions/plans`: `tier_features` has `seniors`; stored beneficiary rows healed exactly as predicted (ben_premium 6.99 → 6.29/5.59, ben_standard 5.99 → 5.39/4.79, ben_military/veteran 3.99 flat). Seniors stays 12.99 on prod — founder-set price, founder decides in the portal (no code change). Read-only seniors charge check on preview: 0 subs / 0 txns; production counts to be run by founder in Render shell (commands in chat).
- **C8b integrity test** (`1da7566b`): heal touches only beneficiary `quarterly_price` / `annual_price` (+ missing derived keys); family discount %, per-member `original_price/family_price/discount`, `custom_discount`, and every subscription amount byte-identical (snapshot before/after in runner).
- **Scope #3 core (`d07c5c7a`)** — founder-defined cycle discounts: every plan carries `quarterly_discount_percent` / `annual_discount_percent`; `plans.cycle_fields()` derives `quarterly_price`, `annual_price`, `allows_billing_toggle` (0/0 = flat). New `PUT /admin/plans/{plan_id}/pricing` (JSON: price?, quarterly_discount_percent?, annual_discount_percent?) for benefactor AND ben_* ids; legacy `/price` form endpoints delegate. Hardcoded ×0.9/×0.8 removed from admin.py, plans.py, SubscriptionPaywall.js, SubscriptionManagement.js (static tests guard). Portal: `PlanPricingRow` (price + two percents) in both pricing cards. Catalog seeds 10/20 (flat ben tiers 0/0) → all charges unchanged; frozen benefactor baseline still passes. Production first load will ADD the percent fields (+ `allows_billing_toggle` on benefactor rows) to stored plans; no price or cycle value changes.
- Parity suite: 350 passed / 0 xfail; check.sh ALL CLEAR (fast suite 450). pip-audit advisory drift 36→38 unchanged (dependency-bump task).
- Still hardcoded (Scope #3 part b, awaiting founder decision): age windows (18–25 / 65+ / age-out 26), verification requirement + doc types per tier, 30-day grace period, proration rule, plan names/notes/features/order.

## Sep 8, 2026 — `feat/founder-pricing-rules` part (b) (`a7d3f194`) — every plan rule founder-owned · NOT PUSHED
- **Plan fields now editable in Admin → Finance → Subs → Details:** name, note, feature bullets, `requires_verification` + `verification_docs`, and for benefactor plans `age_min` / `age_max` / `age_out_plan_id` (new_adult seeds 18/25/standard, seniors 65/open). `PUT /admin/plans/{plan_id}` (replaces the short-lived `/pricing` path; legacy `/price` form endpoints delegate). `PUT /admin/billing-rules`: `grace_period_days` (30), `proration_enabled` (true), `plan_order` (arrows on rows). `get_subscription_settings()` seeds missing rule keys, no longer force-syncs `features` from code, and sorts by `plan_order`.
- **Backend consumers:** `plans.age_eligible_plan_ids()` drives `/subscriptions/status.eligible_tiers` and registration `eligible_tier`; `_age_out_subscribers()` moves anyone past a plan's `age_max` to its `age_out_plan_id` (benefactors included; no 7-day window); verification upload accepts any plan with `requires_verification`; grace period read from settings in `verification_and_lifecycle` and `services/billing_lifecycle`; change-plan honours `proration_enabled`.
...
## Sep 10, 2026 — Production read-only beneficiary billing audit: CLOSED (0 exposure)
Founder ran `python scripts/readonly_ben_tier_billing_audit.py --quick` in the Render shell (prod, `srv-d86h62d7vvec73a7od00`): `user_subscriptions ben_* amount>0: 0` · `payment_transactions ben_* paid: 0` → no beneficiary was ever charged through web checkout before the B1–B11 parity fixes. No refunds owed. Shell output also confirms SOC2 controls armed on prod: `REDACT_PII=1`, `LOG_FORMAT=json`, Sentry env=production, Grok `grok-4.3`, Twilio configured.
Same day: Stripe webhook destination `we_1TNR6xGKXPelgYl93EGTCvmQ` was still pointed at the dead Railway host (`carryon-api-production.up.railway.app` → 404 "Application not found"); founder edited URL → `https://carryon-api-kacr.onrender.com/api/webhook/stripe`, signing secret confirmed matching Render `STRIPE_WEBHOOK_SECRET`; probe from preview returns `400 Invalid signature` (route live, HMAC enforced). Sentry uptime monitor on `/api/health/ready` (project python, 1 min, 10 s timeout) being created by founder; Resend rate-limit raise (10 → 50 req/s) requested via support.
**Sep 10 — deploy verified live on prod:** behavioural probe from preview — 6 concurrent wrong-password logins for `barnetharris` (bcrypt on the 2-thread pool) while sampling `/api/health/live`: idle p50 58 ms → during burst p50 60 ms, max 133 ms (old code would have blocked ~1.4 s). Login latencies staggered 646→1842 ms = pool draining as designed. Side effect: 6 `failed_logins` rows + 6 `login_failed` audit rows for barnetharris (auto-cleared on next successful login). Sentry uptime monitor created by founder. Launch prep complete.

## Sep 10, 2026 — Founder-metered "New Signup Alerts" (`feat/founder-pricing-rules`) · NOT PUSHED
Founder asked for a portal control to meter (or silence) the per-signup founder ping — "jackpot mode" at launch, dial down later.
- `services/signup_alerts.py`: modes `each` (default, unchanged behaviour) · `every_10/25/100` (milestone ping "Signup milestone: #N — latest name") · `hourly`/`daily` summary ("47 new signups in the last hour · N members total") · `off`. Every signup atomically `$inc`s `platform_settings.signup_alert_counter` regardless of mode. Digest window is claimed with a conditional update before sending (no double-send across pods/ticks); scheduler `signup_alert_digest` (60 s tick) registered in `server.py` + `scheduler_worker.py` under the Mongo lock → health now reports 15 schedulers.
- `routes/admin/platform.py`: `GET/PUT /api/admin/signup-alerts` (founder scope) → `{mode, counter, last_digest_at, modes, stats:{last_hour,today,total}}`; PUT validates mode (400 otherwise), stamps `last_digest_at` when entering a digest mode, audit `signup_alert_mode_change` (category platform).
- `routes/auth/register.py`: `notify.founder(...)` replaced by `signup_alerts.on_signup(...)`.
- Frontend `components/admin/SignupAlertMeter.js` rendered at top of Admin → **Notifications** (`NotificationCategoriesTab`): native 16 px `<select data-testid="signup-alert-mode-select">`, live chips (last hour / today / members total / counter), mode hint, sonner toasts. Runbook §0 row added.
- Tests: `tests/test_signup_alert_meter.py` (5 unit tests against preview DB, notify.founder monkeypatched, settings restored) · curl GET/PUT/400/401 · playwright select→hint. check.sh ALL CLEAR, housekeeping 0/0, fast suite 457.

## Sep 17, 2026 — SEO audit follow-through: `/vs` pages, `llms.txt`, unique meta, empty-`<title>` fix (`reconcile`) · NOT PUSHED
Founder's SEO audit: missing competitor comparison pages, shared meta titles on some public pages, empty `llms.txt`, `/home` in sitemap, ungrouped Dependabot PRs.
- **`/vs` hub + 3 comparison pages** — `pages/ComparePage.js` + `data/compareData.js`: `/vs`, `/vs/trustworthy`, `/vs/everplans`, `/vs/resolve-legacy`. Each is dated ("Checked September 16, 2026"), quotes the competitor's public site, lists what they do that we don't, side-by-side table, own `<SEO>` title/description/canonical.
- **Unique metadata** on `/customers`, `/changelog`, `/about`, `/pricing`, `/start`, `/security`, `/vs/*`. `/home` canonicalizes to `/` and is out of `public/sitemap.xml`; `/vs*` added to sitemap. `public/llms.txt` populated (positioning, pillars, public URLs, contact).
- **`.github/dependabot.yml`**: non-security updates grouped per ecosystem (one PR each) so the founder isn't flooded.
- **Bug — `document.title` empty on preview**: root cause is Emergent's *dev-only* visual-edits babel plugin (`plugins/visual-edits/babel-metadata-plugin.js::wrapDynamicExpressionChildren`) wrapping `<title>{title}</title>` in `<span style="display:contents">`; React 19 drops non-string `<title>` children. Production (Vercel build, no plugin) was never affected. Fix: `components/SEO.js` now renders the title via `React.createElement('title', …)` so the plugin skips it — verified `document.title` on `/vs/trustworthy`, `/vs`, `/security`, `/customers` (one `<title>` each, canonical correct).
- `housekeeping.sh` 7c: `ComparePage` joins the marketing-page exclusion set (hardcoded dark bg, same as `CustomersPage`/`ChangelogPage`). `yarn.lock` refreshed (committed lock was missing `@axe-core/playwright` already in `package.json`).
- Note: `/features` is **not a route** — public nav uses the `#features` anchor on `/`; a direct hit falls through the catch-all to `/login`. Nothing links to it; left as-is pending founder decision.
- check.sh ALL CLEAR, housekeeping --strict 0/0, fast suite 376. Testing agent iteration_203 covered the `/vs` pages and meta; title fix self-verified via Playwright.

## Sep 17, 2026 — Brief-fit audit follow-through: visible Stripe checkout path + four-pillar homepage (`reconcile`) · NOT PUSHED
Founder's brief-fit audit (8/10): no visible checkout/payment provider; DTS / CFP / CES / BEC absent from the homepage; `/customers` "renders homepage" (stale prod — closes on push). Founder chose (b) for both gaps.
- **Checkout path — Pick plan → Create account → Stripe, in one line.** `/pricing` card CTAs (main + reduced tiers) now go to `/start?plan=<id>&cycle=<cycle>`; `StartPage` preselects plan + cadence and scrolls to the plan grid. Anonymous "Subscribe" stores `sessionStorage.carryon_checkout_intent` → `/signup?redirect=start`; `SignupPage.postSignupPath()` sends the new account to `/start?resume=checkout` (both the skip-OTP and OTP-verify exits; partner-code path unchanged) where `StartPage` shows "Your account is ready. Taking you to secure checkout…" and hands off to Stripe. Resume with no intent / no user falls back to the normal page (no stuck spinner).
- **Pre-existing bug fixed:** `/start` and `/pricing` POSTed to `/api/subscriptions/create-checkout`, which does not exist (backend route is `/api/subscriptions/checkout`) — every logged-in "Choose Plan / Subscribe" on those pages silently no-op'd since the Job F merge. Both now use new `utils/stripeRedirect.js::startPlanCheckout()` (same route + body as the in-app paywall, persists the `carryon_pending_stripe_session` reconcile breadcrumb, honours `{free:true}` beta/free mode → `/dashboard`, sonner toast on failure).
- **Payment-provider copy:** `TrustBadges.js::StripeNote` ("Secure checkout by Stripe · your card never touches our servers") under every plan CTA on `/pricing` (3 main + 4 reduced), `/start` (3 main + special grid) and archived `/landing-consumer`; `/pricing` gains a "How paying works" 3-step strip (`pricing-how-paying-works`). `/landing-consumer` tier CTAs → `/start?plan=…&cycle=…`.
- **Homepage `#features` regrouped into the four pillars** (`LandingContent.js::PILLAR_GROUPS`, order/names per `benefactorSections.js` + `feature_gates.py`): People (Beneficiaries, MM, FFN, DTS) · Access (SDV, DAV, EGA) · Money (CFP, CES) · Action (IAC, CCP, ECT) = the canonical 12, numbered 01–12, each pillar with its section colour + tagline, plus a separate "For your family, after" card for Beneficiary Estate Concierge (`pillar-card-bec`). New plain-English copy drafted for Beneficiaries, DTS, CFP, CES, BEC (founder to review). End tile: "Four pillars. Twelve tools. One family." `llms.txt` "What CarryOn does" mirrors the same structure; "Digital Asset Vault" typo → Digital Access Vault; added a "Paying:" line.
- Verification: testing agent iteration_204 (7/8 pass; the one failure was the create-checkout 404 above, fixed after) + Playwright: `/pricing` → `/start?plan=premium&cycle=annual` preselects; logged-in resume and `/pricing` CTA both land on `checkout.stripe.com/c/pay/cs_live_…` (no payment made); anonymous resume shows the normal page. QA signup `info+qa-checkout-1789609989@carryon.us` purged via `services.erasure.erase_user` (`memory/scratch/purge_qa_user.py`). check.sh ALL CLEAR, housekeeping 0/0.
- Not done on purpose: `/features` is still not a route (nav anchor `#features`); no homepage pricing block exists (pricing lives at `/pricing`, linked from nav).

## Sep 17, 2026 — Founder headshot: HEIC accepted, browser-reachable URL, audit trail (`reconcile`) · NOT PUSHED
Founder report: photo uploaded on prod (computer, app.carryon.us → Site Content), green success toast, photo shown in the admin card — yet About/Customers show "BH". Probe: prod backend `carryon-api-kacr.onrender.com` `/api/public/founder-headshot` → 404 `No headshot uploaded`; prod `site-content` founder_* all empty; preview identical. So the document existed at upload time and is now gone on prod — nothing in the codebase deletes `site_assets` except the admin Remove button (grep: erasure manifest lists it NO_PII; no drops/TTL). Cannot read prod DB/Render logs from here (founder@carryon.us is not a valid prod login).
- **Backend `routes/admin/platform.py`**: accept `image/heic`/`image/heif` (+ `.heic/.heif` by name) via `pillow-heif` (`register_heif_opener`), still normalised to 512px JPEG. Upload and Remove now write hash-chained `audit_trail` rows (`founder_headshot_upload` / `founder_headshot_remove`, actor email + IP + size/source) so any future disappearance is attributable in the portal, not only in Render logs. `pillow-heif==1.7.0` added to requirements.txt (one line — `pip freeze` in this pod omits installed packages like openpyxl, do not use it to rewrite the file).
- **Backend `routes/public_content.py`**: `founder_photo_url` host now prefers `X-Forwarded-Host` (was `request.url.netloc` = internal cluster name on preview → 403 in the browser); adds `founder_photo_updated_at`.
- **Frontend**: `FounderCard.js::founderPhotoUrl()` — About page + `useFounder` (Customers/homepage card) always load `${API_URL}/public/founder-headshot?v=<updated_at>` (own API base, cache-bust past the 5-min `Cache-Control`). Admin uploader accepts HEIC/HEIF (input `accept` + type check by MIME or extension).
- Verified on preview: HEIC upload → 200 JPEG 300×300 → About `founder-photo` naturalWidth 300, placeholder hidden; Customers card same URL; bad type → 400; audit row written. Test headshot removed afterwards; preview's junk founder_bio/title ("Regression test bio for iteration 59") cleared so defaults render. check.sh ALL CLEAR.
- **Founder next (prod)**: push → redeploy → re-upload → open `https://carryon-api-kacr.onrender.com/api/public/founder-headshot` directly (must show the photo) → About. If it vanishes again: Admin audit trail / Render logs, search "headshot" — the remove row names the actor.

## Sep 17, 2026 — PWA status-bar overlap on `/start` (bug class fixed) · NOT PUSHED
Founder screenshot: `/start` header (logo + Sign In) rendered under the iOS clock/signal bar in standalone PWA. Cause: the `/start` and `/pricing` `<header>`s used plain `py-4` with no `env(safe-area-inset-top)` — unlike every other public page (LoginPage/HomePage/MarketingNav/LandingPage/About/Compare all inset). Fixed the class: `StartPage.js` + `PricingPage.js` headers → `paddingTop: calc(1rem + env(safe-area-inset-top, 0px))`; `TermsPage.js` + `PrivacyPolicyPage.js` (bare `py-12` wrappers) → `calc(3rem + env(...))`. Desktop/browsers unaffected (inset resolves to 0 → 16px, verified). Housekeeping E1–E3 unchanged; check.sh ALL CLEAR.
- **Same day — white-label login `/p/:slug` (PartnerPortalPage.js)**: on phones the hero content is taller than the viewport, so `justify-center` did nothing and the centred 260×160 partner logo started at `py-12` (48px) — under the absolutely-positioned "Create Account →" link (which already had its own safe-area inset). Hero container now `pb-12` + `paddingTop: calc(4rem + env(safe-area-inset-top, 0px))`, reserving the link row + status bar. Verified at 390px: link box y=16–36, logo starts y=64, no overlap; desktop unchanged.

## Sep 17, 2026 — PWA safe-area sweep (69 pages) + partner-page phone layout (`reconcile`) · NOT PUSHED
Founder asked for a full standalone-PWA sweep at iPhone width plus a smaller partner logo so the welcome line and sign-in form land above the fold.
- **Tooling** `memory/scratch/pwa_sweep.py` (+ `shot_pwa.py`): Playwright at 393×852 / iPhone UA, `navigator.standalone` + `display-mode: standalone` faked, QuickStart dismissed for the session, and — since Chromium reports `env(safe-area-inset-*)` as 0 — every stylesheet/inline declaration is rewritten to 59px/34px so the layout behaves like a Dynamic-Island iPhone in PWA mode. Then `elementsFromPoint` over the top-59px band flags any text/interactive element (at scrollY=0; only fixed/sticky elements when scrolled, since content scrolling under the bar is native iOS). Public + benefactor + beneficiary (minted dev-session token) + admin: 69 routes. Retries once on dev-server `ChunkLoadError` (the earlier "Uncaught runtime errors" overlay was that, not app code).
- **Fixed (bug class: top-of-screen chrome without the status-bar inset)**: `TrialLockdownBanner` was a static bar at document top (under the status bar, and the fixed mobile header overlapped it) → now a fixed slot via new `hooks/useTopBannerSlot.js` (mirrors TrusteeBanner: absorbs the inset when topmost, publishes its height into `--cy-offline-banner-h` so header/sidebar/content step down, stacks under an active trustee banner). `PartnerBriefPage` and `QuickStartTrialPage` sticky top bars, `SpeakWithUsPage` hero top, `VoicesPage` header CTA (22px text wrapped to two lines and overflowed the 64px row into the bar → single line, 16px on phones). `/start` `#pricing-section` scroll target gets `scroll-margin-top: 2rem + inset` so the pricing heading lands below the bar after the `/pricing → /start?plan=` hand-off.
- **Partner page (`/p/:slug`) on phones**: logo 200×96 max (260×160 on lg), tighter headline/tagline spacing, the 13 pillar chips move below the sign-in card on phones (`PillarChips`, desktop copy unchanged in the left column) — Sign In card now starts ≈525 CSS px down on an 852px iPhone, above the fold.
- Re-sweep after fixes: 0 collisions across all 69 routes (only leftovers: the `/start?plan=` heading, an artefact of the emulation running after the auto-scroll; and dev-server chunk timeouts on 3 routes). Preview data hygiene: beneficiary "Jane Smith" pointed at a deleted S3 photo (NoSuchKey error spam) → photo reference unset. check.sh ALL CLEAR.
- Visual review sheets: `/tmp/sweep/sheet3.png` (support/dashboard lockdown banner stack, partner page, /start, partner-brief, voices) — status bar drawn as a red band.

## Sep 17, 2026 — ROOT CAUSE of the founder-headshot "not showing" (`reconcile`) · NOT PUSHED · **founder must push**
Founder (frustrated, rightly): JPEG upload on prod → green toast → admin card still "No headshot uploaded yet", About page still "BH".
- **Cause**: the upload was never the problem. Prod `GET carryon-api-kacr.onrender.com/api/public/founder-headshot` returns 200 image/jpeg (his 13:11 UTC upload is stored). But `SecurityHeadersMiddleware` stamped `Cross-Origin-Resource-Policy: same-origin` on every response, and on prod the page origin is `carryon.us` while the API is `onrender.com` → browsers silently refuse to render the cross-origin `<img>`. The admin card's `onError` then showed the *empty* state (a lie), and the About page fell back to initials. Preview is same-origin (site + `/api` on one host) so it never reproduced there — which is why earlier probes looked fine. The earlier "no headshot on prod" 404 was simply the founder having clicked Remove/re-tried between probes.
- **Fix**: `middleware.py` → `headers.setdefault("Cross-Origin-Resource-Policy", "same-origin")` (handlers may opt out); `routes/public_content.py::get_founder_headshot` sends `Cross-Origin-Resource-Policy: cross-origin`. Only API-served image embedded cross-origin by `<img>` (beneficiary photos are presigned S3, partner logos are data URLs, attachments are fetched with auth). `SiteContentTab` now probes on `<img>` error and shows "Saved, but the browser blocked the preview" (`founder-headshot-blocked`) instead of "No headshot uploaded yet" when the server has the file.
- **Tests**: `tests/regression/test_founder_headshot_corp.py` (middleware preserves handler CORP; route sets cross-origin). Testing agent iteration_205: 7/7 — header, cross-origin embed simulation (about:blank `<img>` → API, naturalWidth 512), admin card, Replace Photo round-trip, /about + /customers render, HEIC accepted, audit rows. Its network-dependent e2e file moved to `memory/scratch/headshot_hotfix_e2e_smoke.py` (hits the live preview with creds — not for CI). Preview test headshot removed afterwards. check.sh ALL CLEAR.
- **Founder next**: Save to GitHub → Render redeploys the API → hard-reload `/about` (the 5-min image cache is per-URL; the `?v=` cache-bust handles it). No re-upload needed — the photo is already on prod.

## Sep 17, 2026 — Site Copy mini-CMS, Phase 1: every marketing string founder-editable (`reconcile`) · NOT PUSHED
Founder: "I want the ability to change the words throughout my entire public facing site, as desired, from within my Founder portal." Approved scope: marketing pages first; the 12 official tool names + 4 pillar names locked; plain text + line breaks; page titles / meta descriptions, FAQ included; testimonials stay in their own admin; editor in its own tab.
- **Backend `routes/site_copy.py`** (mounted in `server.py`, collection `site_copy` in the erasure manifest as NO_PII): `GET /api/public/site-copy` → `{overrides:{key:text}, updated_at}`; `PUT /api/admin/site-copy {changes:{key: text|null}}` (founder or `marketing` scope via `require_scope`), null/"" deletes the override (back to default). Keys must match `^[a-z0-9]+(?:\.[a-z0-9_-]+)+$` (blocks `$`-operators), text NFC-normalised, control chars stripped, ≤5000 chars, ≤400 changes per save; each save writes an `audit_trail` row `site_copy_update` with the set/reset key lists. Both routes registered in `route_policies.py`.
- **Frontend registry `src/copy/siteCopy.js`**: 360 fields, page → section → field `{k,label,d,locked,multiline}` with the current live text as the built-in default (Home incl. the `/` sign-in hero + `/home` hero, nav + footer, About, Founder gate, Security, Pricing, Start). `COPY_DEFAULTS` is the fallback — an empty/missing override always renders the default, so the public site can never show a blank.
- **Runtime `src/copy/CopyContext.js`**: `CopyProvider` (wrapped in `App.js`) fetches overrides once, lazily on the first `useCopy()` (logged-in app pages never pay for it), caches them in `localStorage.carryon_site_copy_v1` so repeat visits render the override on first paint; `t(key, vars)` fills `{days}`-style placeholders; `renderCopy()` turns `\n` into `<br>` and `**word**` into `<strong>` — never HTML. `applyOverrides()` lets the editor update the live site in the same tab after save.
- **Wired**: `LandingContent.js` (shared by `/` and `/home`: problem, outcomes, pillars/12 tools/BEC card, platform, steps, security, trust, FAQ + FAQPage JSON-LD, hospice, final CTA, footer — text now comes from keys; icon/colour/order stay in code; `dangerouslySetInnerHTML` on platform cards removed), `HomePage` + `LoginPage` heroes/nav/video headings/SEO, `HeroCtas`, `MobileNav`/`MarketingNav` (`MARKETING_LINKS` gained `k`; test ids still use the static label), `PublicFooter`, `TrustBadges::StripeNote`, `AboutPage`, `FounderAboutPage` (gate + sign-in mode), `SecurityPage` (all 8 sections), `PricingPage`, `StartPage`. `components/landing/heroCopy.js` deleted (folded into the registry). `<SEO>` title/description on Home, `/`, About, Founder, Security, Pricing, Start read from copy (Pricing/Start descriptions stay computed from live prices/trial days).
- **Founder UI — Admin → Marketing → Site Copy** (`/admin/site-copy`, `components/admin/siteCopy/{SiteCopyTab,SiteCopySection,SiteCopyField}.js`, registered in `adminSections.js` + `AdminPage.js`): page pills with edited counts, collapsible sections with field/edited/unsaved counts, one row per field (label, current text, Unsaved/Edited badge, "Reset to default", the default shown under an edited field, SEO char counters 60/160), locked fields rendered read-only with a lock, search across all pages (sections force-open, "no results" state), sticky save bar with unsaved counter + Discard + single Save (sends only real diffs; text equal to the default is sent as a reset).
- **Tests**: `tests/regression/test_site_copy.py` (18: sanitiser, key rules, public shape, round-trip + reset, 400s, 401/403) + testing agent `tests/test_site_copy_audit_iter206.py` (audit row). Testing agent iteration_206: backend 19/19, frontend 100% (override round-trip on /home, locked MM product name, FAQ + footer overrides, editor dirty→save→edited→reset→save, discard, search, locked field has no input, regressions on Site Content / pricing / MarketingNav). All test overrides reset — preview `site_copy` is empty. `fetchpriority` → `fetchPriority` on the hero `<img>`s (React DOM warning the agent spotted). pip-audit ratchet 32 → 33 is advisory-DB drift on `litellm` (pinned by emergentintegrations; no dependency changed) — baseline ratcheted, as on Sep 9. check.sh ALL CLEAR.
- **Not in Phase 1 (by agreement)**: non-marketing pages (Customers, Changelog, Compare/`/vs`, Voices, Wind-Down, Privacy/Terms, Speak-With-Us, Partner pages, in-app text), testimonial text (own admin), `Which plan is right` bullet detail and the `Why monthly` bullets on `/pricing` (contain live numbers), founder name/title/bio (already in Site Content).
- **Founder next**: Save to GitHub → Vercel + Render → open Admin → Marketing → Site Copy on prod, change one line, Save, reload the public page.

## Sep 17, 2026 — Site Copy Phase 2 + change history + live preview (`reconcile`) · NOT PUSHED
Founder picked all three follow-ups. Registry now 629 fields (Phase 1: 360).
- **Phase 2 pages wired** (`copy/siteCopyPhase2.js`, merged into `PAGES` in `copy/siteCopy.js`): Customers, Compare (`/vs` + the three competitor pages — every word except the competitor names; `data/compareData.js` is now structure-only: slugs, names, URLs, which rows are check marks; the "checked on" date is `compare.checked`), What's New (headings/CTA; dated entries stay in `public/changelog.json`), Voices, Wind-Down Promise (every card/bullet/revision note), Privacy, Terms (section heading + body per numbered section), Accessibility. New field flavours in `CopyContext.js`: `copyList()` (one item per line), `renderBlocks()` (line = paragraph, `- ` = bullet, `**bold**`, e-mail addresses auto-link when a `linkClass` is given). New shared `components/landing/MarketingFooter.js` replaces the three copy-pasted footers on Customers/Compare/Changelog (`footer.home`, `footer.short_copyright`).
- **Change history**: `PUT /api/admin/site-copy` now writes `site_copy_history` rows `{key, previous, next, actor_id, actor_email, at}` only when the value actually changed; `GET /api/admin/site-copy/history?key=&limit=` (marketing scope, key validated, limit ≤ 500). Collection registered in `erasure_manifest.RETAIN` (3 y, staff accountability) and both routes in `route_policies.py`. Editor: clock icon + count on any field with history → timeline (each saved version, "Before the first change", `Current` badge) with one-click **Restore** that saves immediately; save-bar **History** opens "Recent changes" across all pages (who · when · page › section › field, struck-through before / after, "Restore previous", click the field name to jump to it).
- **Preview before save** (`SiteCopyPreview.js`): save-bar **Preview** opens a right-hand drawer framing the live page same-origin with `?copyPreview=1`; `CopyProvider` in preview mode skips the API fetch/localStorage, posts `carryon:copy-preview-ready`, and applies `carryon:copy-preview` overrides from the parent (origin-checked) — saved + unsaved edits pushed on every keystroke. Desktop/phone (390px) toggle, path selector (Compare: 4 paths), reload, open-in-new-tab, close. Nothing is saved until Save. Works on prod because the admin (app.carryon.us) frames its own origin, not www.
- **Tests**: `tests/regression/test_site_copy.py` → 20 (history before/after/actor, no-op saves add no row, 401/403/400/422); testing agent iteration_207: backend 27/27, frontend 100% (Phase 2 defaults + overrides on every page, editor pills/hints, field + recent-changes restore, preview drawer incl. path switch and phone width, Phase 1 regression). Preview `site_copy` left empty; history rows kept (audit log).
- **Housekeeping note**: `check.sh` (yarn audit stage) rewrote `frontend/yarn.lock` (+2253/−911 lines — the committed lockfile is stale vs `package.json`, e.g. `@capacitor/android ^6` vs `^8`). Restored with `git checkout frontend/yarn.lock`; not touched on purpose — a lockfile regeneration should be a deliberate founder decision before a Vercel build.
- Known console noise (pre-existing, not ours): visual-editor tooling injects `<span data-ve-dynamic>` into `<select>/<tr>/<tbody>` → React hydration warnings on /customers and /vs pages in the preview environment only.

## Sep 18, 2026 — Site Copy Phase 3: in-app text, scheduled copy, one-click Review, lockfile refresh (iteration_208) · VERIFIED, NOT PUSHED

- **In-app text is editable**: Sign-up wizard (`/signup`), Subscription paywall (`/subscription`) and Onboarding (welcome page + dashboard Setup Guide tile, step names, tips) read from `copy/siteCopyApp.js` (`PAGES_APP`, +131 fields → 760 total). Text with `{curly}` placeholders (`{company}`, `{n}`, `{total}`, `{days}`, `{done}`, `{menu}`…) is filled at runtime via `t(key, vars)`; the placeholder must stay when rewording (Review flags it if it goes missing).
- **Scheduled copy** (`site_copy_schedules`): calendar icon on any field → text to show, go-live + optional revert time (**entered and displayed in US Eastern** — founder choice; stored UTC; `SiteCopySchedule.js::easternToIso` is DST-aware), note. `GET /api/public/site-copy` now returns the *effective* text (saved overrides + whichever schedules are live; latest start wins per key; empty scheduled text = built-in default); the editor loads base text from `GET /api/admin/site-copy/state` (`overrides`, `effective`, `schedules` with `status` upcoming/active/ended). Save-bar **Schedules** panel lists all with Remove; field shows a "Live now" chip + note when a schedule is overriding its saved text. Endpoints: `GET/POST /api/admin/site-copy/schedules`, `DELETE …/schedules/{id}` (marketing scope, audited `site_copy_schedule_create/delete`; 400 on bad dates / end ≤ start / window already ended).
- **Review** (save-bar button): checks your unsaved edits if any, else every field on the current page (or the search results) — `POST /api/admin/site-copy/review` (≤200 fields per call; client batches). Deterministic: double/stray spaces (fix), leading/trailing whitespace (fix), unbalanced `**`, HTML tags, space-before-comma / double period, missing `{placeholder}`, SEO title > 60 / description > 160, empty text. AI typo pass on `XAI_MODEL_LIGHT` (temperature 0, brand names ignored, cost-ledgered as `site_copy.review`); LLM findings that duplicate a deterministic fix are dropped; if xAI is unavailable a notice says only automatic checks ran. Results panel rows: type chip · page › section › field (jump) · message · **Apply fix** (puts corrected text in the editor as an unsaved edit — nothing saves until Save); flagged fields get an amber marker + inline issue rows; sections show "· N flagged".
- **`frontend/yarn.lock` regenerated deliberately** (`yarn install`, 1490 → 1678 resolutions; `yarn install --frozen-lockfile` → "Already up-to-date"; dev server compiles). check.sh's yarn-audit stage no longer rewrites it.
- Tests: `tests/regression/test_site_copy.py` 23/23 (+3 Phase 3 tests: schedule live/revert/delete, validation + scope, deterministic review + endpoint); testing agent iteration_208 backend 35/35 (`tests/test_site_copy_phase3_iter208.py`), frontend 100% (override round-trip to /signup, Review + Apply fix, schedule Live now → public swap → remove → revert, paywall/onboarding defaults). `housekeeping.sh --strict` 0 WARN / 0 FAIL (fixed the E4 input-zoom WARN on the new schedule form). Preview DB left clean (no overrides, no schedules).
- **Founder next**: Save to GitHub → Vercel/Render → on prod: Site Copy → Sign-up wizard → change a line → Review → Save → open /signup; schedule a headline for 5 minutes from now (ET) and watch it swap and revert.

## Sep 18, 2026 (later) — Site Copy drafts, copy alerts, /readiness-score, Guides (launch-gated) (iteration_67) · VERIFIED, NOT PUSHED

- **Drafts** (`site_copy_drafts`; save-bar **Drafts** button): "Save as draft" stores unsaved edits under a name (fields from any page); panel lists drafts (name · N fields · pages · who/when) with **Open** (loads into the editor as unsaved edits; Preview/Review work on it; save bar shows `Draft: name`), **Update**, **Publish** (applies every field in one press — history rows carry `via = draft name`, shown as `· draft “name”`; draft removed) and **Delete**. Endpoints `GET/POST /api/admin/site-copy/drafts`, `PUT …/{id}`, `POST …/{id}/publish`, `DELETE …/{id}` (marketing scope, audited `site_copy_draft_*`). `PUT /api/admin/site-copy` now shares `apply_changes()` with publish.
- **Copy alerts** (`services/site_copy_alerts.py`, scheduler `site_copy_alerts` every 60 s, lock TTL 120 s, registered in server.py + scheduler_worker.py): when a schedule goes live / reverts, every `role=admin` account gets a Resend e-mail (page › field, window in ET, **Before** / **Now showing** blocks, "Open the live page" → `https://www.carryon.us{page_path}`) plus an in-app `notify.founder`. Claim-then-send via `live_alert_at` / `revert_alert_at` stamps (never double-sends across pods). Switch: `platform_settings.copy_alerts_enabled` (default on) — `GET/PUT /api/admin/site-copy/alerts`, checkbox in the Schedules panel. Schedules now store `page_path/page_label/field_label/before_text` (frontend fills them from the registry; `before_text` = what visitors saw).
- **/readiness-score** (public, indexable, sitemap + llms.txt + prerender): plain-English policy for the Estate Readiness Score — short version, the four equal parts with the exact tiers (Documents 27/80/100; Messages 8/7/5/3/1 by age; Checklist 15 = 100; Financials 30/25/25/20), what moves it, what never counts, the separate CCP score (20/15/15/10/10/10/10/10 + labels), commitments, CTA. 36 fields in `copy/siteCopyReadiness.js` (page pill "Readiness Score"). Linked from MarketingFooter, PublicFooter, homepage footer (`footer.readiness`) and a "How is this calculated?" link under the dashboard `ReadinessDial` (`showPolicyLink=false` in the Settings preview).
- **Guides — built, NOT public** (founder directive: "don't publish with any authority yet"): `/guides` + 5 articles (`copy/siteCopyGuides.js`, 105 fields, page pill "Guides") — 12 documents in ten minutes · milestone letter · executor + digital executor · first 72 hours · contingency plan in one evening. Byline "Barnet Harris, Founder" (editable), disclaimer, CTA, Article + BreadcrumbList + CollectionPage JSON-LD, reading time. **Launch switch**: Admin → Marketing → **Guides** (`GuidesTab`, `PUT /api/admin/guides/launch`, `GET /api/public/guides/status`; flag also rides on `GET /api/public/site-copy` → `flags.guides_launched`, exposed via `useCopy().flags`). Unlaunched: visitors get a noindex "This section isn’t open yet." page, no footer link; logged-in admins preview with a gold banner. Launch: pages public immediately, footer "Guides" links appear, `datePublished` = launch moment; `scripts/guides-launch.js` (build-prod.sh) adds the guide URLs to the built sitemap.xml and prerender list **only when launched** — a plain Vercel Redeploy after launching is enough.
- Tests: `tests/regression/test_site_copy.py` 27/27 (+4: draft lifecycle, alerts toggle + e-mail builder, scheduler claims once, guides switch); testing agent iteration_67 backend 13/13 (`tests/test_new_features_iter67.py`), frontend 100% (readiness page, unlaunched/preview/launch/unpublish guides, multi-page draft save → discard → open → update → publish → history via, alerts checkbox, dashboard link). `housekeeping.sh --strict` 0 WARN / 0 FAIL (marketing pages added to the dark-bg allow-list; settings-singleton projection marked). Preview DB clean (overrides {}, no drafts/schedules, guides unlaunched, alerts on).
- **Founder next**: Save to GitHub → deploy. On prod: read the five guides at /guides (you'll see the preview banner), edit anything in Site Copy → Guides, and press Launch only when ready; then Redeploy in Vercel so sitemap/prerender pick them up.


## Sep 18, 2026 (PM) — Guide legal review, scheduled draft publish, score explainers, guide share cards (iteration_68) · VERIFIED, NOT PUSHED

- **Guide Review Pass** — Review panel gains a **Flag legal-advice wording** toggle (`site-copy-review-legal`; `POST /api/admin/site-copy/review {…, "legal": true}`): deterministic phrase check (`routes/site_copy.py::legal_phrase_issues` — "you must file/sign/notarize…", "required by law", "is binding/void", universal deadlines) plus an xAI pass (`llm_legal_issues`) that quotes any sentence reading as legal / tax / medical *advice*; issues typed `legal`. Admin → Marketing → **Guides** has **Review the five guides** (`guides-review`) → `/admin/site-copy?page=guides&review=legal`, which switches to the Guides pill and runs the review on arrival (`SiteCopyTab` deep-link effect; `?field=key` also jumps to a field).
- **Scheduled Publish** — Drafts panel row: **Publish at (ET)…** → datetime (US Eastern) → `PUT /api/admin/site-copy/drafts/{id}/schedule {publish_at ISO | null, pages, field_labels}` (400 bad/past, 404, 403 non-marketing; audited `site_copy_draft_schedule/unschedule`). Chip "Publishes <ET> · set by <who>" with **Change** / **Cancel schedule**. `services/site_copy_alerts.py::publish_due_drafts` (same 60 s loop, claim via `publishing_at`) applies the draft through `apply_changes(via=draft name)`, deletes it, and — when alerts are on — e-mails every founder "Draft published: <name>" with Before / Now showing per field and "Open <page>" buttons, plus an in-app `notify.founder`. Draft rows in History carry `via = draft name`, actor = who scheduled it.
- **Score Explainer Link** — every dashboard pillar tile shows a one-line "<Feature> N% · …" explainer (only enabled features) with **Why?** → `/readiness-score#people|access|money|action`; policy page pillar cards have `id={key}` + `scroll-mt-28` (`readiness-pillar-{key}`), CCP card `#contingency`.
- **Guide Share Cards** — `GET /api/public/guides/{slug}/card.png` (`routes/guides.py`, Pillow, `backend/assets/fonts/Outfit-*.ttf` + `carryon-logo.png`): 1200×630 dark-navy card, logo + "CarryOn · Guides" eyebrow, wrapped title in Outfit Bold (auto-shrinks), gold rule, byline + `carryon.us/guides`; title comes from the effective Site Copy value so edits show up on the next fetch (`Cache-Control: public, max-age=3600`, ETag from title, CORP cross-origin; 404 for unknown slug). `GuidesPage` passes it as `<SEO image alt>`; `SEO.js` repoints the static `og:image` / `twitter:image` / `og:image:alt` tags (no duplicates) and restores the default on unmount; Article JSON-LD `image` uses the same URL.
- Tests: `tests/regression/test_site_copy.py` 30/30 (+3: legal phrase/API, draft schedule + self-publish + e-mail builder, share card + title override). **Fixed flaky test**: publish time is now taken right before each request with a 6 s margin (the bcrypt login before it could push a 2 s target into the past → 400) and both in-process scheduler tests share one event loop (`_run`) because `asyncio.run` closed the loop the module-level motor client was bound to ("Event loop is closed" when run after `test_alert_scheduler_claims_each_schedule_once`). Testing agent iteration_68: backend 4/4 (`tests/test_iter68_extras.py`: 1200×630 via PIL, cache header, override changes bytes, schedule error paths) + 3/3 regression re-run; frontend 100% (deep link → legal review auto-runs on 105 guide fields, schedule/cancel chip in ET, four Why? links + anchors as benefactor, og:image per guide and revert on /about). `housekeeping.sh --strict` 108 PASS / 0 WARN / 0 FAIL; `yarn install --frozen-lockfile` clean. Preview DB clean (overrides {}, no drafts/schedules, guides unlaunched, alerts on).
- **Founder next**: Save to GitHub → deploy. On prod: Admin → Guides → **Review the five guides** and read the legal flags before Launch; save a draft, **Publish at (ET)** 5 min out, watch it go live + the e-mail; open the dashboard → **Why?**; paste a guide URL into a social preview checker to see the card.

## Sep 19, 2026 — heycatch.ai site-audit pass (76/100 baseline) · VERIFIED, NOT PUSHED

Audit + full findings saved at `memory/audits/heycatch_2026-09-18.md`. Applied every fix the audit prescribes that can be done from the repo; everything at full marks untouched.
- **D2.4** `Compare` nav link → `/vs` between Pricing and Customers (`MobileNav.js::MARKETING_LINKS`, so homepage desktop nav, hamburger menu and every `MarketingNav` subpage get it; editable label `nav.compare`). `/pricing` keeps its own minimal header (deliberate; Pass dimension).
- **D3.1** `LiveCountBadge` (`LiveStats.js`) — "124 families set up — live count" pill in the homepage hero under the three security indicators, links to `#trust`; same `/api/public/platform-stats` source + visibility rule as the panel. Label `home.hero.livecount`.
- **D5.1** `home.seo.description` 210 → 154 chars (audit wording, em dash kept).
- **D1.2** `home.problem.stat` — 570-hours line (same claim as `pricing.anchor`) rendered as a callout under the italic line (`problem-stat`).
- **D1.3** `home.outcomes.3` → "Be seen as the one who cared enough to plan" (social JTBD), body keeps "No awkward conversations required".
- **D1.5** `ProductPreview` — checklist ("What to do first") is now tab #1 and the default; eager-loads that shot.
- **D1.6** DAV card H4 "Passwords and accounts", official-name sub-line hidden on the homepage only (`HIDE_OFFICIAL_NAME`; `ToolCard` renders the line only when present) and dropped from the Product JSON-LD featureList; EGA title "An AI review of your documents" (branded name stays beneath); `home.pillars.sub` + `home.handled.text` rewritten without "Four pillars. Twelve tools" (founder: public pages only — pillar/tool names and in-app copy untouched); `/security` meta description + `security.encryption.1` lead with "Scrambled before it’s stored", AES-256-GCM in parentheses.
- **D3.3** `FOUNDER_LINKEDIN_DEFAULT = https://linkedin.com/in/barnetharris` (founder-confirmed) — FounderCard, /about card and Person JSON-LD `sameAs`; Admin → Site Content override still wins.
- **D3.5** Trustpilot slot: `platform_settings.trustpilot_url` (admin allowed key, public on `/api/public/site-content`), Admin → Marketing → Site Content → **Independent Reviews (Trustpilot)** card (regex-validated), `ReviewsCard` in the homepage trust section (Read the reviews / Write a review = `/evaluate/`), one-line review ask on the onboarding welcome screen (`onboarding.welcome.review`). All render nothing until the URL is set. **Founder must create the Trustpilot profile.**
- **Not changed, flagged**: D4.2 / Brief-fit prices (founder: brief is out of date, live Stripe prices are correct); D3.2 testimonials (framework exists, zero published — founder collects); D3.4 money-back guarantee / SOC 2 badge; D2.3 features-section length; D2.5 (auditor-capped); D2.1 hero wording (D1.1 Pass).
- Housekeeping: `anyio` 4.12.1 → 4.14.2 (two new pip-audit advisories published today; count back to baseline 33). Testing agent iteration_69: backend 4/4, frontend all audit flows pass (only note: /pricing header has no Compare — intentional). Regression 47/47 (`test_site_copy` 30 + iter67 13 + iter68 4). `check.sh` ALL CLEAR, `housekeeping --strict` 0/0. Preview DB clean (`trustpilot_url` '').
...
## Sep 21 2026 — Founder is its own menu item everywhere (NOT PUSHED)
...
## Sep 21 2026 — "Meet the Founder" pill on the founder tile (NOT PUSHED)
...
## Sep 22 2026 — Production hardening pass from the second (site-wide) audit (NOT PUSHED) — iteration_70 12/12 backend + frontend PASS
Verified false/stale and NOT acted on: "zero-knowledge" homepage chips (live bundle has only the honest sentence); entity-name claim (contradicts founder option B); mobile header "collision" (just content under a fixed header). Founder decisions applied:
- **Retired phrases off every surface**: Guardian PDF footers (6), trial-reminder + billing + system e-mail footers → "Access controlled and audited"; API field `zero_knowledge` → `access_model`.
- **Timeline**: founded 2025 / generally available 2026 (FAQ, homepage promise card, /customers intro, /changelog ×2, JSON-LD foundingDate).
- **Vercel** (`frontend/vercel.json`): HSTS `max-age=63072000; includeSubDomains; preload`, nosniff, X-Frame-Options SAMEORIGIN (Site Copy preview iframe is same-origin), Referrer-Policy strict-origin-when-cross-origin, Permissions-Policy (camera/mic/geolocation self — ECT share-location uses geolocation; payment self + Stripe), COOP same-origin-allow-popups (Stripe/iOS download popups), ACAO pinned to https://www.carryon.us, **CSP in Report-Only** with `report-uri` → `POST /api/public/csp-report` (30-day TTL, grouped in Admin → Compliance → SOC2 Readiness "CSP findings" card, `GET /api/admin/csp-reports`). Apex→www 308 redirect rule added (only fires if the domain-level redirect is turned off).
- **Real 404s**: rewrites now list the 68 real top-level route segments from App.js → `/shell.html`; anything else falls to branded `public/404.html` (Vercel serves it with status 404). `/guides` stays SPA (its pre-launch page is noindex).
- **Meta Pixel consent-gated**: `index.html` loader only runs when `localStorage.carryon_consent_analytics === 'granted'` (or `window.__carryonLoadPixel()` after Allow); `<noscript>` beacon removed; `AnalyticsConsent` bar (public browser visitors only; hidden in PWA/native/signed-in). Privacy §6 names Meta + consent.
- **Customers** hidden from the top nav until ≥3 approved stories (`flags.published_stories` from `/public/site-copy`; `visibleLinks()`); footers + homepage module unchanged.
- **Hygiene**: build stamp = git SHA@UTC time (`REACT_APP_BUILD_ID` in build-prod.sh, 'dev' on preview); quotaGuard warning silenced in prod / only real errors; `HEAD /api/health` allowed; YouTube → `YouTubeFacade` (click-to-play, youtube-nocookie) on /, /customers, /moments — 0 iframes on first paint; public fetch dedupe `utils/publicCache.js::getPublic` (site-content 5→1, platform-stats 2→1); 44 px tap targets (hamburgers, Sign In, Start).
- **Security copy**: HSTS bullets now describe the header actually shipped (2 years, includeSubDomains, preload flag).
- Awaiting founder: demo-estate address change on prod → screenshot re-capture; hstspreload.org submission; Vercel "expose system env vars" check.

## Sep 22 2026 — Sub-processor list · contrast pass · Search Console tab (NOT PUSHED)
- **Privacy §6 sub-processors** (appended inside §6 so section keys/overrides stay aligned): Stripe, Render, Vercel (+Speed Insights), AWS S3, MongoDB Atlas, Resend, Twilio, xAI (Grok), Google (Places, Fonts, Analytics-with-consent, YouTube), Meta (with consent), Sentry, Apple/Google Play, HighLevel/LeadConnector. "Last updated: September 22, 2026". Also found Firebase/Google Analytics initialising on `/get-started` without consent → now gated behind the same Allow (`services/firebase.js` listens for `carryon:consent-granted`); notice text + privacy bullet name Google Analytics; CSP report-only lists GA hosts.
- **Contrast (WCAG 4.5:1)**: scanned 25 public routes with axe + a computed-contrast walker. Theme vars dark `--t4 #6B7A90→#8B97AB`, `--t5 #475569→#8492A8`; light `--t4→#5B6B82`, `--t5→#64748B`. In public files: `#6b7a90`→`#8b97ab` (31), text uses of `#334155/#3a4a63/#2d3d55/#475569/#4a5568/#64748b`→`#8492a8` (73; borders/backgrounds untouched); footer hover `#7b879e`→`#c9d2e0`. `/our-promise` (cream page): H1 inherited the dark-theme heading colour (1.05:1, invisible) → fixed; dark PublicFooter now sits on its own navy band. `/get-started` (light glass cards): fixed slate text instead of dark-theme vars. Result: 0 failing text nodes on all 25 routes (gold-on-gradient buttons excluded as false positives).
- **Marketing → Search Console** (`routes/admin/search_console.py`, `SearchConsoleTab.js`): `GET /api/admin/seo/index-status` (snapshot or 5 setup steps when `GSC_SITE_URL` / `GSC_SERVICE_ACCOUNT_JSON_B64` are absent), `POST …/refresh` (URL Inspection for 25 canonical pages + 28-day Search Analytics, thread-pooled; friendly 403/429 messages; snapshot in `seo_index_status`, auto-refresh when >24 h old). Verified with canned Google responses; live call needs the founder's service account (steps in chat). google-api-python-client / google-auth already in requirements.
