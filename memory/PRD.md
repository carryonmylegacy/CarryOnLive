...
## LOCKED-IN FEATURES — DO NOT REGRESS (User-Verified April 2, 2026)

**These features have been confirmed working by the user on production iOS/PWA. Any future code change MUST be verified against this list. If a change touches ANY file listed below, run the full housekeeping + end-to-end download test before declaring ready.**

### 1. Video MM Download (iOS PWA)
- **Status**: CONFIRMED WORKING
- **Flow**: User taps download → blob fetched via download proxy → `navigator.share()` → iOS share sheet ("Save Video") → saved to Photos
- **Large file handling**: If blob fetch takes >5s (activation expires), a "Tap to Save" overlay appears → user taps → fresh activation → share sheet
- **Backend**: WebM→MP4 conversion via `imageio-ffmpeg` bundled binary (`imageio_ffmpeg.get_ffmpeg_exe()`)
- **Files**: `downloadFile.js`, `downloads.py` (lines 155-191), `messages.py` (get_message_video)
- **DO NOT**: Use `window.location.href`, `window.open`, or `<a download>` for iOS downloads. These cause blank SFSafariViewController screens.
- **DO NOT**: Remove `imageio-ffmpeg` from requirements.txt or switch to system ffmpeg.
- **DO NOT**: Change `bytes(pdf.output())` back to `pdf.output()` — `bytearray` crashes Starlette Response.

### 2. Text MM PDF Download (iOS PWA)
- **Status**: CONFIRMED WORKING
- **Flow**: Same as video but returns a PDF generated by `fpdf2`
- **Unicode**: `_pdf_safe()` sanitizes smart quotes, em dashes, ellipsis, emoji before PDF rendering (Helvetica = Latin-1 only)
- **Files**: `messages.py` (_build_text_pdf, _pdf_safe), `downloadFile.js`
- **DO NOT**: Replace `fpdf2` with hand-rolled PDF builders. The xref offsets will be wrong and iOS will show blank.
- **DO NOT**: Remove `_pdf_safe()` — iOS keyboards produce Unicode that crashes fpdf2 encoding.
- **DO NOT**: Return `pdf.output()` directly — must wrap in `bytes()`.

### 3. ECT Beneficiary Avatars
- **Status**: CONFIRMED WORKING
- **Flow**: Backend resolves S3 keys → presigned URLs via `resolve_photo_url()`. Falls back to beneficiary record photos if user record has none.
- **Files**: `estate_chat.py` (imports `resolve_photo_url` from `services/photo_urls.py`)
- **DO NOT**: Return raw `photo_url` from MongoDB without calling `resolve_photo_url()`.
- **DO NOT**: Remove the beneficiary photo fallback query in `_enrich_channel()`.

### 4. CCP Plan PDF Download
- **Status**: CONFIRMED WORKING
- **Files**: `downloads.py` (_handle_ccp_plan), `ConnectedProtocolPage.js`
- **DO NOT**: Return `pdf.output()` without `bytes()` wrapper.

### 5. Railway Build
- **Status**: CONFIRMED WORKING
- **Fix**: `--extra-index-url https://d33sy5i8bnduwe.cloudfront.net/simple/` as line 1 of `requirements.txt`
- **DO NOT**: Remove this line — `emergentintegrations` is only available from this private index.

### 6. Universal Download Proxy Architecture
- **Endpoints**: `POST /api/downloads/prepare` (auth) → `GET /api/downloads/{token}` (no auth, one-time-use)
- **12 action types**: `message_pdf`, `message_video`, `message_voice`, `document`, `ega_checklist`, `ega_todo`, `ega_iac_report`, `ega_transcript`, `ega_plan`, `beneficiary_iac`, `ect_file`, `ccp_plan`
- **Files**: `routes/downloads.py`, `services/download_tokens.py`, `utils/downloadFile.js`
...
- **DO NOT**: Remove `promptToSave()` — it's the fix for large-file user activation timeout.

---

## What's Been Implemented

### Completed (April 2, 2026 — iOS Download System Hardened)

**Full Download Stack (User-Verified)**
- Video MM downloads via iOS share sheet (save to Photos)
- Text MM downloads as valid PDFs (fpdf2 + Unicode sanitization)
- CCP Plan PDFs, SDV documents, EGA exports, ECT attachments — all via `platformDownload()`
- "Tap to Save" overlay for large files where `navigator.share()` activation expires
- `bytes(pdf.output())` fix across ALL fpdf2 usage (messages.py, downloads.py, guardian.py, pdf_export.py)
- ECT avatars showing actual photos via `resolve_photo_url()` with beneficiary fallback
- Railway build fixed with private PyPI index URL in requirements.txt
- Housekeeping: 60/60 PASS, ruff clean, zero warnings
...
- **Downloads**: ALL file downloads MUST go through `platformDownload()` in `/app/frontend/src/utils/downloadFile.js`. DO NOT use `window.location.href`, `window.open`, or `<a download>` on iOS. ALL `fpdf2` output MUST be wrapped in `bytes()` before passing to Starlette `Response`. Refer to the LOCKED-IN FEATURES section at the top of this file.
- **SVG in JSX**: Platform's Babel plugin wraps dynamic JSX expressions inside SVG elements in a `<span>`, breaking SVG rendering. Use `dangerouslySetInnerHTML` for any dynamic SVG content.
- **FamilyTree.js**: DO NOT modify styling unless explicitly instructed.
- **ECT Avatars**: Always use `resolve_photo_url()` for any `photo_url` returned from MongoDB. Raw S3 keys start with `/api/photos/` and will NOT display in `<img>` tags.
- **Railway Build**: `requirements.txt` MUST have `--extra-index-url https://d33sy5i8bnduwe.cloudfront.net/simple/` as its first line.
- **Onboarding Flow (March 23, 2026)**: Simplified to 4 steps max.
- **iOS Safe Area (March 23, 2026)**: Platform-wide fix — all Radix UI popper components use `collisionPadding` via `getSafeAreaTop()`.
