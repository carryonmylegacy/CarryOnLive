...
**Sep 26 2026 (latest) — Video poster fix (NOT PUSHED) · previous push verified live.** carryon.us main bundle + Render API confirmed carrying the Sep 22–23 work (founder summary, Explore centering, ladder order, Seniors bullets). Fixed the grey "bot" poster on the portrait video: YouTube's missing-thumbnail placeholder is a 120×90 image the browser renders (no `onError`), so the facade now walks a ladder and skips anything ≤120px; portrait embeds pass `vertical` and get the true 9:16 `oar2.jpg` frame first. `poster` prop available for a custom uploaded image later.

**Sep 23 2026 (latest) — Node.js 24 for Vercel (NOT PUSHED).** Vercel emailed: Node 20 builds fail after Oct 1 2026. Pinned `engines.node = "24.x"` in `frontend/package.json` (Vercel honours this over the dashboard), `.node-version` → 24, refreshed a stale `yarn.lock`. Production build verified on Node 24.21.0 locally (Compiled successfully) and check.sh ALL CLEAR. Founder's dashboard click (Settings → Build and Deployment → Node.js Version → 24.x) is optional hygiene. Render (backend) is Python — not affected.

**Sep 23 2026 (latest) — Homepage founder tile fills its right side on the web (NOT PUSHED).** `FounderCard withSummary` (homepage only): when the Founder page is public and the site is NOT running as an installed PWA, the empty right side shows "Why I built CarryOn" + a 3-sentence summary of the Founder page + a gold text link "Learn more about the Founder →" (replaces the pulsing pill). Below `md` it stacks under the name block. PWA keeps the pill/tile exactly as before. All three strings are Site Copy fields (`home.trust.founder.summary*`). Verified desktop web, mobile web, and simulated PWA on preview.

**Sep 22 2026 (latest) — Audit hash chain: honest re-anchor (NOT PUSHED).** Prod card showed "Chain broken · first broken link 2026-05-20" — the known pre-CAS fork (chaining shipped May 18; writer was read-latest-then-insert until the atomic head pointer landed June 5; two concurrent writes claimed the same predecessor). `verify_audit_chain` now treats rows stored before `audit_chain_state.created_at` as the pre-CAS era: each row's SHA-256 is still recomputed (an edit anywhere is a break), but links are enforced only from the first CAS-era row onward; pre-CAS twins are counted as `historical_forks` (+ `first_fork_at`, `pre_cas_rows`, `link_enforced_from`). Nothing rewritten — append-only intact. Card shows the census line ("Links enforced from <date> (atomic head pointer). N earlier chained rows hash-verified · 1 historical fork (first <date>) — pre-CAS write race, no content changed"). Regression: `tests/regression/test_audit_chain_precas.py` (scratch DB; fork→ok, pre-CAS edit→break, CAS-era twin→break, no anchor→legacy behaviour). Expected on prod after deploy: green, "1 historical fork (first 2026-05-20)".

**Sep 22 2026 (latest) — Tiles: existing plain / additions gold below · Subs matrix in tier-ladder order (NOT PUSHED).** On `/start` each tile lists its existing features as plain text (no check), then what the tier adds beneath them with the gold check; the beneficiary line sits apart, just above the Subscribe button (hairline separator). Order inside every tile and in Admin → Finance → Subs → Feature Gates now comes from `feature_gates.py::ladder_order(gates)`: what Base includes → what Standard adds → what Premium adds → special-tier-only → OFF everywhere (canonical order inside each band; `PLATFORM_FEATURES` itself unchanged). Used by `GET /subscriptions/plans` (`tier_features`, so the in-app paywall follows too) and `GET /admin/feature-gates` (rows re-sort on next load after publish). Verified on preview; check.sh ALL CLEAR.

**Sep 22 2026 (latest) — /start plan tiles rebuilt around the Subs toggles (NOT PUSHED).** Seniors was in the main grid AND the special row (Sep 20 filter miss) → now special only. One shared `components/start/PlanTile.js`: every tile lists its ON features from `tier_features` (canonical order, official names from the locked `home.tools.*.product` registry, no abbreviations, no "Everything in X"; stored `plan.features` bullets no longer used on `/start`). Gold check + bold = added over the baseline (Standard vs Base, Premium vs Standard, every special tier vs Base); legend lines above each group. Special Pricing = one framed box with Military · Veteran · Seniors · New Adult · Hospice ($0.00, "Start at no cost", no Stripe note), `flex-wrap justify-center` → 3+2 desktop / 2+2+1 tablet / stacked phone; hospice text link removed. "People you invite: free while you're alive" → **"Unlimited beneficiary enrollment — free for your lifetime"** (`start.plans.invited`, shown when the Beneficiaries gate is ON; also the one-plan box default; migration 0008 drops a stale override). `/pricing` still carries the old line (founder scoped this to /start). Verified at 1920/768/390 + checkout hand-off; check.sh ALL CLEAR.
...
**Sep 21 2026 (latest) — Approval e-mail Brian → Barnet (NOT PUSHED).** `founder_invites.py` L236; zero "Brian" left in source. check.sh ALL CLEAR.
