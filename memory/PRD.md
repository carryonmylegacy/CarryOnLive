...
- **DRY: getAuthHeaders Consolidation**: Removed 3 redundant `getAuthHeaders()` definitions from SubscriptionPage, SettingsPage, and SecuritySettingsPage. All now use the single definition from AuthContext.
...
- **Feature: Estate Name Personalization Prompt**: One-time modal that appears on login for benefactors whose estate still has the default "{LastName} Family Estate" name. Users can enter a custom name (with Save/toast) or dismiss ("Keep current name"). Either action sets `name_customized: true` on the estate, preventing the prompt from ever returning. New endpoints: `GET /api/estates/rename-check` and `POST /api/estates/customize-name`.
- **Admin Estate Diagnostic**: Added `GET /api/admin/estate-diagnostic` endpoint showing all estates grouped by owner with beneficiary counts, flagging users with duplicate estates. Founder-only, read-only.
- **Regression Verified**: All 38 housekeeping checks pass. ESLint clean. ruff clean. Frontend build passes. Testing agent confirmed 100% pass rate (iterations 129, 130, 131).
