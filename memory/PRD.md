...
**Sep 19 2026 (night, later) — Phone product shots start at the content (NOT PUSHED).** `m-contacts.webp` opens on the beneficiary tiles, `m-vault.webp` on the document cards (header / buttons / explainers scrolled out; desktop unchanged). `capture_product_screenshots.py` `PAGES` rows carry desktop + phone scroll targets. Verified on the `/home` phone frame; check.sh ALL CLEAR. **Founder next**: push → Vercel → phone → "See inside CarryOn" → "Who to call first" / "Document vault".
