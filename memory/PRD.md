...
- User decision: option **b** — reconcile backends first, then make every feature live in both environments and keep the preview DB mirroring live config.
- **ROOT CAUSE FOUND (Sep 16 night, from user's Render PDFs + public GitHub repo `carryonmylegacy/CarryOnLive`)**: two Emergent jobs forked from commit 5631da78 on Apr 12. Job L (other chat) built the real product (2,274 commits → ac0663c, running on Render). THIS job (Job F, 50 commits of marketing) **force-pushed GitHub main on 2026-09-15 03:07 UTC**, orphaning Job L, and Republished the frontend on Sep 16 (now on www + app.carryon.us). Job L's tip is preserved locally as git branch **`live-render`** in /app. Full evidence + phased plan: `/app/memory/RECONCILIATION_PLAN.md`. Recommended: Option A (Job L base, port Job F marketing). Awaiting user approval + marketing-set decision.
