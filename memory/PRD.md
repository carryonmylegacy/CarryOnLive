...
- **Beneficiary Succession Hierarchy**: Benefactors can now set a ranked succession order (Primary → Secondary → Tertiary → etc.) via drag-and-drop on the Beneficiaries page. Key changes:
  ...
  - Frontend: Cards display succession badges (PRIMARY, SECONDARY, TERTIARY, etc.) with color-coded styling. "Succession Hierarchy" explainer box explains drag-to-reorder. Old "Designate as Primary" / "Change Primary" buttons removed.
