...
**Erasure service — inventory + design delivered for founder approval (Sep 1 2026; see `/app/memory/ERASURE_DESIGN.md`). NO CODE WRITTEN. Waiting on decisions D1–D8.** `/security` transcript sentence replaced (approved wording; ships with the next push). Key findings: existing Settings → Privacy "Request deletion" button writes `deletion_requests` that nothing processes (promises 30-day deletion + email); `otps` stores plaintext codes with a dead TTL; `activity_logs` phantom collection in estates.py. Dependency pass scheduled before erasure Phase 2 (pypdf/pillow/python-multipart/starlette first).
