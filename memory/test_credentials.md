...
## /benefactor acquisition page (added Sep 19, 2026)
- Public, noindex: `/benefactor` (Premium only) and `/benefactor?v=all` (three plans). Logged-out CTAs → `/signup` with `sessionStorage.carryon_signup_intent = {"preferred_plan":"premium","landing_page":"benefactor"}`; "Subscribe today" → `/start?plan=premium&cycle=monthly`.
