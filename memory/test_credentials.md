...
- Site Content (Sep 19, 2026): `trustpilot_url` in `PUT /api/admin/platform-settings` + `GET /api/public/site-content` — MUST be left `""` on preview (founder has no Trustpilot profile yet); when set, the homepage `reviews-card` and onboarding `onboarding-review-ask` appear.
- Drafts / alerts / guides (Sep 18, 2026 PM): drafts `GET/POST /api/admin/site-copy/drafts`, `PUT|DELETE …/{id}`, `POST …/{id}/publish`, `PUT …/{id}/schedule {"publish_at": ISO|null, "pages":[{path,label}], "field_labels":{}}` (a due draft is published by the 60 s loop and e-mails founders for real — cancel/delete QA drafts well before their time) — delete QA drafts when done; alerts switch `GET/PUT /api/admin/site-copy/alerts {"enabled"}` — leave enabled=true (a live schedule e-mails founder@carryon.us for real); review legal pass `POST /api/admin/site-copy/review {"fields":[…], "legal": true}` (issues typed `legal`); guides `GET /api/public/guides/status`, `PUT /api/admin/guides/launch {"launched"}` — MUST leave launched=false (founder has not approved publishing); share card `GET /api/public/guides/{slug}/card.png` (public, 1200×630 PNG from the effective title). Public pages: /readiness-score (anchors #people #access #money #action #contingency), /guides (unlaunched → "isn’t open yet", admins preview with banner). Admin: /admin/guides (`guides-review` deep-links to `/admin/site-copy?page=guides&review=legal`).
- Regression suite needs `REACT_APP_BACKEND_URL` exported: `cd /app/backend && REACT_APP_BACKEND_URL=$(grep REACT_APP_BACKEND_URL /app/frontend/.env | cut -d= -f2) python -m pytest tests/regression/test_site_copy.py -q` (otherwise every API test skips).

## Acquisition pages /benefactor, /ready, /moments + attribution (Sep 19–20, 2026)
- Public, noindex: `/benefactor` (Premium only; `?v=all` three plans), `/ready` (quiz first), `/moments` (Milestone Messages). Admin read-out: `GET /api/admin/funnel-analytics/landing-pages?days=30` (founder token) → rows per `landing_page`; fed by `POST /api/diagnostics/funnel-event {event:'landing_view'|'landing_cta_click', meta:{page}}`. `/sources` is public + indexed. Logged-out CTAs → `/signup` with `sessionStorage.carryon_signup_intent = {"preferred_plan":"premium","landing_page":"benefactor"}`; "Subscribe today" → `/start?plan=premium&cycle=monthly`.
- `POST /api/auth/register` accepts optional `preferred_plan` (must be in `PLAN_ORDER`, else dropped) + `landing_page`; `GET /api/auth/me` returns `preferred_plan`. QA accounts: `backend/tests/test_benefactor_iter210.py` (auto-purge) — OTP codes are read from `db.otps` (Resend is live; codes are NOT in the logs).
- Trial length: `platform_settings {key:"trial_policy"}.trial_days` = **10** on preview to mirror the founder portal — leave it.
